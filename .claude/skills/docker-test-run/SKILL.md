...
1. **Перед PR**: все тесты (Docker-команды выше)
2. **Разработка фичи**: только unit-тесты
3. **Проверка интеграции**: интеграционные тесты
4. **Быстрая проверка**: запуск без пересборки
5. **Перед правкой таймингозависимого кода**: перф- и медленные тесты — в PR-гейтах их нет
