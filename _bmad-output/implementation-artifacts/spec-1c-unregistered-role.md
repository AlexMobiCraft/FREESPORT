...
- Импорт 1С назначает роль **только при создании** записи. ~~Роль существующего пользователя импорт не меняет никогда — её выдаёт менеджер при верификации.~~ **Отменено** — см. Spec Change Log, запись «2026-08-05 — итерация 3». Вторая половина правила действует лишь для записей, проходящих `unlinked_1c_record_q()`; роль привязанного аккаунта приезжает из соглашения 1С.
...
| Импорт существующего | Контрагент из 1С совпал с пользователем, у которого `role='wholesale_level2'` | ~~Данные обновляются, `role` остаётся `wholesale_level2`~~ **Отменено** (Spec Change Log, «2026-08-05 — итерация 3»): данные обновляются, а `role` берётся из вида цен соглашения 1С, если запись не проходит `unlinked_1c_record_q()` | N/A |
...
- ~~Given пользователь с ролью `wholesale_level2`, выданной менеджером, when запускается импорт контрагентов из 1С, then роль остаётся `wholesale_level2`.~~ **Отменено** — см. Spec Change Log, «2026-08-05 — итерация 3». Действующий критерий: роль остаётся прежней, только если 1С не отдала однозначного вида цен, несущего роль, либо запись проходит `unlinked_1c_record_q()`.
...
**KEEP — должно пережить перевывод кода.** (1) Устранение первопричины в `processor.py`: удалённые `ROLE_MAPPING`/`map_role`, фильтр по `<Роль>Покупатель</Роль>`, ~~отказ от перезаписи роли существующего пользователя~~ **отменено** (Spec Change Log, «2026-08-05 — итерация 3») — сохраняется только запрет менять роль записи, проходящей `unlinked_1c_record_q()`. (2) `_find_by_tax_id`, возвращающий список кандидатов вместо `.get()` — снимает `MultipleObjectsReturned` независимо от привязки. (3) Обе миграции и признак `password=''` для выборки записей 1С. (4) Проверенный факт: парсер отдаёт `<Роль>` для всех 500 контрагентов реального XML, фильтр отсеивает 0.

### 2026-08-05 — итерация 3: роль привязанного аккаунта приезжает из 1С (стори 40.4)

**Что отменено.** Правило «импорт никогда не меняет роль существующего пользователя» — во всех четырёх местах, где оно повторено: Boundaries → Always, строка I/O-матрицы «Импорт существующего», соответствующий Acceptance Criterion и пункт 1 блока KEEP. Три из четырёх мест находятся внутри `<frozen-after-approval>`; правка санкционирована AC стори 40.4 в `epics.md` (решение Alex) и является тем самым renegotiation, которого требует атрибут `reason`. Остальное содержимое frozen-блока не изменялось.

**Чем заменено.** Источник истины по уровню цен — соглашение в 1С (FR-40-07, FR-40-12). Роль привязанного аккаунта заменяется ролью, разрешённой из вида цен соглашения через справочник `PriceType.user_role`, включая случай, когда прежняя роль выдана менеджером вручную; прежнее значение сохраняется в `AuditLog` с `action="role_from_1c"` (FR-40-08). Отчёт сессии импорта объясняет каждое решение девятью счётчиками (FR-40-09).

**Что осталось в силе.** Роль назначается при **создании** записи — новый контрагент по-прежнему получает `unregistered`. Запись, проходящая `unlinked_1c_record_q()`, роли не получает ни при каком виде цен: смена её роли выбила бы запись из всех путей привязки разом.

**Известно-плохое состояние, которого это избегает.** Портал считает клиента «Опт 3», 1С — «Опт 2»; расхождение всплывает на заказе, а не в отчёте, и его приходится ловить вручную. Обратная сторона отмены — если бы правило было ослаблено и для непривязанных записей, вернулся бы баг, чинившийся миграцией `0018`: регистрация с известным ИНН отказывает, фильтр «Кандидат 1С» пуст, `find_link_candidates` возвращает пустой список.
