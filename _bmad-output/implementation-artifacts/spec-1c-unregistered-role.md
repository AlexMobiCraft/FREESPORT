...
- Не удалять `PortalLinkConfirmView`, `PortalLinkConfirmSerializer` и задачу письма — они остаются в коде неиспользуемыми (решение человека), удаление вынесено в `deferred-work.md`. Маршрут `auth/portal-link/confirm/` при этом снят: держать открытый эндпоинт, меняющий email и пароль записи 1С, ради невызываемого кода нельзя (решение второго раунда ревью).
...
### 2026-07-26 — итерация 2: находки второго раунда ревью

**Что вскрыло ревью.** (1) Проверка дубля ИНН шла через `normalize_inn`, принимающий только 10 и 12 цифр, тогда как `validate()` допускает 8–12 для Беларуси и Казахстана — 9-значный УНП проверку не проходил, и одну компанию можно было регистрировать неограниченно. (2) `is_unlinked_1c_record` опирался на `not self.password`, но `create_user(password=None)` пишет `"!<random>"`, и такая запись блокировала регистрацию всей компании; Django при этом считает пустой пароль usable, так что идиоматичная проверка дала бы обратный ответ. (3) `validate_role` запрещал только `unregistered`, а `role="admin"` принимался. (4) Парсер читал первый `<Роль>` через `find()`, тогда как CommerceML повторяет элемент — контрагент «Поставщик + Покупатель» пропускался и переставал синхронизироваться. (5) Условие показа РРЦ `userRole !== 'retail'` открывало B2B-цены новой роли. (6) `approve_b2b_users` не ставил `is_active=True`, поэтому одобренная заявка не работала — а после отключения привязки этим путём идёт весь B2B. (7) Password-reset выдавал доступ к записи 1С мимо менеджера, и «покрывающий» тест не проверял мок. (8) Маршрут `auth/portal-link/confirm/` оставался открытым без тестов.

**Что изменено.** Все восемь закрыты; (6)–(8) — по отдельному решению человека, поскольку выходили за исходные границы спеки. Маршрут снят, view оставлен. Добавлены тесты на каждый пункт, включая инвариант «импорт создаёт запись, удовлетворяющую `is_unlinked_1c_record`» и разбор нескольких `<Роль>`.

**Известно-плохое состояние, которого это избегает.** Регистрация, проходящая проверки, но приводящая к аккаунту, которым невозможно пользоваться; вход в аккаунт юрлица через сброс пароля; безлимитная регистрация по одному белорусскому УНП.
