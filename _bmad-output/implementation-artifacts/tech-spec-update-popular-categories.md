...
В настоящее время блок "Популярные категории" на главной странице (и на `/test`) отображает произвольный набор из 6 корневых категорий. Бизнес-требования предписывают отображать конкретный упорядоченный список из 7 категорий.
...
- **Компонент**: `CategoriesSection` используется на `src/app/page.tsx` (Продакшн) и `src/app/test/page.tsx` (Тест).
...
- **Manual**: Проверить визуально на `/test` (Desktop/Mobile).
