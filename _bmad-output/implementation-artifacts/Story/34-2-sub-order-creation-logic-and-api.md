...
**Результаты Twentieth Follow-up (2026-04-18) — OrderItem.product nested contract, numeric order.id, transport_schedule enum, TS clean:**
- Backend не изменялся в этом follow-up; последний backend результат — `124 passed` (Eighteenth Follow-up).
- Frontend полный suite: `133 test files passed, 2218 tests passed, 14 skipped` (добавлены 3 новых теста: `items[].product nested object`, `numeric order.id`, `transport_schedule` contract regression).
- `npx tsc --noEmit`: чистый вывод, exit code 0 (устранены все pre-existing TS ошибки в тест-файлах).
...
- ✅ Resolved review finding [High]: `OrderItem.product` синхронизирован с backend depth=1 contract — добавлен `OrderItemProduct` interface `{ id: number; name: string }`; `OrderItem.product: number` заменён на `OrderItem.product: OrderItemProduct`; MSW mock items, `OrderSuccessView.tsx` key и 3 test files обновлены. AC12 подтверждён для detail response.
- ✅ Resolved review finding [Medium]: `order.id`/`orderId` numeric contract синхронизирован — MSW GET `/orders/:id/` теперь возвращает `id: Number(params.id)` (numeric), test `getById` переведён с UUID `'550e8400...'` на `'1'` с ожиданием `result.id === 1`. Contract regression тест `items[].product nested object` добавлен.
- ✅ Resolved review finding [Medium]: `transport_schedule` добавлен в `DeliveryMethodCode` union type — `DeliveryMethod` теперь покрывает все 5 backend значений. Contract regression тест `transport_schedule является валидным значением` добавлен в `ordersService.test.ts`.
- Дополнительно: устранены pre-existing TypeScript ошибки в 4 тест-файлах (`OrderSuccessView.test.tsx`, `OrderDetail.test.tsx` — добавлены Story 34-1 поля; `OrdersList.test.tsx` — переведён на `OrderListItem[]`; `ordersService.test.ts` — type assertions через `unknown`). `npx tsc --noEmit`: чистый.
- Frontend полный suite (Twentieth Follow-up): `133 test files, 2218 tests passed, 14 skipped`. TypeScript: чистый.
...
- `frontend/src/types/order.ts` — добавлен `OrderItemProduct` interface `{ id, name }`; `OrderItem.product` изменён с `number` на `OrderItemProduct`; `transport_schedule` добавлен в `DeliveryMethodCode`.
- `frontend/src/__mocks__/handlers/ordersHandlers.ts` — items[].product переведены на объект `{ id, name }`; GET `/orders/:id/` возвращает numeric `id: Number(params.id)`.
- `frontend/src/components/checkout/OrderSuccessView.tsx` — исправлен key: `item.product.id` вместо `item.product`.
- `frontend/src/services/__tests__/ordersService.test.ts` — `getById` тест переведён на numeric contract (`'1'` → `result.id === 1`); добавлены 2 contract regression теста (nested product, transport_schedule); исправлены pre-existing TS type assertions через `unknown`.
- `frontend/src/components/checkout/__tests__/OrderSuccessView.test.tsx` — items[].product → объект; добавлены Story 34-1 поля в mockOrder.
- `frontend/src/components/business/OrderDetail/OrderDetail.test.tsx` — items[].product → объект; добавлены Story 34-1 поля в mockOrder.
- `frontend/src/components/business/OrdersList/OrdersList.test.tsx` — `mockOrders` переведён с `Order[]` на `OrderListItem[]` (соответствует component props).
- `_bmad-output/implementation-artifacts/Story/34-2-sub-order-creation-logic-and-api.md` — закрыты 3 Nineteenth Follow-up items; обновлены File List, Completion Notes, Debug Log, Change Log, Status → review.
...
### Twenty-First Follow-up Review (AI)
...
- Найдено 4 issue: 1 High, 3 Medium.
- Type-level синхронизация detail-contract из Twentieth follow-up не доведена до runtime UI: пользовательские экраны всё ещё локально не покрывают все backend `delivery_method` значения.
- Основной риск: AC12 заявлен как закрытый, но success/detail pages продолжают показывать raw backend codes для `transport_company`/`transport_schedule`, а текущий test suite это не ловит.

#### Findings

1. **[High] `OrderSuccessView` остаётся несовместимым с backend `delivery_method` contract** — в компоненте маппинг способов доставки содержит ключ `transport`, которого backend не возвращает, и не содержит `transport_company`/`transport_schedule`. Для заказов с этими значениями UI выводит сырые коды вместо русских лейблов, что нарушает обещанную backward compatibility клиентского контракта после Story 34-2. [frontend/src/components/checkout/OrderSuccessView.tsx:42-47, frontend/src/components/checkout/OrderSuccessView.tsx:119-124, frontend/src/types/order.ts:22-27]
2. **[Medium] `OrderDetail` покрывает `transport_company`, но по-прежнему не локализует `transport_schedule`** — после Twentieth follow-up тип `DeliveryMethodCode` уже включает это значение, однако детальная страница заказа не обновила runtime label map. В результате один из валидных backend delivery codes всё ещё не доведён до user-facing UI. [frontend/src/components/business/OrderDetail/OrderDetail.tsx:71-76, frontend/src/components/business/OrderDetail/OrderDetail.tsx:188-192, frontend/src/types/order.ts:22-27]
3. **[Medium] Regression coverage не защищает runtime fix для новых delivery codes** — tests, изменённые в Twentieth follow-up, валидируют типы `OrderItem.product`, numeric `order.id` и сам union `transport_schedule`, но не проверяют фактический рендер локализованных лейблов в `OrderSuccessView`/`OrderDetail`. Поэтому текущий runtime баг спокойно прошёл зелёным suite'ом. [frontend/src/services/__tests__/ordersService.test.ts:324-336, frontend/src/components/checkout/__tests__/OrderSuccessView.test.tsx:132-155, frontend/src/components/checkout/__tests__/OrderSuccessView.test.tsx:272-276, frontend/src/components/business/OrderDetail/OrderDetail.test.tsx:114-118]
4. **[Medium] `Dev Agent Record -> File List` не отражает полный текущий review snapshot** — фактический git status включает изменённый `_bmad-output/implementation-artifacts/sprint-status.yaml`, но `File List` Twentieth follow-up документирует только frontend files + story. Это оставляет несинхронизированный changed-files audit trail, хотя story уже переведена обратно в reviewable lifecycle. [_bmad-output/implementation-artifacts/Story/34-2-sub-order-creation-logic-and-api.md:371-391, _bmad-output/implementation-artifacts/sprint-status.yaml:129-132]

#### Action Items

- [ ] [High] Исправить runtime mapping delivery labels в `OrderSuccessView` под backend values `transport_company` и `transport_schedule`.
- [ ] [Medium] Добавить `transport_schedule` в label map `OrderDetail` и проверить локализацию на detail page.
- [ ] [Medium] Добавить regression-тесты на рендер локализованных labels для `transport_company`/`transport_schedule` в success/detail UI.
- [ ] [Medium] Обновить `File List` так, чтобы текущий review snapshot включал `sprint-status.yaml` и оставался воспроизводимым.
...
| 2026-04-18 | Twentieth review follow-ups: resolved 3 items (1 High, 2 Medium). `OrderItem.product` синхронизирован с depth=1 backend contract — введён `OrderItemProduct` interface, mock/test objects обновлены. Numeric `order.id` contract закреплён в MSW handler и тесте. `transport_schedule` добавлен в `DeliveryMethodCode`. Pre-existing TS ошибки в 4 тест-файлах устранены. TypeScript: чистый. Frontend полный suite: `133 files, 2218 tests passed`. Status → review. |
| 2026-04-18 | Twenty-First follow-up code review (AI, YOLO): добавлены 4 новых Review Follow-ups (1 High, 3 Medium). Выявлен runtime drift по локализации `delivery_method` в `OrderSuccessView`/`OrderDetail` после type-level fix Twentieth follow-up и несинхронизированный `File List` относительно текущего snapshot (`sprint-status.yaml` не отражён). Status → in-progress. Outcome: Changes Requested. |
