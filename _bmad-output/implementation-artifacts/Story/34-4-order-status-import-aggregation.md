...
- [x] [Review][Patch] `master_order_ids` содержит мастеров с изменением только `sent_to_1c_at` и может дублировать id между batch'ами, хотя AC11 определяет его как список мастеров с изменённым `status` или `payment_status` [backend/apps/orders/services/order_status_import.py:881] — исправлено: aggregated_master_ids теперь set[int], add внутри проверки status/payment_status
- [x] [Review][Patch] Legacy master без `sub_orders` может обновиться с `updated_count > 0`, но сигнал `orders_bulk_updated` уйдёт с пустым `order_ids`, потому что `updated_sub_ids` пополняется только для субзаказов [backend/apps/orders/services/order_status_import.py:208] — исправлено: updated_sub_ids.append(sub.pk) для всех обновлённых заказов
- [x] [Review][Patch] Отсутствует заявленное AC15 e2e-покрытие через `_handle_orders_xml` / HTTP `mode=file`, несмотря на отмеченный выполненным Task 11.8 [backend/tests/integration/test_order_status_import_db.py:537] — добавлен test_sub_orders_status_update_aggregates_master в test_orders_xml_mode_file.py
- [x] [Review][Patch] AC13-тест `vat_group=None` проверяет только helper `_aggregate_master_status`, но не сценарий XML-импорта sub-order с `vat_group=None`, описанный в acceptance criteria [backend/tests/unit/test_order_status_import.py:2348] — добавлен test_vat_group_none_sub_included_in_xml_import_aggregation в test_order_status_import_db.py
...
- ✅ Review Finding 1: `aggregated_master_ids` переведён на `set[int]`, `add()` внутри проверки status/payment_status — дедупликация и AC11 compliance
- ✅ Review Finding 2: `updated_sub_ids.append(sub.pk)` для всех обновлённых заказов (включая legacy master без parent_order_id) — сигнал с непустым `order_ids`
- ✅ Review Finding 3: добавлен `test_sub_orders_status_update_aggregates_master` в `test_orders_xml_mode_file.py` — e2e через HTTP mode=file (AC15/Task 11.8)
- ✅ Review Finding 4: добавлен `test_vat_group_none_sub_included_in_xml_import_aggregation` в `test_order_status_import_db.py` — AC13 XML-импорт sub с vat_group=None
- 📝 Итого после review: 133 теста (94 unit + 17 integration + 22 mode=file) — все зелёные, flake8/black чисто
...
- `backend/tests/integration/test_order_status_import_db.py` — TestMasterAggregationDB (7 тестов, +1 AC13 vat_group=None XML-import); обновлён query count в test_bulk_fetch
- `backend/tests/integration/test_orders_xml_mode_file.py` — добавлен test_sub_orders_status_update_aggregates_master (e2e HTTP mode=file, Task 11.8)

## Change Log

| Date | Change |
|------|--------|
| 2026-04-21 | Review follow-ups: 4 findings исправлены — AC11 signal compliance, legacy master order_ids, e2e HTTP test, AC13 XML-import test. 133 теста — все зелёные |
