...
Закрытие 4 замечаний ревью (2026-08-06, раунд 2 — только тестовые файлы и артефакты):

- `pytest -q tests/unit/test_services/test_link_1c_customer.py tests/integration/test_link_applies_role_from_1c.py` → **41 passed** (число тестов не изменилось: усилены существующие, новых не заводилось).
- Регресс Task 6.2 (9 файлов) + новый интеграционный — **219 passed**, 3:35. Состав совпадает с прогоном раунда 1.
- Полный прогон `pytest -q` **без** `-m` — **2994 passed, 6 skipped, 19 subtests passed, 0 failed**, 38:34. Регрессий нет, состав совпадает с прогонами 2026-08-05 и раунда 1.
- `black --check` + `flake8` на обоих тестовых файлах — чисто (`2 files would be left unchanged`).
- `manage.py makemigrations --check --dry-run` → `No changes detected`.
- `git diff HEAD --stat` — 4 файла: два тестовых, story-файл, `sprint-status.yaml`. `apps/**` не затронут (AC14).
- `npx gitnexus detect-changes --scope all` → `No changes detected`: изменённые файлы — тесты и артефакты, индексированных символов правки не касаются.
...
- ✅ **Resolved review finding [Patch]: интеграционный тест не фиксировал допустимую B2B-роль.** Довзвод роли справочника ужесточён до `!= EXPECTED_ROLE` (константа `wholesale_level2`) — `retail`/`admin`/`wholesale_level1` у засеянной записи больше не дают ложного падения.
- ✅ **Resolved review finding [Patch]: AC2 покрыт вакуозным тестом.** Цель теперь заводится с непустым `onec_price_type_id` (`OPT4_GUID` → `wholesale_level4`), а роль остаётся `wholesale_level2` — ассерт различает «не затёрли» и «затёрли пустым» и попутно доказывает, что резолвер читает источник. Добавлены ассерты по журналу: обоих ключей нет в `transferred_fields`, прежний GUID есть в `previous_values`.
- ✅ **Resolved review finding [Patch]: в журнале `sprint-status.yaml` указан несуществующий переход.** Запись от 2026-08-06 исправлена на «статус сохранён review»; фактический единственный переход диапазона — `backlog -> review` (подтверждено `git diff c880773b..HEAD`).
- ✅ **Resolved review finding [Patch]: незаявленный `tech-debt.md` в story-диффе.** Файл добавлен в File List с пояснением несвязанного scope — по правилу, уже принятому для `AGENTS.md`/`CLAUDE.md`.
- **Продакшен-код в этом раунде не менялся.** Изменены два тестовых файла, `sprint-status.yaml` и story-файл; `git diff HEAD --stat` подтверждает, что `apps/**` не затронут (AC14).
...
- `_bmad-output/planning-artifacts/tech-debt.md` — MODIFIED (несвязанный scope; см. Review Findings)
...
- [x] [Review][Patch] Интеграционный тест не фиксирует допустимую B2B-роль [backend/tests/integration/test_link_applies_role_from_1c.py:132] — при уже существующем `PriceType` тест меняет `user_role` только если она пуста. Роль `retail`/`admin` либо `wholesale_level1` допустима для справочника, но сервис по AC6 обязан оставить роль заявителя неизменной; тест же ожидает смену и ложно упадёт. В подготовке сценария принудительно задать активной записи `user_role="wholesale_level2"`, как в unit helper, и сохранить только реально изменённые поля.
  - **Резолюция (2026-08-06).** Замечание принято. Ожидаемая роль вынесена в модульную константу `EXPECTED_ROLE = "wholesale_level2"` (в `User.B2B_ROLES`, отличается от исходной `wholesale_level1` заявителя), условие довзвода изменено с `if not price_type.user_role` на `if price_type.user_role != EXPECTED_ROLE` — теперь `retail`, `admin` и `wholesale_level1` у засеянной записи справочника перезаписываются, а не приводят к ложному падению. Список реально изменённых полей и единственный `save(update_fields=...)` сохранены. Комментарий переписан: объясняет, почему «только из пустой» недостаточно. Ожидаемая роль по-прежнему читается из объекта справочника (`expected_role = price_type.user_role`), GUID не зашит (5.4).

- [x] [Review][Patch] AC2 покрыт вакуозным тестом [backend/tests/unit/test_services/test_link_1c_customer.py:442] — `test_empty_source_price_type_does_not_wipe_target` оставляет `onec_price_type_id` цели пустым. Такой ассерт не отличает корректное «не затирать» от ошибочного присваивания пустого значения. Создать цель с непустым видом цен и проверить, что пустой источник сохраняет это значение и роль.
  - **Резолюция (2026-08-06).** Замечание принято. Цель заводится с непустым `onec_price_type_id` — намеренно «говорящим» `OPT4_GUID`, засеянным через `ensure_price_type(..., "wholesale_level4")`. Теперь тест различает «не затёрли» и «затёрли пустым», а сохранение роли `wholesale_level2` при виде цен, дающем `wholesale_level4`, дополнительно доказывает, что резолвер читает **источник**, а не присвоенное значение цели (подзадача 1.5). Добавлены три ассерта по журналу: `onec_price_type_id` и `role` отсутствуют в `transferred_fields` (изменений не было), а `previous_values["onec_price_type_id"]` равен прежнему GUID цели — обратная сторона AC7/AC9, ранее не покрытая.

- [x] [Review][Patch] В журнале sprint-status указан несуществующий переход [_bmad-output/implementation-artifacts/sprint-status.yaml:78] — запись от 2026-08-06 сообщает `in-progress -> review`, хотя предыдущая запись уже перевела стори в `review`, а статус до изменений этого диапазона был `backlog`. Заменить на фактическое «статус сохранён `review` после закрытия замечания».
  - **Резолюция (2026-08-06).** Замечание принято, факт подтверждён `git diff c880773b..HEAD -- sprint-status.yaml`: в диапазоне ровно одно изменение ключа — `backlog -> review`. Запись исправлена на «Статус сохранён review: перехода не было, ключ перешёл backlog -> review записью от 2026-08-05 и с тех пор не менялся». Добавлена отдельная запись за текущий раунд закрытия замечаний — с той же формулировкой про сохранённый статус.
...
| 2026-08-06 | 1.2 | Закрыты замечания ревью — 4 пункта: принудительная B2B-роль в интеграционном тесте, содержательный тест AC2 (цель с непустым видом цен + ассерты журнала), исправлена запись перехода в `sprint-status.yaml`, `tech-debt.md` внесён в File List с пояснением. Продакшен-код не менялся. Статус сохранён review | claude-opus-5 |
