...
- [x] **T9. Проверка на проде (AC9).** Выкат выполнен 2026-08-27, проверка снята: 0 `failed`, 0 зависших, 0 непрочитанных файлов, каталог пуст, 6 вариантов из 2470 без свежего остатка (след прогона до guard'а). Ограничение — прогон на 9 сессиях, полномасштабный не повторён; см. Completion Notes.
...
- [x] AC9 проверен на проде после реальной выгрузки, результат записан в Dev Agent Record. Первый прогон (223 сессии) вскрыл остаточный дефект `mode=complete`, он исправлен и выкачен; повторный прогон дал 0 `failed`. Оговорка про размер повторного прогона — в Completion Notes.
