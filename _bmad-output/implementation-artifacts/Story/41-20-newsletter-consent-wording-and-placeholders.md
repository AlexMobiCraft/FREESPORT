...
#### Третье ревью 18.09.2026

- [x] [Review][Patch] AC5: добиться буквального нуля `example` в production bundle — решение владельца 18.09.2026 отменяет vendor-исключение: найти источник строки `Example` в общем чанке `3688-912aa751a9ddbcb4.js`, устранить её из клиентского bundle без ослабления проверки и повторить fail-closed проверку всех пяти страниц без учёта регистра. До подтверждённого результата Story остаётся `in-progress`. **Закрыто стори 41.21, 18.09.2026:** источник — middleware `devtools` zustand (не `destroy`-предупреждение, как ошибочно записано выше), подключён только вне production (`stores/devtoolsInDev.ts`, коммит `a2a717ab`); прод `ce727776`: 0 вхождений `example` без учёта регистра в 341 загрузке JS-чанков 16 публичных страниц рецепта 1.6 стори 41.21 (включая пять страниц AC5 41.20), `zustand devtools middleware` — 0; fail-closed гейт `npm run check:build` в `frontend-ci.yml`.
- [x] [Review][Rejected] К Story не приложено воспроизводимое browser/chunk evidence AC5 — решение владельца 18.09.2026: существующее визуальное подтверждение владельца и зафиксированный результат chunk-проверки признать достаточным production evidence; повторный снимок не требуется. Статус `done` сохранить.

##### Rejected

- [Rejected][medium] Последовательный deploy действительно создаёт краткое окно несовместимости frontend/backend, но изменение требует пересмотра утверждённого контракта отклонения прежних версий и deployment-стратегии в самой Story; открытая старая вкладка уже документирована как намеренно получающая `consent_text_outdated`.
- [Rejected][low] Регистрозависимый `grep -q "example"` не ловит `Example`, но это дефект acceptance-команды в Story, а не runtime-кода; исправление только документационное и не устраняет пользовательский дефект.
- [Rejected][false] `/password-reset` не обязан содержать чекбокс рассылки: AC1 задаёт четыре формы с чекбоксом, а AC3 — пять форм с email-полем; агрегированная формулировка AC5 не доказывает требование чекбокса на пятой странице.
- [Rejected][low] Команда 4.8 по слову `placeholder` действительно видит разрешённый Tailwind-класс, но фактический DOM-инвариант проверяет отдельный зелёный тест; исправление касается только текста Story.
- [Rejected][medium] Повторный сигнал про окно несовместимости deploy отклонён по той же причине: корректное изменение требует продуктового решения и правки контракта Story, а не однозначного code patch.
- [Rejected][low] Формулировка цели о соответствии 152-ФЗ/38-ФЗ шире тестируемого изменения, но это мотивация Story, а не доказанное runtime-нарушение; исправление меняет спецификацию.
- [Rejected][false] Равенство действующих маркетинговых текстов уже закреплено реестром, кросс-граничным тестом четырёх форм и точными ожиданиями D5 в component-тестах; отдельная проверка равенства в одном backend-тесте не требуется.
- [Rejected][false] В каждой из пяти форм текущего scope ровно одно `input[type="email"]`; выбор первого email-поля не пропускает существующий дефект.
- [Rejected][false] Ручной `PUBLIC_FORMS` содержит все пять маршрутов, прямо перечисленных AC3; гипотетическая будущая форма не входит в scope Story 41.20.
- [Rejected][false] Ни одно проверяемое email-поле сейчас не использует несколько ID в `aria-labelledby`; описанный отказ теста на текущей разметке недостижим.
- [Rejected][false] Числа 218 и 222 относятся к явно датированным последовательным прогонам до и после review-фиксов; финальный результат обозначен как 222 passed, противоречия нет.
- [Rejected][low] Перечень GitNexus-символов в Completion Notes неполон как отчёт, но текущий impact по всем пяти формам и `CONSENT_TEXT_VERSIONS` имеет LOW risk; исправление затрагивает только Story.
- [Rejected][low] Два integration-теста расположены не в той секции File List, а новый story-файл назван `UPDATE`; это косметические ошибки отчёта без влияния на код или приёмку.
...
- 18.09.2026 — решение владельца после третьего code review: vendor-исключение отменено, AC5 трактуется буквально — требуется 0 вхождений `example` без учёта регистра в production bundle. Добавлен открытый `[Review][Patch]`; статус `done` → `in-progress`. Реализация bundle-исправления не выполнялась.
- 18.09.2026 — `[Review][Patch]` третьего ревью закрыт стори 41.21 (прод `ce727776`: 0 вхождений `example` без учёта регистра в 341 загрузке JS-чанков 16 публичных страниц рецепта 1.6 стори 41.21 (включая пять страниц AC5 41.20), `zustand devtools middleware` — 0). Статус → done.
