...
- [ ] [Review][Patch] AC5: добиться буквального нуля `example` в production bundle — решение владельца 18.09.2026 отменяет vendor-исключение: найти источник строки `Example` в общем чанке `3688-912aa751a9ddbcb4.js`, устранить её из клиентского bundle без ослабления проверки и повторить fail-closed проверку всех пяти страниц без учёта регистра. До подтверждённого результата Story остаётся `in-progress`.
