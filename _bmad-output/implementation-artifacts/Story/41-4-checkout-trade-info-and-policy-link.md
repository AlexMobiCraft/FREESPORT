...
> ⚠️ **Пересмотрено 2026-09-10** (`_bmad-output/planning-artifacts/sprint-change-proposal-2026-09-10.md`). Стори рассматривала сценарий «авторизован, корзина не пуста», а сканер и новый посетитель видят сайт анонимом с пустой корзиной. Пересматриваются:
...
- [x] [Review][Defer][P2] Выбранное пользователем сохранение нового адреса молча пропускается, если заказ отправлен до завершения `getAddresses()`: чекбокс уже доступен, но `shouldSaveAddress` остаётся false из-за `addressesLoaded`. Поведение предсуществует changeset и требует отдельного UX-решения для загрузки адресов. [frontend/src/components/checkout/CheckoutForm.tsx:86,267-268] — deferred, pre-existing
- [x] [Review][Defer][P2] Checkout не различает пустую и ещё загружаемую/ошибочную корзину: при прямом входе до завершения `fetchCart()` выводится «Корзина пуста», без loading/error/retry состояния. Поведение предсуществует changeset. [frontend/src/app/(blue)/checkout/CheckoutPageClient.tsx:19-25; frontend/src/components/checkout/CheckoutForm.tsx:300,328-335] — deferred, pre-existing
- [x] [Review][Defer][P3] Подсказка телефона не связывается с полем для скринридера: `ContactSection` передаёт `aria-describedby={undefined}`, а `Input` разворачивает props после собственного `${inputId}-description`, перезаписывая вычисленную связь с helper. Дефект предсуществует смене текста подсказки и относится к общему API `Input`. [frontend/src/components/checkout/ContactSection.tsx:65-68; frontend/src/components/ui/Input/Input.tsx:115-118] — deferred, pre-existing
