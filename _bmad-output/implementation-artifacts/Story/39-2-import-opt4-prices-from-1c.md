...
Прогоны после снятия `else`-fallback (правка вне ACs, см. «Дефект на эскалацию»):

| Прогон | Результат |
|---|---|
| RED: новые тесты до правки | 3 failed (fallback возвращал `retail_price`) |
| GREEN: точечный прогон маппера + guard | 6 passed |
| `pytest -q -m unit` (полный) | **1039 passed**, 1 skipped, 8:20 |
...
Хронология подтверждается данными: старый снимок (2025-11-26) содержал ровно 6 видов цен — список без «Партнера» и «Опт 4». Оба появились после правки этого списка.

Вид цен «Партнер» создан в 1С **2021-07-29** (декодирован timestamp из UUID v1), то есть за 11 дней до «Опт 4»; новым он не является. Утверждение задания (`dev-task-role-from-1c-agreement.md:114`) «на портал не выгружается» было верно ровно до правки списка.

Чинить маппер в этой стори **запрещено** явным указанием Dev Notes: маппинг «Партнера» затрагивает роль `federation_rep` / поле `federation_price` и пересекается с решениями эпика 40 (FR-40-13 явно исключает `federation_rep`).

**Остаточный риск на стороне портала — ИСПРАВЛЕН по прямому указанию Alex (2026-08-02), вне ACs этой стори.** Правка в 1С убирала симптом, но не дыру: `else`-fallback в `_map_price_type_to_field` отправлял **любой** неопознанный вид цен в `retail_price`, поэтому следующая добавленная строка в белом списке 1С молча испортила бы розницу снова.

Сделано:

- `parser.py` — заведён `logger = logging.getLogger("import_products")` (раньше модуль логгера не имел); `else`-ветка `_map_price_type_to_field` возвращает `""` вместо `"retail_price"` и пишет `warning` с именем вида цен и подсказкой, где его убрать в 1С.
- `variant_import.py` — в `update_variant_prices` явная защита: вид цен с пустым `product_field` пропускается (`continue`) с однократным `warning` на тип (дедупликация через новый `self._unmapped_price_types_logged`, по образцу `_missing_variants_logged`). Раньше пустое значение отсекалось неявно, через `hasattr(variant, "")` — поведение верное, но незаметное при чтении.
- Тесты: 3 unit в `test_xml_parser.py` (пустая строка вместо fallback, warning в логе) и 2 integration в `test_variant_import.py` (неопознанный вид цен не затирает `retail_price`; не мешает применить остальные цены того же предложения).
- Существующее утверждение `_map_price_type_to_field("Неизвестный тип") == "retail_price"` в `test_map_price_type_to_field` заменено на `== ""` — оно фиксировало ровно то поведение, которое чинится.

Попутно исправлено: у класса `TestVariantImportProcessor` (`test_variant_import.py:259`) **отсутствовал маркер `@pytest.mark.integration`** — 13 тестов выпадали и из `-m unit`, и из `-m integration`, то есть никогда не запускались в CI-фильтрах (нарушение `project-context.md` §4). Маркер добавлен, все 13 проходят. Без этого два новых регресс-теста тоже не запускались бы.

Ветку «партнер» в маппер **не добавлял** — запрет Dev Notes в силе, целевое поле для «Партнера» по-прежнему за эпиком 40.

**Данные — инцидент локализован и устранён.**

- **Прод не затронут** (подтверждено Alex 2026-08-02): импорт на снимке с «Партнером» там не прогонялся.
- **Dev-БД была затронута**: запись `PriceType(«Партнер») → retail_price` создана, у **всех 31** вариантов `retail_price` равнялся партнёрской цене. Переимпорт их не вылечил бы — у этих предложений в выгрузке нет РРЦ, перезаписывать `retail_price` нечем.
...
Вне ACs стори, по прямому указанию Alex (см. «Дефект на эскалацию» — снятие `else`-fallback):

- `backend/apps/products/services/parser.py` — logger модуля, `""` вместо fallback, warning
