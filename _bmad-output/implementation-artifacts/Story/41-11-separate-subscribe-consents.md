...
#### Code review 2026-09-13 — документальный проход

- [x] [Review][Patch] В `deferred-work.md` к evidence пункта про редиректы `/blog` и `/news` ошибочно приклеен текст соседнего долга про `required` чекбокса ПДн [`_bmad-output/implementation-artifacts/deferred-work.md:1135`] — исправлено

**Rejected:**
- `false` — production-код отсутствует ожидаемо: этот проход намеренно ограничен документами, код и тесты проверены отдельным первым проходом.
- `false` — `status: done` quick-dev-спеки не противоречит `Status: review` story: реализация завершена, story ждёт внешнего code review.
- `false` — `review_loop_iteration: 0` относится к циклу quick-dev-спеки, а описанный в story шаг 4 не меняет этот счётчик.
- `false` — `subscribe-request-contract.test.ts` присутствует в Suggested Review Order спеки; правка Code Map была бы редактурой проверяемой спеки, а не дефектом реализации.
- `false` — ссылки с номерами строк сейчас разрешаются в правильные места и являются принятым форматом этого артефакта.
- `false` — фраза epic-context про старый формат описывает payload на корректном `/subscribe/`; отдельная поправка story описывает поведение реального старого бандла с URL без слэша.
- `false` — отсутствие тоста уже записано отдельным пунктом в `deferred-work.md`.
- `false` — `YYYY-MM-DD` в story явно обозначен как иллюстративный плейсхолдер; утверждённая спека фиксирует метку `2026-09-12`.
- `false` — формат `deferred-work.md` не требует полей владельца и срока; новый пункт соответствует соседним записям.
- `false` — rollback не входит в AC story; миграций нет, а CRITICAL GitNexus для общего файла типов задокументирован как артефакт файловых импортов.
- `false` — сценарий URL без слэша исправлен и не является входной границей контракта согласий в I/O-матрице спеки.
- `false` — Edge Case Hunter не получил production-код ожидаемо из-за документальной границы второго прохода.
- `false` — Acceptance Auditor не получил production-код ожидаемо; код проверен первым проходом.
- `false` — API-контракт отсутствует только во втором diff; он проверен вместе с кодом в первом проходе.
- `false` — тесты отсутствуют только во втором diff; они проверены в первом проходе.
- `false` — правило React Hook Form `required` у ПДн не является нативным HTML-атрибутом `required`; порядок регистрации делает текущий путь рабочим, риск уже отложен отдельно.
- `false` — MSW-хендлер намеренно не менялся по AC10 и уже записан в `deferred-work.md`.
- `false` — замечание Acceptance Auditor о статусах опровергается разными жизненными циклами spec и story.
- `false` — дублирование текстов ошибок уже явно записано в `deferred-work.md`; это не пропущенный долг документального прохода.
