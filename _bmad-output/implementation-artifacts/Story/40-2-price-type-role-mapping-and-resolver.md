...
2. **AC2 (FR-40-03, NFR-3940-06; уточнено решением Alex 2026-08-05).** Миграция идемпотентна: повторный прогон не создаёт дублей и **не перезаписывает** уже непустой `user_role` (поле правится менеджером из админки — слепой `update()` затёр бы его ручную настройку). `reverse` очищает роль у записей, которые соответствуют **актуальному** маппингу `ONEC_EXCHANGE` и имеют ожидаемое значение роли; запись «Опт 4», засеянную вместе с ролью миграцией `0053_seed_price_type_opt4`, `reverse` не трогает. Происхождение значения не хранится, поэтому при откате может быть очищена вручную выставленная, но совпадающая роль; при изменении `ONEC_EXCHANGE` после forward rollback не обязан воспроизводить историческое состояние. Это осознанный lossy-контракт, принятый вместо схемной миграции.
...
- [x] **Task 1: Data-миграция `products` — заполнить `user_role`** (AC: 1, 2)
  ...
  - [x] 1.4: `reverse`: очищать `user_role` у записей, где он равен ожидаемой роли **и** GUID входит в актуальный список миграции, **исключая** GUID «Опт 4» (его роль поставила `0053`). Происхождение роли не отслеживается; обратимость намеренно lossy (решение Alex 2026-08-05).
...
- [x] [Review][Decision] AC2 по-прежнему нарушен: reverse стирает совпавшую ручную роль — **решено 2026-08-05, вариант 2:** AC2 формально уточнён и принимает lossy reverse без provenance.
- [x] [Review][Decision] Reverse миграции зависит от текущих `ONEC_EXCHANGE` — **решено 2026-08-05, вариант 2:** runtime-маппинг остаётся сознательно нерепродуцируемым контрактом rollback.
...
| Дата | Изменение |
|---|---|
| 2026-08-05 | Повторный review: подтверждены 64 целевых теста. Решением Alex формально уточнён AC2: reverse `0054` сознательно lossy, использует актуальный `ONEC_EXCHANGE` и не хранит provenance ручной роли; отдельная схемная миграция не добавляется. Статус: `review` → `done`. |
