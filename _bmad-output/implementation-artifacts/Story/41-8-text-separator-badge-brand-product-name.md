...
**`review_head` требует сдвига после коммита этой итерации.** В отличие от итерации 1.4 (только документация),
здесь изменён исходный файл — `ProductCard.text-separation.test.tsx`. По правилу фронтматтера область приёмки
должна включать этот патч, поэтому при коммите правок `review_head` проставляется на новый коммит.
Сам коммит не выполнялся: push и коммит — только по явной просьбе владельца (Task 9).
