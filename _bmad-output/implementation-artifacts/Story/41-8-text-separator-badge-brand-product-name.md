...
- [x] **Task 10. Выкат** — выполнен 2026-09-09, включая приёмку на проде
  - [x] Мерж PR в `develop`, затем `develop` → `main`; на проде `git fetch origin main; git reset --hard origin/main` (не `git pull`). PR #144 (`feature/…` → `develop`, мерж `b479177c`), PR #145 (`develop` → `main`, мерж `07877d23`); прод переведён с `329dcc40` на `07877d23`
  - [x] `docker compose --env-file .env.prod -f docker/docker-compose.prod.yml up -d --build frontend` — полная пересборка; частичное копирование `.next/` даёт `Failed to find Server Action`
  - [x] После рестарта frontend на проде — `docker compose restart nginx`, иначе nginx держит старый IP upstream и отдаёт 502
  - [x] Проверка на проде: открыть `https://optisport.ru/catalog`, в консоли снять `document.querySelector('[role="article"]').textContent` → ожидается `Made In Russia Балетки для танцев белые350 ₽…` с пробелом между брендом и названием; эталон «до» с того же URL — склейка `Made In RussiaБалетки…` (замер 2026-09-08). **Копированием выделения не проверять** — оно разделяет слова и без правки, результат ложноотрицательный. Карточки рендерятся на клиенте, поэтому `curl` разметку с бейджами не покажет — проверка только в браузере
    - **Выполнена владельцем 2026-09-09** (в сессии выката браузерных инструментов не было, проверка отдана ему). Снятое значение: `Made In Russia Балетки для танцев белые350 ₽В корзину` — совпадает с ожидаемым эталоном «после» посимвольно, пробел между брендом и названием есть. Эталон «до» с того же URL — `Made In RussiaБалетки…` (замер 2026-09-08)
  - [x] ISR к карточкам товаров отношения не имеет (данные грузятся клиентом), ручная ревалидация не требуется — подтверждено на живом проде: в HTML `https://optisport.ru/catalog` (30 260 байт) нет ни одного `role="article"`, названий товаров и разделителей
...
### Review Findings (2026-09-09, шестое ревью Codex)

- [x] [Review][Patch] Подтвердить AC2 сопоставимым браузерным замером: таблица в Dev Agent Record заявляет попиксельное совпадение, но для `/home` сравнивает разные карточки (`brandTop`: prod `192`, local `—`) и разные товары. Повторить замер на одинаковых данных или контролируемой фикстуре, зафиксировать геометрию карточки с брендом и бейджем до/после и только затем заявлять отсутствие визуальной регрессии. [_bmad-output/implementation-artifacts/Story/41-8-text-separator-badge-brand-product-name.md:444-469]
...
**AC2 — закрыт замером. Доказательная редакция — итерация 1.8** (раздел «Пункт шестого ревью» ниже);
таблица ниже — первый, несопоставимый заход, оставлена как след. Браузерные MCP-инструменты в сессии
оказались недоступны, поэтому осмотр выполнен через Playwright (`@playwright/test@1.57.0`, chromium,
headless): снята геометрия карточек на проде (`https://optisport.ru` — **старый код**) и локально
(`http://localhost` — **новый код**) на тех же страницах и ширинах.

> ⚠️ **Этой таблицей AC2 не подтверждается — находка шестого ревью (Codex, 2026-09-09) верна.**
> Прод и локальная среда содержат разные товары, поэтому строки `/home` сравнивают **разные карточки**:
> у прод-карточки бренд есть (`brandTop` 192), у локальной его нет вовсе (`—`). Совпадение чисел при
> разном составе карточки доказательством отсутствия визуальной регрессии не является. Таблица
> сохранена как след замера; AC2 подтверждается замером итерации 1.8 — A/B на одном стенде, одной БД
> и одном товаре, где единственное различие между прогонами — код.
...
**Пункт шестого ревью (Codex, 2026-09-09) — закрыт (1 из 1).**

✅ *Resolved review finding [Patch]: подтвердить AC2 сопоставимым браузерным замером.* Находка верна.
Прежний замер сравнивал **прод против локального стенда**, а составы каталогов у сред разные: на `/home`
прод-карточка имела бренд (`brandTop` 192), локальная — не имела (`—`), то есть сравнивались разные
карточки разных товаров. Совпадение размеров при разном составе карточки регрессию не исключает: оно
означает лишь, что две несопоставимые карточки одинаковой ширины. Замер выполнен заново по схеме,
которую находка и предписывает — **контролируемое A/B на одном стенде**.

**Метод.** Один и тот же локальный стенд, одна и та же БД, один и тот же товар, один и тот же URL;
единственное различие между прогонами — код:

1. Прогон `before` — четыре файла фронта откачены на `baseline_commit`
   (`git checkout e3e05a46 -- ProductBadge.tsx index.ts ProductCard.tsx ElectricProductCard.tsx`).
   Что откат даёт ровно «код до правки», проверено диффом: `git diff e3e05a46 HEAD` по этим файлам
   состоит **только** из обёрток во фрагмент и вставок `<TextSeparator />` — ни одного изменения классов,
   разметки или логики.
2. Прогон `after` — те же файлы возвращены (`git checkout HEAD -- …`).
3. Прогон `after2` — повтор `after` **без единого изменения кода**: нужен как замер собственного шума
   метода (см. попиксельное сравнение ниже).

Целевая карточка в каждом прогоне выбиралась как первая карточка страницы, у которой есть **и бейдж,
и бренд**; её название запоминалось и в последующих прогонах карточка искалась по этому названию —
поэтому во всех прогонах сравнивается один и тот же товар, а не «первая попавшаяся карточка».
Снималось: размер карточки, координаты бренда, заголовка и контейнера описания **относительно карточки**,
геометрический отпечаток **всех** её потомков (12–19 узлов на комбинацию, сами разделители из отпечатка
исключены), `document.body.scrollWidth`/`clientWidth`, `textContent`, computed style разделителей
и PNG-скриншот карточки.

**Грабли, из-за которых первый прогон `before` оказался ложным** (записаны, чтобы следующий замер
не наступил на них снова): в `docker-compose.yml` фронт смонтирован bind-mount'ом с хоста, а Docker
Desktop на Windows не пробрасывает в контейнер file-watch события — Next не заметил отката файлов
и продолжал отдавать сборку **с разделителями** (`span.sr-only` = 2 при откаченном коде). Ложный прогон
пойман контрольной проверкой числа разделителей и выброшен. Рабочая последовательность: правка файлов →
`docker compose restart frontend` → дождаться `Ready in` в логах → `docker compose restart nginx`
(без него nginx держит старый IP апстрима и отдаёт 502/504) → прогон. Оба зачётных прогона сняты
по этой процедуре, симметрично.

**Геометрия — совпадает во всех восьми комбинациях.** Числа ниже одинаковы в `before` и `after`
(колонки не разделены именно поэтому: расхождений нет ни в одном столбце). `y` — координата
относительно верхнего края карточки.

| Комбинация | Товар (один и тот же в обоих прогонах) | Карточка `W×H` | бренд `y` | `h3` `y` | узлов сверено | `scrollWidth`/`clientWidth` | `span.sr-only` до→после |
|---|---|---|---|---|---|---|---|
| `/home` 375 | Пояс тяжелоатлетический BoyBo «Fire» (BoyBo) | 180×296 | 192 | 212 | 12 | 377 / 375 | 0 → 2 |
| `/home` 1440 | тот же | 180×296 | 192 | 212 | 12 | 1440 / 1440 | 0 → 2 |
| `/catalog?is_hit=true` grid 375 | Гантель неопреновая Espado ES1115 (Espado) | 343×539 | 359 | 383 | 13 | 377 / 375 | 0 → 2 |
| `/catalog?is_hit=true` grid 1440 | тот же | 285.33×481.33 | 301.33 | 325.33 | 13 | 1440 / 1440 | 0 → 2 |
| `/catalog?is_hit=true` list 375 | тот же | 457.36×156 | 16 | 40 | 16 | 473 / 375 | 0 → 2 |
| `/catalog?is_hit=true` list 1440 | тот же | 904×156 | 16 | 40 | 16 | 1440 / 1440 | 0 → 2 |
| `/electric` 375 | Пояс тяжелоатлетический BoyBo «Fire» (BoyBo) | 220×402 | 215 | 231 | 19 | 390 / 375 | 0 → 2 |
| `/electric` 1440 | тот же | 220×396 | 219 | 235 | 19 | 1440 / 1440 | 0 → 2 |

Сверялись не три числа, а **весь** отпечаток карточки: 12–19 узлов × (тег, класс, x, y, ширина, высота),
всего 120 узлов по восьми комбинациям — расхождений 0. Переполнение на 375 px (377, 390 и 473 против 375)
присутствует в обоих прогонах одинаково: оно пре-существующее, к разделителям отношения не имеет
(та же картина, что и в прежних замерах, записана в `deferred-work.md`).

**Попиксельное сравнение скриншотов — и замер собственного шума метода.** Скриншот карточки снимался
в каждом прогоне; сравнение сделано попиксельно (декодирование PNG в canvas, сравнение RGBA):

| Пара прогонов | Код | Комбинаций с расхождением | Худшая комбинация |
|---|---|---|---|
| `before` vs `after2` | **разный** | **0 из 8** | — |
| `before` vs `after` | **разный** | 1 из 8 | `/electric` 375: 11 px из 88 440 (0.012%), Δ ≤ 15 |
| `after` vs `after2` | **одинаковый** | 2 из 8 | `/catalog` list 1440: 4 294 px из 141 024 (3.04%), Δ ≤ 4 |

Ключ к чтению таблицы — третья строка: **два прогона одного и того же кода расходятся сильнее, чем
прогоны разного кода.** То есть ненулевой пиксельный дифф здесь — шум рендеринга (сглаживание, момент
декодирования картинки), а не эффект правки. Прямое подтверждение: те же 11 пикселей `/electric` 375
воспроизводятся и при сравнении `after` с `after2`, где код идентичен, а пара `before` vs `after2` даёт
0 расхождений во всех восьми комбинациях. Сами 11 пикселей лежат на скошенной границе кнопки
«В КОРЗИНУ» (x ≈ 24, y ≈ 365–368) — вне зоны бейджа, бренда и названия, куда вставлены разделители.

**Разделители на живой странице** (прогон `after`, все восемь комбинаций): по два на карточку,
`class="sr-only left-0 top-0"`, `textContent` = `' '`, computed style `position: absolute`, `left: 0px`,
`top: 0px`, размер 1×1. Предшествующий элемент — ровно тот, чей текст разделяется: `<SPAN>Хит</SPAN>`
и `<P>BoyBo</P>` в `ProductCard`, `<DIV>` бейджа и `<P>` бренда в `ElectricProductCard`. В list-layout
порядок обратный (`<P>` бренда, затем `<SPAN>` бейджа) — как и требует AC1.

**`textContent` того же товара, до → после** (склейка исчезает ровно на заявленных стыках):

| Комбинация | `before` | `after` |
|---|---|---|
| `/home` | `ХитBoyBoПояс тяжелоатлетический…` | `Хит BoyBo Пояс тяжелоатлетический…` |
| `/catalog` grid | `ХитEspadoГантель неопреновая…` | `Хит Espado Гантель неопреновая…` |
| `/catalog` list | `EspadoХитГантель неопреновая…` | `Espado Хит Гантель неопреновая…` |
| `/electric` | `Хит♡BoyBoПояс тяжелоатлетический…` | `Хит ♡BoyBo Пояс тяжелоатлетический…` |

Остаток `♡BoyBo` на `/electric` — известная отложенная находка повторного ревью (между разделителем
бейджа и брендом стоит кнопка избранного), не регрессия 41.8.

**Данные стенда.** Флаги в локальной БД в этой итерации **не** проставлялись и не снимались — товары
с флагами в ней уже были (`is_hit` 7, `is_new` 8, `is_sale` 10, `is_promo` 8 при 9236 товарах), и
карточка с брендом и бейджем находилась на всех трёх маршрутах. БД за время замера не изменялась,
прод не затрагивался. Скрипты замера (`measure.mjs`, `compare.mjs`, `pixeldiff.mjs`), три JSON-набора
метрик и 24 скриншота лежат в рабочем каталоге сессии; в репозиторий не добавлялись — тестами
раскладка не проверяется по построению (`css: false`).

**Проверки после закрытия шестого ревью.** Исполняемый код не менялся ни на символ: временный откат
четырёх файлов на baseline выполнялся только на время прогона `before` и возвращён
(`git status` по `frontend/` чист, `TextSeparator` на месте во всех четырёх точках вставки).
Правки итерации — только в стори и `sprint-status.yaml`. Регрессионный прогон `npm run test` →
**166 файлов, 2797 passed | 16 skipped (2813)** — ровно базис прошлой итерации, дельта **0**.
`npx tsc --noEmit` — exit 0; `npm run lint` (`eslint . --max-warnings=0`) — без вывода;
`npm run format:check` — `All matched files use Prettier code style!`.
`npx gitnexus detect-changes --scope all --repo "C:\Users\1\DEV\FREESPORT"` → **`No changes detected`**:
символов продакшн-кода не затронуто, что и ожидалось. GitNexus `impact` в этой итерации не требовался —
ни один символ не редактировался (замер, документация); последняя картина по `TextSeparator` —
CRITICAL, 16 символов, 3 прямых вызывающих, сообщена владельцу в итерации 1.7. Бэкенд не трогался,
pytest и `generate:types` не запускались (AC6). Локальный стенд после замера оставлен в рабочем
состоянии: `frontend` и `nginx` перезапущены, `http://localhost/home` отвечает 200 на коде стори.

**`review_head` не сдвигается — остаётся `0830b703`.** Правило фронтматтера: поле двигает только
итерация, трогающая исходные файлы. Здесь исходных файлов нет ни одного — итерация состоит из замера
и записи о нём, правки только в стори и `sprint-status.yaml`, и это подтверждено независимо:
`detect-changes --scope all` → `No changes detected`. Область приёмки прежняя —
`e3e05a46..0830b703` минус `4a2379ed`. Task 10 не выполнялся: на прод не выехало ничего, `main` правок
не получал; коммит и push этой итерации — по явной просьбе владельца (требование Task 9).

> **Устарело с 2026-09-09:** абзац выше описывает состояние на момент итерации 1.8. Task 10 выполнен —
> см. раздел «Выкат на прод» ниже. `review_head` он не сдвигает: выкат не менял ни одного файла.

### Выкат на прод (Task 10, 2026-09-09)

**Что выехало.** Ветка `feature/story-41-8-text-separator-product-card` на момент выката не была
запушена вовсе — Task 10 начался с `git push -u origin`, а не с мержа готового PR.

| Шаг | Результат |
|---|---|
| PR #144 `feature/…` → `develop` | Все 5 обязательных контекстов зелёные (`Бэкенд: тесты` 3m25s, `Фронтенд: тесты` 2m59s, `build (3.12)` 12m54s, `Контракт синхронен с кодом` 1m10s, `Проверки качества кода` 30s); сверх них `E2E Tests` и `claude-review` тоже зелёные. Мерж-коммит `b479177c` |
| PR #145 `develop` → `main` | 5 из 5 зелёные (`build (3.12)` 12m04s). Мерж-коммит `07877d23` |
| Область релиза | `git diff --name-status origin/main..origin/develop` — 15 файлов: 9 файлов стори 41.8 плюс документационный хвост стори 41.7 (`41-7-…md`, `docs/legal/login-form-consent-justification.md`), оставшийся в `develop` с прошлого релиза, и автоблок GitNexus в `AGENTS.md`/`CLAUDE.md`. Ни одной миграции, ни одного файла бэкенда |
| Прод: код | `git fetch origin main; git reset --hard origin/main` — `329dcc40` → `07877d23`. `git remote -v` проверен до сброса: `origin` → приватный `FREESPORT.git` |
| Прод: сборка | `up -d --build frontend` — образ `freesport-frontend:local` пересобран (npm-стадия 135.6s), контейнер `Recreated` → `Started`, healthcheck прошёл |
| Прод: миграции | `showmigrations \| grep "^ \[ \]"` → пусто. Проверено по требованию навыка `production-update`, хотя диффа бэкенда в релизе нет |
| Прод: nginx | `restart nginx` выполнен — без него держится старый IP upstream и весь сайт отдаёт 502 |
| Прод: контейнеры | `backend`, `celery-beat`, `celery-worker` — `Up 2 days (healthy)`; `frontend` — `Up 15 seconds (healthy)`; `nginx` — перезапущен |

**Проверка доставки кода.** Финальный gate Task 10 — снятие `element.textContent` в консоли браузера —
**не выполнен**: браузерные инструменты в сессии выката были недоступны. Вместо него снята проверка
другого класса — что собранный бандл с разделителем реально отдаётся клиенту:

```
https://optisport.ru/home      -> 200
https://optisport.ru/catalog   -> 200
https://optisport.ru/electric  -> 200

# в контейнере frontend на проде
grep -rl "sr-only left-0 top-0" /app/.next/static/chunks
  /app/.next/static/chunks/8682-c394fc98484dada3.js
  /app/.next/static/chunks/1346-447eb6072005d65d.js
  /app/.next/static/chunks/8570-fc49e1572e24f3df.js

# те же три чанка снаружи, через nginx: HTTP 200, по одному вхождению в каждом
curl https://optisport.ru/_next/static/chunks/8682-c394fc98484dada3.js
  ...let l=()=>(0,a.jsx)("span",{className:"sr-only left-0 top-0",children:" "});l.displayName="TextSeparator"
```

Публично отдаваемый чанк содержит `TextSeparator` с классом `sr-only left-0 top-0` и пробелом-ребёнком —
то есть на прод выехал именно код стори, включая координаты из фикса `c7017a7f`. Это доказывает
**доставку**, но не **результат исполнения**: `textContent` живой карточки этой проверкой не снимается.

**Почему `curl` дальше не идёт.** HTML `https://optisport.ru/catalog` — 30 260 байт, в нём `role="article"`
встречается 0 раз, названий товаров нет. Карточки рендерятся на клиенте, что заодно закрывает последний
подпункт Task 10: ISR к ним отношения не имеет, ручная ревалидация не нужна.

**Приёмка на проде (владелец, 2026-09-09).** Проверка снята владельцем в браузере на
`https://optisport.ru/catalog`:
...
Совпадает с ожидаемым эталоном «после» посимвольно; эталон «до» с того же URL и того же товара —
`Made In RussiaБалетки для танцев белые…` (замер 2026-09-08). **Стык «бренд → название» на живом проде
разделён — AC1 подтверждён на реальных данных.**

Что этот замер **не** покрывает, и почему это не пробел в приёмке:

- **Стык «бейдж → бренд»** — у первой карточки `/catalog` бейджа нет, поэтому в строке его и не видно.
  Этот стык закрыт тестами на реальном `ProductBadge` (18 + 9 кейсов, покрытие подтверждено шестью
  мутациями исходников) и контролируемым A/B-замером AC2, где `span.sr-only` 0 → 2 на карточке с бейджем.
- **`350 ₽В корзину`** — склейка «цена + кнопка» осталась. Это не регрессия и не недоделка: она за
  границей FR-41-19 (AC6) и вынесена в `deferred-work.md` вместе со склейкой «название + цена» ещё
  в итерации 1.0. Замер лишний раз показывает, что дефект того же класса на проде существует.
...
| `_bmad-output/implementation-artifacts/sprint-status.yaml` | Статус стори: `ready-for-dev` → `in-progress` → `review` (и обратно на каждой итерации ревью). Пятое ревью: шапочный комментарий переписан под фактический эффект (`element.textContent`, улучшение чтения диктором не заявляется), запись о сохранении screenreader-цели помечена как пересмотренная, добавлена пропущенная запись о четвёртом ревью. Шестое ревью: добавлена запись об итоге замера AC2 (контролируемое A/B, геометрия и пиксели), `in-progress` → `review` |
| `_bmad-output/implementation-artifacts/Story/41-8-text-separator-badge-brand-product-name.md` | Чекбоксы, Dev Agent Record, File List, Change Log, Status, `review_head`. Третье ревью: дополнительно синхронизирован блок Dev Notes «Реализация». Четвёртое ревью: переписаны роль и outcome в `## Story` и обоснование AC2 — оба места названы самими action item'ами (`Story:24-40`, `Story:42-48`), иначе пункты не закрываются. Пятое ревью: сужены формулировки о произношении во врезке роли, абзаце ценности и двух местах Dev Agent Record; синхронизированы Task 6, Task 7 и «Что сделано». Шестое ревью: прежняя таблица prod-vs-local помечена как несопоставимая и оставлена следом, добавлен раздел с контролируемым A/B-замером AC2 (метод, геометрия 8 комбинаций, попиксельное сравнение с замером собственного шума, грабли bind-mount) |
...
|---|---|---|---|
| 2026-09-09 | 2.0 | **Приёмка на проде — стори закрыта.** Владелец снял оставшийся gate Task 10 в браузере на `https://optisport.ru/catalog`: `document.querySelector('[role="article"]').textContent` вернул `Made In Russia Балетки для танцев белые350 ₽В корзину` — посимвольное совпадение с эталоном «после», пробел между брендом и названием есть; эталон «до» с того же URL и товара — `Made In RussiaБалетки…`. Стык «бейдж → бренд» этой карточкой не покрыт (у неё нет бейджа) и остаётся подтверждён тестами на реальном `ProductBadge` и A/B-замером AC2, где `span.sr-only` 0 → 2. Склейка `350 ₽В корзину` в выводе — известный отложенный дефект AC6, не регрессия. Все 10 задач и все AC закрыты, Status → done. Кода итерация не меняет, `review_head` остаётся `0830b703` | Amelia (Dev Agent) |
| 2026-09-09 | 1.9 | **Task 10 (выкат) выполнен** — по явной просьбе владельца. Ветка на момент старта задачи не была запушена, поэтому выкат начался с `git push -u origin`, а не с мержа готового PR. PR #144 (`feature/…` → `develop`) и PR #145 (`develop` → `main`) прошли все 5 обязательных контекстов и смержены (`b479177c`, `07877d23`); в релиз попал ещё документационный хвост стори 41.7, оставшийся в `develop` с прошлого раза — миграций и файлов бэкенда в диффе нет. Прод переведён с `329dcc40` на `07877d23` через `fetch` + `reset --hard`, `frontend` пересобран целиком, `showmigrations` неприменённых не показал, `nginx` перезапущен. Три страницы отвечают 200. Финальный gate — снятие `element.textContent` в консоли браузера — **не выполнен**: браузерные инструменты в сессии были недоступны; вместо него снята проверка доставки кода — три публично отдаваемых чанка содержат `TextSeparator` с классом `sr-only left-0 top-0` и пробелом-ребёнком. Это подтверждает, что код на проде, но не результат его исполнения, поэтому подпункт остаётся `[ ]`, а Status — `review` до прогона владельцем. Заодно подтверждено, что ISR не при делах: в HTML `/catalog` нет ни одного `role="article"`. Ни одного файла кода итерация не меняет, `review_head` остаётся `0830b703` | Amelia (Dev Agent) |
| 2026-09-09 | 1.8 | Закрыта находка шестого ревью (Codex) — 1 из 1: AC2 подтверждён сопоставимым замером. Находка верна — прежняя таблица сравнивала прод с локальным стендом, а составы каталогов у сред разные, поэтому на `/home` сопоставлялись разные карточки (у прод-карточки бренд есть, `brandTop` 192; у локальной его нет, `—`). Замер выполнен заново как контролируемое A/B: один стенд, одна БД, один и тот же товар, единственное различие между прогонами — код (откат четырёх файлов фронта на `baseline_commit` и возврат; дифф между состояниями состоит только из фрагментов и вставок `<TextSeparator />`). Восемь комбинаций (`/home`, `/catalog` grid, `/catalog` list, `/electric` × 375/1440): совпали размер карточки, координаты бренда, заголовка и контейнера описания, `scrollWidth`/`clientWidth` и полный геометрический отпечаток — 120 узлов, расхождений 0; `span.sr-only` 0 → 2, `textContent` из склеенного стал разделённым на том же товаре. Скриншоты сверены попиксельно, и отдельно снят собственный шум метода: два прогона **одного и того же** кода расходятся сильнее (до 4 294 px, 3.04%), чем прогоны разного кода (`before` vs `after2` — 0 расхождений во всех восьми), поэтому единственный ненулевой дифф (11 px на скошенной границе кнопки «В КОРЗИНУ») отнесён к шуму рендеринга, а не к правке. Прежняя таблица оставлена следом с пометкой о несопоставимости. Попутно записаны грабли замера: Docker bind-mount на Windows не пробрасывает file-watch, из-за чего первый прогон «до» шёл на сборке с разделителями и был выброшен — нужен `restart frontend` + `restart nginx`. Исполняемый код не менялся: 166 файлов, 2797 passed \| 16 skipped — дельта 0, `tsc`/`lint`/`format:check` чисты, `detect-changes --scope all` → `No changes detected` | Amelia (Dev Agent) |
