...
- [ ] **Task 10. Выкат** — выполнен 2026-09-09, кроме финальной браузерной проверки (см. последний подпункт)
  ...
  - [ ] Проверка на проде: открыть `https://optisport.ru/catalog`, в консоли снять `document.querySelector('[role="article"]').textContent` → ожидается `Made In Russia Балетки для танцев белые350 ₽…` с пробелом между брендом и названием; эталон «до» с того же URL — склейка `Made In RussiaБалетки…` (замер 2026-09-08). **Копированием выделения не проверять** — оно разделяет слова и без правки, результат ложноотрицательный. Карточки рендерятся на клиенте, поэтому `curl` разметку с бейджами не покажет — проверка только в браузере
    - **Не выполнена: в сессии выката браузерные инструменты были недоступны.** Вместо неё снята более слабая, но объективная проверка — что новый код действительно отдаётся клиенту (см. «Выкат на прод» в Dev Agent Record). Это подтверждает доставку кода, но не результат его исполнения в браузере, поэтому подпункт остаётся `[ ]` до прогона владельцем
...
**Остаток по Task 10 — один пункт для владельца.** Открыть `https://optisport.ru/catalog` и снять
в консоли:
...
Ожидается пробел между брендом и названием (`Made In Russia Балетки…`); эталон «до» с того же URL —
`Made In RussiaБалетки…` (замер 2026-09-08). Копированием выделения не проверять — ложноотрицательно.
До этого прогона стори остаётся в `review`: приёмка на проде не подтверждена.
