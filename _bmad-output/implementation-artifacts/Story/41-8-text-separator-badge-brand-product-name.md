...
review_head: "100ce0c1" # 2026-09-08, закрытие четвёртого ревью: тест разделителей ElectricProductCard (eaeafb2e) + синхронизация роли стори, deferred-work и docstring с замерами (100ce0c1). Сдвинут с f985c849 по решению владельца — итерация добавляет исходные файлы (новый тест и docstring), они обязаны попасть в область приёмки
...
**Состояние ветки и `review_head`.** Правки этой итерации **не закоммичены** — коммит и push только по
явной просьбе владельца (Task 9). В рабочем дереве четыре файла: `TextSeparator.tsx` (docstring),
`deferred-work.md`, `sprint-status.yaml` и сама стори. HEAD ветки — `100ce0c1`, на нём же стоит
`review_head`.

⚠️ **Требуется решение владельца при коммите.** Итерация трогает **исходный файл** (`TextSeparator.tsx`),
а не только метаданные, — то есть по прецеденту итерации 1.6 патч обязан попасть в область приёмки, и
`review_head` подлежит сдвигу. Рекомендуемая схема — два коммита, ровно как описано во фронтматтере:
`docs(story-41-8): сузить утверждения о произношении до наблюдаемого` (docstring + `deferred-work.md` +
`sprint-status.yaml`), затем завершающий документационный коммит со стори, где `review_head` = хеш
первого. Пока это не сделано, версия стори на HEAD указывает `review_head: 100ce0c1` и правку docstring
в область приёмки не включает. Task 10 не выполнялся: на прод не выехало ничего, `main` правок не получал.
