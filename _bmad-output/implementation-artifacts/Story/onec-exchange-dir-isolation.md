...
**Выкат и контрольная выгрузка — прод, 28.08.2026, 16:00-16:34 UTC.**

Выкат выполнил Alex (коммиты `8935f4ca`, `10cc1544` в `main`, прод переведён на
`10cc1544`). Проверено не по факту рестарта, а по коду внутри контейнеров:
`_prune_imported_exchange_images` и `consumed_image_sources` присутствуют в
`backend`, `celery` и `celery-beat`. Внешний API через nginx — 200, 502-грабли с
upstream не сработали. `cleanup-stale-exchange-dirs` в эффективном расписании beat,
каждые 6 ч.

**Приёмка (110 сессий, `created_at > 16:00 UTC`):**

| Метрика | До выката (24 ч) | После выката |
|---|---|---|
| Всего сессий | 614 | 110 |
| `completed` | 605 | **110** |
| `failed` | 9 | **0** |
| «не найден в каталоге обмена» | 9 | **0** |
| Упало после ожидания лока | 9 | **0** |
| Ждали лока и дождались | — | **48** |
| `goods`-сегментов, все `completed` | — | 22 / 22 |

Решающая строка — «ждали лока: 48». До правки ожидание лока означало падение почти
достоверно (9 из 9 за сутки, 85 из 85 за неделю). После — 48 сессий отстояли
очередь и все дошли: очередь сохранена (AC6), падения после неё устранены.

**AC1/AC3 на живых данных.** В логе воркера каждый прогон пишет
`Удалено XML файлов: 1` — сессия читает и убирает ровно свой сегмент. Каталоги
сессий наблюдались строго 1:1 с числом живых сессий (`dirs == in_progress` на всех
замерах) и уходили в 0 сразу по завершении очереди — AC5 без накопления.

**AC2 на живых данных.** `Image not found` за всю выгрузку — **0**. XML из
сессионных каталогов разрешили все картинки в общем `import_files`. `media/products/base`
не изменился (3,6 ГБ / 15 438 файлов): каталог переимпортировался, копии уже были в
хранилище, и ветка `default_storage.exists()` пометила исходники потреблёнными без
дублирования.

**Tech-debt п. 27 на живых данных.** В логе — `Удалено перенесённых изображений:
295…356` каждые ~45 с. Пик общего каталога в ходе выгрузки — 2046 файлов / 148 МБ,
после разбора сегмента падал до 1727 / 45 МБ. Остаток по завершении — **4720 файлов
/ 121 МБ, из них >100 КБ ноль**, то есть исключительно превью ниже порога. Без правки
там осело бы 5–8 ГБ. Свободное место на диске не изменилось: 20 ГБ из 79.

⚠️ **AC8 закрыт не полностью:** контракт требует окна в сутки. Замер выше снят за
34 минуты после выката и покрывает контрольную выгрузку (п. 4-5
`post-deploy-verification.md`) целиком, но суточное окно закрывается 29.08.2026
около 16:00 UTC. Статус `done` ставится по нему.

**Замер под tech-debt п. 27 (точка отсчёта для решения «через неделю»):**
4720 файлов / 121 МБ превью после полной выгрузки. Класс подтверждён замером —
ни одного файла крупнее порога в остатке нет.
...
**Регресс раунда 3:** `test_exchange_dir_isolation.py` + `test_import_cleanup_race.py`
— 101 passed. Полный backend-набор — **3218 passed, 75 skipped, 0 failed,
19 subtests passed** (было 3214 + 4 новых теста). Black и Flake8 чисто
(`flake8 apps/products apps/integrations`). Mypy — те же 9 сообщений вне
изменённых диапазонов, что и раньше.
...
| 2026-08-28 | Закрыт tech-debt п. 27: уборка общего `import_files` переведена на ссылочный критерий без возраста — команда удаляет исходники, копии которых подтверждены в хранилище (`consumed_image_sources` → `_cleanup_consumed_images`, строго в границах `IMPORT_DIR`), периодическая задача страхует упавшие прогоны (`_prune_imported_exchange_images`). Превью ниже порога оставлены под наблюдение по решению Alex. Регресс 3218 passed, 75 skipped, 0 failed. |
