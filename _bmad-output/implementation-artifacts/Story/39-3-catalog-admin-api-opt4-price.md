...
- [x] [Review][Decision] В API-контракт попали посторонние структурные изменения — `docs/api/openapi.yaml` содержит не только `opt4_price` в `ProductList`/`ProductDetail`, но также перестановки операций `/orders/{id}/` и `/cart/items/`, а также изменения вокруг bonus/country-схем. AC10 / Task 8.3 требуют отдельно оценить такой структурный дрейф и при нём остановиться для решения Alex; решение Alex: оставить полную перегенерацию OpenAPI в Story 39.3.
