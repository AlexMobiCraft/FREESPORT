...
### Review Findings (2026-05-04 CR-5)

- [x] [Review][Patch] Ветка repair с fallback внутри placeholder-поддерева деактивирует категории, но не переносит товары в fallback [`backend/apps/products/management/commands/fix_category_tree_public_roots.py:100`]
- [x] [Review][Patch] Merge repair-anchor ищет произвольный root `СПОРТ` через `.first()` до фильтрации sentinel/blank `onec_id`, поэтому может не слить repair-якорь с реальным импортом [`backend/apps/products/services/variant_import.py:1714`]
- [x] [Review][Patch] `/categories-tree/` при нескольких активных root `СПОРТ` только логирует warning и всё равно отдаёт детей произвольного якоря [`backend/apps/products/views.py:301`]
- [x] [Review][Patch] DRY-RUN repair-аудит перечисляет категории, но не показывает конкретные товары из placeholder-веток, хотя Task 4.2 требует аудит товаров в них [`backend/apps/products/management/commands/fix_category_tree_public_roots.py:72`]
- [x] [Review][Scope] Diff текущей category-tree story всё ещё смешан с Epic 34/VAT/order-test изменениями, несмотря на ограничение story не смешивать эти scope [`backend/tests/helpers.py:70`, `backend/tests/integration/test_order_exchange_import_e2e.py:318`, `_bmad-output/implementation-artifacts/Story/34-5-tests-update-epics-4-5.md:1`] — acknowledged: изменения уже зафиксированы в коммите `f3cd783d` (тем же commit-ом завершена Story 34.5: status → done). Переписывание истории не выполняется. Для будущих story принят чек-лист «один scope = один коммит».
...
| 2026-05-04 | 1.7 | Закрыты 5 замечаний CR-5: cycle-guard переносит товары в fallback и отвязывает fallback от placeholder-родителя; merge repair-anchor использует sentinel/blank onec_id фильтр на уровне БД до .first(); /categories-tree/ возвращает union детей всех активных якорей СПОРТ; DRY-RUN перечисляет конкретные товары; scope-finding по mixed commit зафиксирован как acknowledged (история не переписывается). Добавлены 4 регрессионных теста. | claude-opus-4-7 |
...
- CR-5 финальный прогон: `pytest apps/products -m unit -q` → 101 passed; `pytest apps/products -m integration -q` → 61 passed. Регрессий нет.
- CR-5 промежуточный fail: первая итерация DRY-RUN listing товаров использовала `Product.values_list("pk", "sku", ...)`, но `sku` есть только у `ProductVariant` — заменено на `slug`.
...
**CR-5 Follow-ups (2026-05-04):**
- ✅ Resolved review finding [Patch] (CR-5 #1): `fix_category_tree_public_roots.py` cycle-guard теперь сначала переносит товары из placeholder-поддерева в fallback (`exclude(category_id=fallback.pk)`), затем деактивирует все категории кроме fallback, и отвязывает fallback от placeholder-родителя (`fallback.parent = None`). Тест `test_fix_category_tree_cycle_guard_moves_products_to_fallback`.
- ✅ Resolved review finding [Patch] (CR-5 #2): `process_categories()` фильтрует `onec_id__isnull=True | onec_id="" | onec_id=REPAIR_ANCHOR_ONEC_ID` ORM-запросом до `.first()` и сортирует по `pk`, исключая выбор stale-дубля root СПОРТ. Тест `test_process_categories_picks_repair_anchor_when_stale_duplicate_root_exists`.
- ✅ Resolved review finding [Patch] (CR-5 #3): `CategoryTreeViewSet.get_queryset()` использует `parent__in=anchor_qs` + `.distinct()` вместо `.first()`-якоря — при нескольких активных СПОРТ возвращается union детей. Warning остаётся как сигнал оператору. Тест `test_public_tree_returns_union_when_multiple_active_anchors`.
- ✅ Resolved review finding [Patch] (CR-5 #4): DRY-RUN репорт repair-команды теперь перечисляет до 50 первых товаров каждой placeholder-ветки (`[product] id=… sku=… name=…`) и счётчик «и ещё N товаров». Тест `test_fix_category_tree_dry_run_lists_products_in_placeholder_branches`.
- ✅ Resolved review finding [Scope] (CR-5 #5): scope-смешение в commit `f3cd783d` (helpers.py + test_order_exchange_import_e2e.py относятся к Story 34.5, которая тем же commit-ом помечена как `done`) acknowledged без переписывания git-истории.
...
- `backend/apps/products/management/commands/fix_category_tree_public_roots.py`
- `backend/apps/products/tests/unit/test_variant_import_migrated.py` (CR-5: + `test_process_categories_picks_repair_anchor_when_stale_duplicate_root_exists`)
- `backend/apps/products/tests/unit/test_fix_category_tree_public_roots.py` (CR-5: + `test_fix_category_tree_cycle_guard_moves_products_to_fallback`, `test_fix_category_tree_dry_run_lists_products_in_placeholder_branches`)
- `backend/apps/products/tests/test_visible_categories.py` (CR-5: + `test_public_tree_returns_union_when_multiple_active_anchors`)
- `frontend/src/services/categoriesService.ts`
