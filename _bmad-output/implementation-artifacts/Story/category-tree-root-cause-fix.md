...
- [ ] [Review][Patch] Ветка repair с fallback внутри placeholder-поддерева деактивирует категории, но не переносит товары в fallback [`backend/apps/products/management/commands/fix_category_tree_public_roots.py:100`]
- [ ] [Review][Patch] Merge repair-anchor ищет произвольный root `СПОРТ` через `.first()` до фильтрации sentinel/blank `onec_id`, поэтому может не слить repair-якорь с реальным импортом [`backend/apps/products/services/variant_import.py:1714`]
- [ ] [Review][Patch] `/categories-tree/` при нескольких активных root `СПОРТ` только логирует warning и всё равно отдаёт детей произвольного якоря [`backend/apps/products/views.py:301`]
- [ ] [Review][Patch] DRY-RUN repair-аудит перечисляет категории, но не показывает конкретные товары из placeholder-веток, хотя Task 4.2 требует аудит товаров в них [`backend/apps/products/management/commands/fix_category_tree_public_roots.py:72`]
- [ ] [Review][Patch] Diff текущей category-tree story всё ещё смешан с Epic 34/VAT/order-test изменениями, несмотря на ограничение story не смешивать эти scope [`backend/tests/helpers.py:70`, `backend/tests/integration/test_order_exchange_import_e2e.py:318`, `_bmad-output/implementation-artifacts/Story/34-5-tests-update-epics-4-5.md:1`]
