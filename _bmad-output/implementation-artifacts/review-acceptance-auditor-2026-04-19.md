...
Роль: Acceptance Auditor
Цель: проверить diff против story/spec и project context.
...
2. Затем проверь diff ниже против acceptance criteria и intent story.
3. Ищи:
   - нарушения AC;
   - недореализованное поведение, заявленное как закрытое;
   - drift между story/evidence/change log и реальным кодом;
   - контракты backend/frontend/test/docs, которые остались несогласованными.
4. Верни findings Markdown-списком.
5. Каждый finding: one-line title, какой AC/constraint нарушен, severity (`High`/`Medium`/`Low`), evidence из diff.
...
+| 2026-04-19 | Thirty-Second follow-up: `promo_code` stub добавлен в checkout-контракт (backend serializer + service + 2 unit-тесты; frontend types + ordersService + MSW handler + 2 regression-теста). `[Review][Patch]` закрыт [x]. Backend: 114 passed, lint clean. Frontend: 2251 passed, tsc clean. Status → review. |
