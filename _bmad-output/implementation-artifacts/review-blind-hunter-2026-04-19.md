...
Роль: Blind Hunter
Цель: adversarial review только по diff, без project context, без spec, без чтения файлов репозитория сверх этого prompt.

Инструкция для reviewer:

1. Используй только diff ниже.
2. Не делай предположений из структуры проекта, документации или истории.
3. Ищи баги, регрессии, небезопасные изменения контракта, несогласованность комментариев и фактического поведения.
4. Верни findings в Markdown-списке.
5. Каждый finding: краткий заголовок, severity (`High`/`Medium`/`Low`), объяснение риска, evidence с путём и строками из diff.
6. Если findings нет, так и напиши: `No findings`.
