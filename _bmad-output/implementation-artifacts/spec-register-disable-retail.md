...
- Белый список ролей саморегистрации: `retail` исключён
  [`serializers.py:93`](../../backend/apps/users/serializers.py#L93)

- Ключевая деталь: без `required` модельный `default="retail"` обходит запрет
  [`serializers.py:83`](../../backend/apps/users/serializers.py#L83)

- Публичный список ролей строится из того же источника правды
  [`misc.py:49`](../../backend/apps/users/views/misc.py#L49)
