...
- На странице каталога `(blue)/catalog/page.tsx`: первичная загрузка через `brandsService.getAll({ has_stock: true })`; в `fetchProducts` параллельно с `getVisibleCategories` вызывать `getVisibleBrands(filters)`; результат складывать в `sidebarVisibleBrandIds: Set<number> | null` (null = показывать все).
...
- [ ] `frontend/src/app/(blue)/catalog/page.tsx` — В `fetchProducts` после `getVisibleCategories` (или параллельно в том же `Promise.all`) добавить `brandsService.getVisibleBrands(filters).then(ids => setSidebarVisibleBrandIds(new Set(ids))).catch(() => setSidebarVisibleBrandIds(null))`. Не блокировать рендер на этом запросе (как сейчас для visible-categories).
