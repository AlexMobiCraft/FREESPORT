...
> **Актуальность после 2026-05-05:** эта спецификация описывает старую реализацию выбора популярных категорий на главной. Формулировки про получение "всех корневых категорий" не являются актуальным контрактом публичного дерева. Для навигационного дерева используется `/api/v1/categories-tree/` как публичная проекция прямых потомков `СПОРТ`; для отдельного блока `CategoriesSection` допустима собственная логика отбора (`is_homepage`, `sort_order` или явный список), но не предположение "все `parent=None` = публичные категории".
