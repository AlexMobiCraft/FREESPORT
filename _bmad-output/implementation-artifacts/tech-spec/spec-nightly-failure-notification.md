...
**Approach:** Отдельный job `notify` (`needs: performance`, `if: always()`) с единственным шагом на `actions/github-script@v7`, ветвящимся по `needs.performance.result`: при падении — дописать комментарий в открытый issue с меткой `nightly-failure` либо создать его, если такого нет; при успехе — закрыть все такие issue. Один живой issue на одну непрерывную полосу падений, а не по issue за ночь. Сигнал покрывает **любое** падение прогона: и падение тестов, и сбой окружения до них (`pip install`, postgres, `migrate`, подъём service-контейнеров) — различаются они текстом сообщения, а не наличием сигнала.

## Boundaries & Constraints

**Always:**
- Оповещение только для прогонов на ветке по умолчанию (`develop`); `workflow_dispatch` с feature-ветки issue не создаёт. Отбор по ветке выполняется **внутри скрипта** и оставляет след (`core.notice`), а не в `if` job-а: пропущенный job — серый квадрат без единой строки в логе, то есть отказ в сторону тишины у механизма, чья задача — не молчать.
- Сигнал даёт **любое** падение прогона, а не только падение шага тестов. Причина различается по `steps.tests.outcome`, проброшенному через `outputs` тестового job-а, и уходит в текст сообщения: `failure` → «тесты падают», `success` → «упал шаг после тестов», пусто → «до тестов дело не дошло». Заголовок issue нейтрален к причине — иначе первый по времени повод навсегда определил бы название задачи, в которую потом дописываются падения другого рода.
- Отмена прогона (ручная или по `timeout-minutes`) сигнала не даёт и не должна: намеренную отмену человеком от таймаута не отличить. Ограничение фиксируется в документации.
- Поиск открытого issue — по метке `nightly-failure`, не по заголовку, с детерминированным порядком (`sort: created`, `direction: asc`). Пустая метка обязана обрывать шаг: без неё выборка вернёт весь трекер, а ветка зелёного прогона закроет его скопом.
- В теле issue и комментариев только: имя workflow, ветка, ссылка на прогон, номер и попытка прогона, SHA. Репозиторий `AlexMobiCraft/FREESPORT` публичный.
- Оповещение живёт в **отдельном job**, а не шагом внутри тестового. Две независимые причины: шаг не исполняется, если прогон упал на подъёме service-контейнеров, — а `needs` + `always()` срабатывают; и `issues: write` не должен действовать там, где ставятся зависимости из PyPI и исполняется тестовый код.
- Права выдаются пожобово при `permissions: {}` на уровне workflow: `contents: read` тестовому job-у, `issues: write` — оповещающему. Дописанный позже job не должен молча унаследовать дефолт репозитория.
- Отказ закрытия issue не красит зелёный прогон в красный, но и не молчит: каждая итерация закрытия обёрнута в свой `try/catch` с `core.warning`, а незакрытый issue обозначает себя тем, что остаётся открытым.
...
| Сбой окружения до тестов | `pip install`, postgres, `migrate` или подъём service-контейнеров упали; шаг тестов не выполнялся | Issue создаётся так же, но причина — «до тестов дело не дошло» | N/A |
| Повторное падение | Открытые issue с меткой уже есть (независимо от причины) | Комментарий в **самый ранний** из них; новый issue **не** создаётся | N/A |
| Починка | Прогон зелёный, открытые issue с меткой есть | Закрыты **все**, в каждом комментарий об успехе | Сбой на одном issue не мешает остальным |
| Стабильно зелёный | Прогон зелёный, открытых issue нет | Ничего не происходит | N/A |
| Ручной прогон с feature-ветки | `GITHUB_REF_NAME != default_branch` | Issue не трогается; в лог уходит `core.notice` с названием ветки | N/A |
...
| Отмена или пропуск прогона | `needs.performance.result` вне `success`/`failure` | Ни оповещения, ни закрытия; причина названа в логе | Ограничение задокументировано |
| Метка не задана | `NIGHTLY_LABEL` пуст | Шаг обрывается `setFailed`, **ни один** issue не тронут | Предохранитель от закрытия всего трекера |
| Issue создан с изъяном | GitHub отбросил assignee или метку, вернув 200 | `core.warning` **и** комментарий в сам issue: аннотацию в логе никто не прочтёт | N/A |
