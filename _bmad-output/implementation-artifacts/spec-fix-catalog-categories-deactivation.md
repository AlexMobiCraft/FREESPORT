...
- [x] Прод восстановлен **2026-08-09 полной выгрузкой из 1С**, а не командой: `update_or_create` в ШАГ 1 `process_categories` ставит `is_active=True` каждой пришедшей категории, поэтому полная выгрузка поднимает дерево сама. Проверено: 12 из 12 детей якоря активны, `GET /api/v1/categories-tree/` отдаёт 12, погашенных внутри дерева витрины — 0 (26 погашенных лежат вне якоря: чужие корни и технические). Команда `reactivate_catalog_categories` на проде не запускалась — осталась как инструмент на случай, когда полная выгрузка недоступна. В интерфейсе каталога видно 10 корней: сайдбар скрывает ветки без товаров в наличии (`(blue)/catalog/page.tsx:249`), а «Зимние товары» (1 товар) и «Оборудование» (5 товаров) сейчас без остатков.
