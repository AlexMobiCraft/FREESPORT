...
- E09 — ✅ выполнено 15.09.2026 (решение Alex, исполнение DEV через Django shell на проде, одна транзакция): товар id=12643 «Трико борцовское BoyBo синее, BT45-RU» → brand_id=1 (BoyBo); бренд «No Brand» id=40 удалён (маппингов у него не было); создан `Brand1CMapping` `onec_id=00000000-0000-0000-0000-000000000000` → «Без ТМ» (id=38). Проверка: prod БД — `nobrand` не осталось, `products.brand_id` у id=12643 = 1; `GET /api/v1/brands/` — 13 брендов, «No Brand» отсутствует; `GET /api/v1/products/?brand=no-brand` — 0; `?search=BT45-RU` — id=12643 отдаёт brand «BoyBo».
