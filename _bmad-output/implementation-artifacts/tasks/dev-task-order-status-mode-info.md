...
## Замечания по итогам проверки реализации

Ниже зафиксированы замечания, которые нужно учесть при доработке агентом разработчиком, даже если базовая поддержка `mode=info` уже реализована.

### Follow-up 1. Единый источник дефолтного экспортного статуса

В реализации `handle_info` нельзя опираться только на прямое чтение:

```python
settings.ONEC_EXCHANGE["ORDER_DEFAULTS"]["STATUS"]
```

Почему это важно:

- **`OrderExportService` имеет fallback на `Не согласован`**, даже если `ORDER_DEFAULTS` отсутствует или равен `None`.
- **`mode=info` должен отдавать тот же дефолтный статус, который сайт реально отправляет в 1С** в реквизите `Статус заказа`.
- Иначе возможен рассинхрон: сайт экспортирует `Не согласован`, а справочник `mode=info` этот статус не содержит.

Что требуется:

- **использовать тот же источник / fallback**, что и `OrderExportService`;
- **исключить дублирование логики** вычисления дефолтного статуса между экспортом заказа и `mode=info`;
- **гарантировать**, что фактический дефолтный экспортный статус всегда присутствует в XML-справочнике.

Что проверить тестом:

1. временно переопределить конфигурацию так, чтобы `ORDER_DEFAULTS` отсутствовал или был `None`;
2. вызвать `GET ?type=sale&mode=info`;
3. убедиться, что ответ всё равно содержит:

```xml
<Ид>Не согласован</Ид>
<Название>Не согласован</Название>
```

### Follow-up 2. Формат блока `ПлатежныеСистемы`

Для максимальной совместимости с Bitrix/1С предпочтительно отдавать пустой блок платёжных систем в явном виде:

```xml
<ПлатежныеСистемы></ПлатежныеСистемы>
```

а не только в self-closing форме:
...
- **Bitrix-совместимость здесь важнее компактности XML**;
- некоторые внешние парсеры и legacy-обработчики сравнивают ожидаемую структуру слишком буквально.

Что требуется:

- **предпочтительно использовать парный открывающий/закрывающий тег** `ПлатежныеСистемы`;
- если сохраняется self-closing форма, **обязательно подтвердить** её совместимость ручной проверкой в 1С при выполнении `AC6`.

### Follow-up 3. Явное подтверждение regression scope для `AC5`

В рамках этой задачи недостаточно проверить только новый `mode=info` и fallback для unknown mode.

Нужно явно подтвердить, что после доработки **не сломаны существующие режимы обмена**, перечисленные в `AC5`:
...
- **перед закрытием задачи прогнать существующие integration/e2e/regression-тесты** по обмену заказами, относящиеся к этим режимам;
- **в отчёте dev-агента указать**, какие именно тесты или ручные сценарии подтвердили отсутствие регрессии;
- если часть проверки не автоматизирована, **явно отметить это как manual verification**, а не считать `AC5` закрытым по умолчанию.
