...
### ADR-008: Отписка от маркетинговой email-рассылки

- **Контекст:** Story 41.15 требует публичной отписки до подключения Yandex Cloud Postbox. В текущем коде `Newsletter` хранит состояние подписки, но маркетингового отправщика и публичного экрана нет.
- **Источник истины:** FREESPORT и `Newsletter.is_active`; Postbox на следующем этапе отвечает только за доставку. История `UserConsent` при отписке не удаляется и не переписывается.
- **Локальная отправка:** получатели выбираются только из активных `Newsletter`. Подготовленная очередь хранит ID подписки. Перед каждой первоначальной или повторной попыткой транспорт повторно читает запись; неактивная или отсутствующая запись получает результат `suppressed`, транспорт не вызывается. Этап 1 использует порт `MarketingTransport.send(message) -> TransportResult` и тестовый транспорт без SMTP, HTTP и Postbox SDK.
- **Шаблон:** `message` содержит `recipient`, `subject`, HTML, text/plain, headers, `campaign_id` и `recipient_ref`. Оба тела содержат заметную отписку, URL строится от доверенного публичного origin. Транзакционные письма не затрагиваются.
- **Токен:** уникальный непрозрачный bearer-токен из 32 криптографически случайных байтов в URL-safe Base64 без padding. Он хранится в `Newsletter`, не кодирует email или иные ПДн и действует без календарного срока до нового явного согласия. Каждый успешный запрос подписки атомарно ротирует токен; отписка токен не ротирует. Поэтому повторная отписка идемпотентна, а старая ссылка после нового согласия недействительна.
- **Интерактивный API:** `POST /api/v1/newsletter/unsubscribe/` принимает только JSON `{"token": "…"}`, не использует DRF authentication и возвращает одинаковый `200 processed` для активной и уже неактивной подписки. Отсутствующий, повреждённый, неизвестный или ротированный токен даёт единый `400 invalid_or_expired_unsubscribe_token`; `429` означает ограничение частоты, `503 unsubscribe_processing_failed` — неподтверждённую операцию БД. Ответы не содержат email и не создают подписку. GET/HEAD не мутируют состояние.
- **Legacy API:** существующий анонимный endpoint отписки по email временно сохраняется для совместимости и помечается deprecated. Даты автоматического удаления нет; отключение возможно только отдельным решением владельца после аудита внешних клиентов. Риск отписки по известному email принят на этот период.
- **One-click:** этап 1 реализует собственный RFC 8058 endpoint `POST /api/v1/newsletter/unsubscribe/one-click/<token>/`, принимающий точное form-тело `List-Unsubscribe=One-Click`. GET безопасен. Локальный шаблон формирует `List-Unsubscribe` и `List-Unsubscribe-Post`; этап 2 проверяет передачу заголовков через Postbox и не зависит от событий провайдера для изменения FREESPORT.
- **Публичный экран:** письмо ведёт на `/unsubscribe#<token>`. Fragment не передаётся серверу; клиент читает его, сразу очищает адрес через `history.replaceState` и отправляет токен только в JSON body. Экран доступен анонимно во всех режимах сайта, имеет подтверждение, выполнение, нейтральный успех, недействительную ссылку, `429` и восстанавливаемую ошибку. Он исключён из аналитики, не показывает email, отдаётся с `noindex`, `nofollow`, `Cache-Control: no-store` и `Referrer-Policy: no-referrer`; GET и предпросмотр письма не отписывают.
- **Конкурентность:** подписка и отписка сериализуют изменение одной строки `Newsletter`; первый `unsubscribed_at` не переписывается повтором. Гарантия подавления действует, если отписка зафиксирована до финальной проверки активности. Уже переданное внешнему провайдеру письмо может быть доставлено; владелец принял эту границу 15.09.2026.
- **Граница этапов:** этап 1 доказывает локальную цепочку и не требует подключения Postbox. Доступ, отправитель, реальный шаблон, контролируемая доставка и полная приёмка AC1–AC5 остаются условиями этапа 2.
