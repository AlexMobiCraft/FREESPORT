...
FR-40-01: Парсер контрагентов читает блок `<ЗначенияРеквизитов>` в `customer_data["price_type_ids"]` (GUID, нижний регистр, без пробелов, **дедуплицированные**) и `customer_data["price_type_meta"]` (наименование вида цен, наименование соглашения, признак `СоглашениеТиповое`). Отсутствие блока → пустой список. Обработка `<Роль>` не меняется.
FR-40-02: Сервис `resolve_role_from_price_types(price_type_ids)` возвращает `RoleResolution(role, reason, matched)` с `reason ∈ {resolved, no_data, unknown_price_type, ambiguous}` по правилам: пусто → `no_data`; ни один GUID не найден в `PriceType` → `unknown_price_type`; больше одного GUID с непустой ролью → `ambiguous`; ровно один → `resolved`.
...
FR-40-05: Поле `User.onec_price_type_id` хранит GUID вида цен из 1С и заполняется импортом **всегда**, независимо от того, применена ли роль.
...
FR-40-09: Сессия импорта ведёт счётчики `roles_updated`, `roles_skipped_no_data`, `roles_skipped_unknown_price_type`, `roles_skipped_ambiguous` и показывает их в отчёте.
FR-40-10: Отчёт сессии помечает прогон как аномалию, если блок `<ЗначенияРеквизитов>` не встретился **ни разу** (детектор регресса выгрузки при обновлении расширения БУС).
...
NFR-3940-07: **API-контракт.** После правки сериализаторов и вью — обновление `docs/api/openapi.yaml` и регенерация `npm run generate:types`; рассинхрон ломает сборку фронта.
...
NFR-3940-09: **Производительность импорта.** Маппинг `PriceType → user_role` разрешается не запросом на каждого контрагента: 3202 контрагента в пакете, справочник читается один раз на сессию.
...
- **Данные для тестов эпика 40:** контрольная выгрузка `data/import_1c/contragents_pricetype/` (10 файлов, `test_base` 2026-08-01; каталог `data/import_1c` в git игнорируется). Ожидаемое содержимое: 485 контрагентов = 433 партнёра с блоком, 0 контрагентов с более чем одним различным `ТипЦенId`.
...
**Контекст:** делается по образцу `wholesale_level3` / `opt3_price`. Цены живут на `ProductVariant` (`products/models.py:875,899`), не на `Product`. Вид цен «Опт 4» в 1С действует, `ИспользоватьПриПродаже = Истина`, GUID `4c1962d2-f8ed-11eb-81f3-00155d3cae02`.
...
**Then** поле создано, а `CheckConstraint products_opt4_price_positive` по образцу `products/migrations/0003_add_constraints.py:37` отклоняет отрицательное значение (FR-39-01, NFR-3940-06).
