...
**Acceptance Criteria:**
**Given** получены образцы файлов `orders.xml` от рабочей базы 1С клиента
**When** проведен анализ структуры XML
**Then** задокументированы ключевые теги для: идентификатора заказа, статуса заказа, даты оплаты/отгрузки
**And** составлена таблица маппинга "Статус 1С" -> "Статус FREESPORT"
**And** определена версия CommerceML (2.10 или 3.1) и наличие контейнеров
**And** результаты сохранены в `docs/integrations/1c/analysis-orders-xml.md`
