...
NFR-3940-07: **API-контракт.** После правки сериализаторов и вью — обновление `docs/api/openapi.yaml` и регенерация `npm run generate:types`. ⚠️ **Это требование к артефакту, а не автоматический гейт** (уточнено 2026-08-03, `tech-debt.md` п. 20): `frontend/src/types/api.generated.ts` не импортируется ни одним модулем фронта и не проверяется в CI, поэтому рассинхрон контракта **не ломает** ни `tsc`, ни сборку — прежняя формулировка «рассинхрон ломает сборку фронта» была неверна. Фактическая синхронизация фронта с новым полем достигается ручными правками `types/api.ts` и `services/productsService.ts` (эпик 39 — стори 39.4), и её полнота проверяется только ревью и тестами Vitest.
