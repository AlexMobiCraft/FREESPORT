...
Полный текст эпика и девять стори — в отдельном документе: **`_bmad-output/planning-artifacts/epic-41-site-audit.md`** (решение Alex, 2026-08-24: вынесен из `epics.md`, чтобы не разрастался общий индекс).

Сайт приводится в соответствие с собственной опубликованной политикой обработки ПДн и требованиями ФЗ-152 / ЗоЗПП в точках сбора данных и оплаты, корректно представляется в поиске и отдаёт непротиворечивые заголовки безопасности на всех ответах, включая статику и пользовательские загрузки.

**Источник:** внешний аудит `tmp/audit-report-https___optisport.ru_-full.pdf` от 24.08.2026, общая оценка 20/100. Триаж проверен по коду на коммите `db125f82`; ложные срабатывания сканера отсеяны и перечислены в `_bmad-output/implementation-artifacts/intent-site-audit-2026-08-24.md`.
**Состав:** 25 FR, 7 NFR, 9 стори, 47 критериев приёмки.
**Приоритет:** 🔴 CRITICAL — драйвер запуска сайта, требования регулятора.
**Порядок стори:** 41.0 → 41.5 → 41.3 → {41.1, 41.2, 41.4, 41.6, 41.7} → 41.8.
**Standalone:** да, зависимостей от других эпиков нет.

**Ключевые решения (круглый стол, 2026-08-24):**
- **Критерий приёмки — соответствие закону, а не зелёный отчёт.** Часть замечаний неустранима: чтобы сканер позеленел на форме входа, пришлось бы просить согласие на обработку ПДн ради входа в аккаунт. По этому замечанию оформляется письменное обоснование для регулятора (FR-41-24), само оно остаётся красным.
- **Стори 41.0 (настоящий HTTP 404) — ключевая.** Гасит пять замечаний одной правкой, убирая 14 несуществующих адресов из обхода сканера. Ставится первой: меняет исходные данные для всех SEO-требований.
- **Найдена уязвимость, аудитом не обнаруженная (FR-41-22).** В nginx `add_header` внутри `location` заменяет весь унаследованный набор — `/static/`, `/media/`, картинки товаров и `/health` отдаются без единого заголовка безопасности. `/media/` с файлами из выгрузки 1С идёт без `nosniff`.
- **Снято как избыточное:** чекбокс согласия в форме оформления заказа — заказ требует `IsAuthenticated`, согласие получено при регистрации.

**Вынесено за пределы эпика:** `tech-debt.md` п. 22 (2FA, заявленная политикой), п. 23 (декоративный CSP), `tasks/dev-task-marketing-consent-journal-cleanup.md` (чистка недостоверных записей `marketing_email` за 3,5 месяца — операция над данными прода).
