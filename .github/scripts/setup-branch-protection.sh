...
#    текущее состояние.
# 2. Три из четырёх строк в REQUIRED_CONTEXTS, скорее всего, ни с чем не совпадают.
#    GitHub именует check-run по имени джобы (плюс матрица), а не «workflow (job)»:
#    реальный чек называется `build (3.12)`, а не «Django CI (build)». У backend-ci.yml
#    и frontend-ci.yml джобы обе называются `test` — их чеки неразличимы между собой,
...
#    paths, которая всегда сообщает успех. Preflight этого НЕ ловит — он смотрит один
#    коммит, где нужные пути могли быть затронуты.
