...
ONEC_EXCHANGE = {
    ...
    # vat_rate указывается только для складов с ЕДИНОЙ фиксированной ставкой.
    # "1 СДВ склад" и "Intex ОСНОВНОЙ" имеют переменную ставку (22% и 10%) —
    # для них ставка берётся из goods.xml (variant.vat_rate), не из склада.
    "WAREHOUSE_RULES": {
        "1 СДВ склад": {"organization": "ИП Семерюк Д.В."},
        "Intex ОСНОВНОЙ": {"organization": "ИП Семерюк Д.В."},
