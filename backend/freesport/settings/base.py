...
# ============================================================================
# Политика саморегистрации
# ============================================================================
# Розничная саморегистрация выключена: портал работает как B2B-площадка, и
# розничные заявки никто не верифицирует. Отключение временное — розница
# вернётся отдельным сайтом на этом же бэкенде, поэтому список ролей строится
# из флага, а не зашит в код: включение не требует релиза, только правки
# окружения и рестарта.
#
# Флаг управляет тремя местами сразу (см. apps/users/serializers.py:
# get_self_service_roles): choices поля `role` при регистрации, серверной
# проверкой роли и списком ролей, который отдаёт GET /api/v1/users/roles/.
# Розничный фронтенд, когда появится, поднимается с REGISTRATION_ALLOW_RETAIL=True.
REGISTRATION_ALLOW_RETAIL = config("REGISTRATION_ALLOW_RETAIL", default=False, cast=bool)
...
ONEC_EXCHANGE = {
    ...
    # Все склады имеют фиксированную ставку НДС.
    "WAREHOUSE_RULES": {
        "1 СДВ склад": {"organization": "ИП Семерюк Д.В."},
    ...
    "PRICE_TYPE_ID_BY_NAME": {
        "РРЦ": "3d1482c4-bd77-11e4-afc8-20cf3073dde3",
