...
class TestPersonalCabinetWorkflow:
    ...
    def test_complete_order_workflow(
        ...
        """Тест полного workflow: создание заказа → отображение в дашборде → история заказов"""
        ...
        assert float(updated_data["total_order_amount"]) == 8000.0  # 5000 + 3000
        assert float(updated_data["avg_order_amount"]) == 4000.0  # 8000 / 2
        ...
        order3 = order_factory.create(
            user=user, order_number="INT-003", status="cancelled", total_amount=1500.00
        )
        ...
        assert float(final_data["total_order_amount"]) == 9500.0  # 5000 + 3000 + 1500
        assert abs(float(final_data["avg_order_amount"]) - 3166.67) < 0.01  # 9500 / 3
    ...
    def test_b2c_vs_b2b_workflow_differences(self, user_factory, order_factory):
        ...
        b2b_user = user_factory.create(
            role="wholesale_level2", is_verified=True, company_name="B2B Test Company"
        )
