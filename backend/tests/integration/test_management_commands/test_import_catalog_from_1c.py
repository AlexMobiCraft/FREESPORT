...
class TestImportCatalogFrom1CCommand:
    ...
    def test_command_creates_products_with_mock_data(self):
        ...
        call_command("import_catalog_from_1c", "--mock-data", verbosity=0)
    ...
    def test_command_with_json_file_success(self):
            ...
            "products": [
                {
                    ...
                    "sku": "JSON-SKU-001",
                    "stock_quantity": 10,
                    "prices": {"retail_price": "1000.00", "opt1_price": "900.00"},
                }
            ]
        }
        ...
        try:
            call_command(
                "import_catalog_from_1c", f"--file={json_file_path}", verbosity=0
            )
    ...
    def test_command_with_nonexistent_file_fails(self):
        ...
        with pytest.raises(CommandError, match="Файл не найден"):
            call_command(
                "import_catalog_from_1c", "--file=/nonexistent/file.json", verbosity=0
            )
    ...
    def test_command_xml_not_implemented(self):
        ...
        try:
            with pytest.raises(CommandError, match="XML парсер будет реализован"):
                call_command(
                    "import_catalog_from_1c", f"--file={xml_file_path}", verbosity=0
                )
        finally:
    ...
    def test_command_without_parameters_fails(self):
        ...
        with pytest.raises(CommandError, match="Укажите либо --file"):
            call_command("import_catalog_from_1c", verbosity=0)

    def test_command_dry_run_no_changes(self):
        """Тест что dry-run не создает товары"""
        initial_count = Product.objects.count()

        call_command("import_catalog_from_1c", "--mock-data", "--dry-run", verbosity=0)
    ...
    def test_command_force_overwrites_existing(self):
        ...
        call_command("import_catalog_from_1c", "--mock-data", "--force", verbosity=0)
    ...
    def test_command_without_force_skips_existing(self):
        ...
        out = StringIO()
        call_command("import_catalog_from_1c", "--mock-data", stdout=out, verbosity=0)
