...
def setup_test_data():
    ...
    brand, created = Brand.objects.get_or_create(
        ...
        defaults={'slug': 'nike-detail-test', 'description': 'Тестовый бренд для детального просмотра'}
    )
    ...
    for product_data in related_products_data:
        ...
        print(f"Связанный товар: {'создан' if created else 'найден'} - {related_product.name}")
...
def test_product_detail_basic():
    ...
    print_test_result(f"GET /products/{test_product.id}/ (полная информация)", success,
    ...
    if success:
        ...
        required_fields = ['id', 'name', 'sku', 'description', 'specifications', 
                          'current_price', 'stock_quantity', 'images', 'related_products',
                          'category_breadcrumbs', 'brand', 'category', 'seo_title', 'seo_description']
...
def test_role_based_pricing():
    ...
    if retail_token:
        ...
        response = requests.get(f"{BASE_URL}/products/{test_product.id}/", headers=headers)
        ...
        if response.status_code == 200:
            ...
            print_test_result("Retail пользователь (RRP/MSRP скрыты)", success,
    ...
    if wholesale_token:
        ...
        if response.status_code == 200:
            ...
            print_test_result("B2B пользователь (RRP/MSRP отображаются)", success,
...
def test_product_images():
    ...
    response = requests.get(f"{BASE_URL}/products/{test_product.id}/")
    success = response.status_code == 200
    print_test_result("Галерея изображений в response", success, f"Status: {response.status_code}")
    ...
    if success:
        ...
        for i, image in enumerate(images):
            ...
            missing_fields = [field for field in required_image_fields if field not in image]
            if missing_fields:
                print(f"   [WARNING] Изображение {i+1} - отсутствуют поля: {missing_fields}")
            else:
...
def test_related_products():
    ...
    response = requests.get(f"{BASE_URL}/products/{test_product.id}/")
    success = response.status_code == 200
    print_test_result("Связанные товары в response", success, f"Status: {response.status_code}")
...
def test_specifications_and_details():
    ...
    response = requests.get(f"{BASE_URL}/products/{test_product.id}/")
    success = response.status_code == 200
    print_test_result("Спецификации и детали", success, f"Status: {response.status_code}")
    ...
    if success:
        ...
        print(f"   SEO Description: {seo_description[:50]}..." if seo_description else "   SEO Description: None")
        
        # Проверяем информацию о наличии
        stock_quantity = data.get('stock_quantity')
        min_order_quantity = data.get('min_order_quantity')
        print(f"   Наличие: {stock_quantity} шт., Минимальный заказ: {min_order_quantity} шт.")
...
def test_discount_calculation():
    ...
    if trainer_token:
        ...
        if response.status_code == 200:
            ...
            print_test_result("Расчет скидки для trainer", success,
...
def test_product_not_found():
    ...
    print_test_result("GET /products/999999/ (404 Not Found)", success,
                     f"Status: {response.status_code}")
...
def main():
    ...
    # Подсчет результатов
    total_tests = 7
    passed_tests = sum([ac1_result, ac2_result, ac3_result, ac4_result, ac5_result, discount_result, not_found_result])
