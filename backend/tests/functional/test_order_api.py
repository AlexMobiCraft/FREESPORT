...
from tests.conftest import (
    UserFactory, ProductFactory, CategoryFactory, BrandFactory,
...
class TestOrderAPI:
    ...
    def test_create_order_success(self, api_client, authenticated_user, cart_with_items, test_product):
    ...
    def test_create_order_insufficient_stock(self, api_client, authenticated_user, test_product):
    ...
    def test_create_order_b2b_payment_validation(self, api_client, b2b_user, test_product):
    ...
    def test_create_order_b2b_success(self, api_client, b2b_user, test_product):
        ...
        response = api_client.post(url, data, format='json')
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data['total_amount'] == '1900.00'  # 900 (B2B цена) + 1000 (транспорт)
    ...
    def test_delivery_cost_calculation(self, api_client, authenticated_user, test_product):
    ...
    def test_order_response_structure(self, api_client, authenticated_user, cart_with_items):
        ...
        required_fields = [
            'id', 'order_number', 'customer_display_name', 'status',
            'total_amount', 'delivery_cost', 'delivery_address',
            'delivery_method', 'payment_method', 'payment_status',
            'created_at', 'updated_at', 'items', 'subtotal',
            'total_items', 'calculated_total', 'can_be_cancelled'
        ]
        ...
        item_fields = ['id', 'product', 'product_name', 'product_sku', 
...
class TestOrderAPIEdgeCases:
    ...
    def test_create_order_min_quantity_validation(self, api_client, authenticated_user):
        ...
        cart = CartFactory.create(user=authenticated_user)
        CartItemFactory.create(cart=cart, product=product, quantity=2)  # меньше min_order_quantity
    ...
    def test_order_with_multiple_products(self, api_client, authenticated_user):
        ...
        product1 = ProductFactory.create(
            ...
            retail_price=Decimal('1000.00'), stock_quantity=10
        )
        product2 = ProductFactory.create(
            category=category, brand=brand,
            retail_price=Decimal('2000.00'), stock_quantity=5
        )
