...
def register_test_user():
    ...
    print_test_result(
        "Регистрация пользователя", success, f"Status: {response.status_code}"
    )
...
def test_dashboard(access_token):
    ...
    success = response.status_code == 200
    print_test_result(
        "GET /users/profile/dashboard/", success, f"Status: {response.status_code}"
    )

    if success:
        data = response.json()
        print(
            f"   Счетчики: заказы={data.get('orders_count', 0)}, "
...
def test_addresses(access_token):
    ...
    response = requests.post(
        f"{BASE_URL}/users/addresses/", json=address_data, headers=headers
    )
    success = response.status_code == 201
    print_test_result(
        "POST /users/addresses/ (создание)", success, f"Status: {response.status_code}"
    )
    ...
    success = response.status_code == 200
    print_test_result(
        "GET /users/addresses/ (список)", success, f"Status: {response.status_code}"
    )
    ...
    if success and address_id:
        ...
        print(
            f"   Найдено адресов: {len(addresses) if isinstance(addresses, list) else 1}"
        )

        # Тест обновления адреса
        update_data = {"city": "Санкт-Петербург"}
        response = requests.patch(
            f"{BASE_URL}/users/addresses/{address_id}/",
            json=update_data,
            headers=headers,
        )
        success = response.status_code == 200
        print_test_result(
            "PATCH /users/addresses/{id}/ (обновление)",
            success,
...
def test_favorites(access_token):
    ...
    if not Product.objects.exists():
        ...
        brand, _ = Brand.objects.get_or_create(
            name="Test Brand", defaults={"slug": "test-brand"}
        )
    ...
    response = requests.post(
        f"{BASE_URL}/users/favorites/", json=favorite_data, headers=headers
    )
    success = response.status_code == 201
    print_test_result(
        "POST /users/favorites/ (добавление)",
        success,
        f"Status: {response.status_code}",
    )
    ...
    success = response.status_code == 200
    print_test_result(
        "GET /users/favorites/ (список)", success, f"Status: {response.status_code}"
    )
    ...
    if success:
        ...
        if isinstance(favorites, list) and len(favorites) > 0:
            ...
            response = requests.delete(
                f"{BASE_URL}/users/favorites/{favorite_id}/", headers=headers
            )
            success = response.status_code == 204
            print_test_result(
                "DELETE /users/favorites/{id}/ (удаление)",
                success,
                f"Status: {response.status_code}",
            )
...
def test_orders_history(access_token):
    ...
    success = response.status_code == 200
    print_test_result(
        "GET /users/orders/ (история заказов)",
        success,
        f"Status: {response.status_code}",
    )
