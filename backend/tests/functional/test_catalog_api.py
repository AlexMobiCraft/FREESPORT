...
def test_products_list():
    ...
    print_test_result(
        "GET /products/ (список товаров)", success, f"Status: {response.status_code}"
    )
    ...
    if success:
        ...
        if results:
            ...
            print(
                f"   Первый товар: {first_product.get('name')} - {first_product.get('current_price')} руб."
            )
...
def test_products_filtering():
    ...
    success = response.status_code == 200
    print_test_result(
        "GET /products/?in_stock=true", success, f"Status: {response.status_code}"
    )
    ...
    success = response.status_code == 200
    print_test_result(
        "GET /products/?is_featured=true", success, f"Status: {response.status_code}"
    )
...
def test_products_sorting():
    ...
    success = response.status_code == 200
    print_test_result(
        "GET /products/?ordering=name", success, f"Status: {response.status_code}"
    )
    ...
    success = response.status_code == 200
    print_test_result(
        "GET /products/?ordering=-retail_price",
        success,
        f"Status: {response.status_code}",
    )
    ...
    if success:
        ...
        if len(results) >= 2:
            print(
                f"   Первый товар: {results[0].get('name')} - {results[0].get('current_price')} руб."
            )
            print(
                f"   Второй товар: {results[1].get('name')} - {results[1].get('current_price')} руб."
            )
...
def test_role_based_pricing():
    ...
    if retail_token:
        ...
        response = requests.get(
            f"{BASE_URL}/products/?search={product_sku}", headers=headers
        )
        ...
        if response.status_code == 200:
            ...
            if data["results"]:
                ...
                print_test_result(
                    "Ценообразование для retail", True, f"Цена: {retail_price} руб."
                )
    ...
    if wholesale_token:
        ...
        if response.status_code == 200:
            ...
            if data["results"]:
                ...
                print_test_result(
                    "Ценообразование для wholesale_level1",
                    True,
    ...
    if trainer_token:
        ...
        if response.status_code == 200:
            ...
            if data["results"]:
                ...
                print_test_result(
                    "Ценообразование для trainer", True, f"Цена: {trainer_price} руб."
                )
...
def test_categories():
    ...
    if success:
        ...
        print(
            f"   Найдено категорий: {len(categories) if isinstance(categories, list) else 1}"
        )
...
def test_brands():
    ...
    success = response.status_code == 200
    print_test_result(
        "GET /brands/ (список брендов)", success, f"Status: {response.status_code}"
    )
...
def test_product_detail():
    ...
    if response.status_code == 200:
        ...
        if results:
            ...
            success = response.status_code == 200
            print_test_result(
                f"GET /products/{product_id}/ (детали товара)",
                success,
                f"Status: {response.status_code}",
            )
            ...
            if success:
                ...
                print(
                    f"   Категории в breadcrumbs: {len(product.get('category_breadcrumbs', []))}"
                )
