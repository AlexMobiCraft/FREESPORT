...
class OrderCreationPerformanceTest(TestCase):
    ...
    def test_single_item_order_performance(self):
        ...
        self.assertLess(response_time, 1.0, f"Single item order creation time {response_time:.2f}s exceeds 1s limit")
    ...
    def test_multiple_items_order_performance(self):
        ...
        self.assertLess(response_time, 1.5, f"Multi-item order creation time {response_time:.2f}s exceeds 1.5s limit")
        self.assertEqual(response.status_code, 201)
    ...
    def test_b2b_order_performance(self):
        ...
        self.assertLess(response_time, 1.2, f"B2B order creation time {response_time:.2f}s exceeds 1.2s limit")
        self.assertEqual(response.status_code, 201)
    ...
    def test_large_quantity_order_performance(self):
        ...
        self.assertLess(response_time, 1.5, f"Large quantity order creation time {response_time:.2f}s exceeds 1.5s limit")
        self.assertEqual(response.status_code, 201)
    ...
    def test_order_calculation_performance(self):
        ...
        self.assertLess(response_time, 2.0, f"Order calculation time {response_time:.2f}s exceeds 2s limit")
        self.assertEqual(response.status_code, 201)
    ...
    def test_concurrent_order_creation_performance(self):
        ...
        for i in range(5):
            ...
            thread = threading.Thread(
                target=create_order,
                args=(user, product_id, results_queue)
            )
        ...
        for result in results:
            ...
            self.assertLess(result['response_time'], 3.0, f"Concurrent order creation time exceeds 3s")
    ...
    def test_order_database_queries_optimization(self):
        ...
        self.assertLess(queries_count, 20, f"Order creation generates too many DB queries: {queries_count}")
        self.assertEqual(response.status_code, 201)
    ...
    def test_order_memory_usage(self):
        ...
        self.assertLess(memory_mb, 40, f"Order creation memory usage {memory_mb:.2f}MB exceeds 40MB limit")
