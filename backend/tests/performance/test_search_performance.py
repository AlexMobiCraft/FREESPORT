...
class SearchPerformanceTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        for i in range(5):
            category = Category.objects.create(
                name=f'Search Category {i}',
                slug=f'search-category-{i}'
            )
        ...
        for i in range(3):
            brand = Brand.objects.create(
                name=f'Search Brand {i}',
                slug=f'search-brand-{i}'
            )
    ...
    def test_simple_search_performance(self):
        ...
        self.assertLess(response_time, 0.5, f"Simple search response time {response_time:.2f}s exceeds 0.5s limit")
    ...
    def test_complex_search_performance(self):
        ...
        self.assertLess(response_time, 1.0, f"Complex search response time {response_time:.2f}s exceeds 1s limit")
        self.assertEqual(response.status_code, 200)
    ...
    def test_full_text_search_performance(self):
        ...
        response = self.client.get('/api/v1/products/', {
            'search': 'Описание товара для теннис'
        })
        
        end_time = time.time()
        response_time = end_time - start_time
        
        self.assertLess(response_time, 0.8, f"Full text search response time {response_time:.2f}s exceeds 0.8s limit")
        self.assertEqual(response.status_code, 200)
    ...
    def test_empty_search_performance(self):
        ...
        response = self.client.get('/api/v1/products/', {
            'search': 'несуществующий_товар_xyz123'
        })
        
        end_time = time.time()
        response_time = end_time - start_time
        
        self.assertLess(response_time, 0.3, f"Empty search response time {response_time:.2f}s exceeds 0.3s limit")
        self.assertEqual(response.status_code, 200)
    ...
    def test_search_pagination_performance(self):
        ...
        self.assertLess(response_time, 0.6, f"Search pagination response time {response_time:.2f}s exceeds 0.6s limit")
        self.assertEqual(response.status_code, 200)
    ...
    def test_search_sorting_performance(self):
        ...
        for sort_by in sorting_options:
            with self.subTest(sort_by=sort_by):
                ...
                self.assertLess(response_time, 0.7, f"Search sorting by {sort_by} response time {response_time:.2f}s exceeds 0.7s limit")
                self.assertEqual(response.status_code, 200)
    ...
    def test_search_stress_test(self):
        ...
        self.assertLess(avg_response_time, 0.6, f"Average search response time {avg_response_time:.2f}s exceeds 0.6s")
        self.assertLess(max_response_time, 1.5, f"Max search response time {max_response_time:.2f}s exceeds 1.5s")
