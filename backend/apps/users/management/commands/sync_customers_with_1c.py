...
class Command(BaseCommand):
        ...
        for customer in progress_bar:
            try:
                ...
                if export_result['success']:
                    ...
                    customer.save(
                        update_fields=[
                            'onec_id', 'needs_1c_export', 'sync_status',
    ...
    def _import_customer_updates(self) -> int:
        ...
        for update_data in progress_bar:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'❌ Ошибка импорта обновления {update_data.get("onec_id", "UNKNOWN")}: {str(e)}'
                    )
                )
    ...
    def _import_customer_update(self, update_data: Dict) -> bool:
        ...
        try:
            ...
            if update_data.get('phone_number') != customer.phone_number:
