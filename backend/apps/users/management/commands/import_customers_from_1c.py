...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--file',
            type=str,
            help='Путь к файлу данных 1С (XML или JSON)'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if self.use_mock_data:
                ...
                self.stdout.write(f"📦 Загружены тестовые данные: {len(customers_data)} клиентов")
            else:
    ...
    def _get_mock_customers_data(self) -> List[Dict]:
        ...
        mock_roles = ['retail', 'wholesale_level1', 'wholesale_level2', 'wholesale_level3', 'trainer']
        mock_companies = [
            'Спорт Маркет ООО', 'Атлет Спорт', 'Фитнес Клуб Энергия', 
            'Спортивная База', 'Олимп Спорт', 'Динамо Снаб', 'Чемпион Трейд'
        ]
        ...
        for i in range(1, 16):  # 15 тестовых клиентов
            customer = {
                ...
                'company_name': mock_companies[i % len(mock_companies)] if i % 3 == 0 else '',
