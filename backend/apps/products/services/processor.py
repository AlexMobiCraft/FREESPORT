...
class ProductDataProcessor:
    """
    Процессор для создания и обновления товаров на основе данных из
    XMLDataParser

    Методы:
    - create_product_placeholder() - создание заготовки товара из goods.xml
      с импортом изображений
    - import_product_images() - копирование изображений товара из 1С в
      Django media (Story 3.1.2)
    ...
    def create_product_placeholder(
        ...
        """
        Создание заготовки товара из goods.xml с опциональным импортом
        изображений
        ...
        try:
            ...
            try:
                ...
                self._log_error(
                    f"Error saving product placeholder: {save_error}",
                    goods_data,
                )
                return None
    ...
    def _log_brand_mapping_missing(self, brand_id: str, parent_id: str) -> None:
        ...
        self._increment_brand_fallbacks()
        logger.warning(
            "Brand1CMapping not found for onec_id=%s, product=%s, "
            "session=%s, using 'No Brand' fallback",
            brand_id,
            parent_id,
            self.session_id,
        )
    ...
    def _update_image_stats(self, image_result: dict[str, int]) -> None:
        ...
        # Исправляем несоответствие ключей: метод возвращает "copied",
        # "skipped", "errors" а статистика ожидает "images_copied",
        # "images_skipped", "images_errors"
        key_mapping = {
    ...
    def process_categories(self, categories_data: list[CategoryData]) -> dict[str, int]:
        ...
        for category_data in categories_data:
            try:
                ...
                if not onec_id or not name:
                    # Skipping category with missing id or name:
                    # {category_data}
                    result["errors"] += 1
                    continue
        ...
        # Categories processed: {result['created']} created,
        # {result['updated']} updated, {result['errors']} errors,
        # {result['cycles_detected']} cycles detected
        return result
    ...
    def process_brands(self, brands_data: Sequence[BrandData]) -> dict[str, int]:
        ...
        Returns:
            dict с количеством brands_created, mappings_created,
            mappings_updated
        """
        ...
        for brand_data in brands_data:
            try:
                ...
                if existing_mapping:
                    # Маппинг уже существует - обновляем onec_name если
                    # изменилось
                ...
                if existing_brand:
                    # Бренд существует - создаём только маппинг (объединение
                    # дубликатов)
                    ...
                    # Создаём бренд (normalized_name установится автоматически
                    # в save())
    ...
    def import_product_images(
        ...
        Args:
            ...
            base_dir: Базовая директория импорта (например,
                      data/import_1c/goods/)
        ...
        logger.info(
            f"Starting image import for product {product.onec_id} with "
        ...
        for image_path in image_paths:
            try:
                ...
                if not source_path.exists():
                    # Image file not found: {source_path} for product
                    # {product.onec_id}
                    result["errors"] += 1
                    continue
                ...
                # image_path:
                # "00/001a16a4-b810-11ed-860f-fa163edba792_24062354.jpg"
                ...
                if default_storage.exists(destination_path):
                    ...
                    if not main_image_set:
                        logger.info(
                            f"Setting existing file as main_image: "
                            f"{destination_path}"
                        )
        ...
        if main_image_set or gallery_images:
            ...
            logger.info(
                f"Product {product.onec_id} updated with main_image: "
                f"{product.main_image}, gallery count: {len(gallery_images)}"
            )
