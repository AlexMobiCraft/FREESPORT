...
class TestImportProductsFallback(TestCase):
    def test_import_variants_fallback_logic(self):
        ...
        mock_parser.parse_offers_xml.return_value = [] # Return empty list so loop doesn't run, we just check base_dir logic
        ...
        with patch("os.path.exists") as mock_exists:
            ...
            mock_exists.side_effect = lambda result: True if result == os.path.join(data_dir, "offers", "import_files") else False
            
            # Mock _collect_xml_files to return one file
            command._collect_xml_files = MagicMock(return_value=["/tmp/data/offers/offers.xml"])
            ...
            self.assertFalse(any(fallback_msg in str(c) for c in calls), "Fallback should not trigger when offers dir exists")
        ...
        with patch("os.path.exists") as mock_exists:
            ...
            fallback_msg = "Изображения будут загружаться из"
            calls = [args[0] for args, _ in command.stdout.write.call_args_list]
            self.assertTrue(any(fallback_msg in str(c) for c in calls), "Fallback SHOULD trigger when offers dir is missing and goods dir exists")
