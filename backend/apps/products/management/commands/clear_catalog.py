...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--confirm", action="store_true", help="Подтверждение очистки всех данных"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.WARNING(
                "Это удалит все товары, бренды, категории и связанные данные!"
            )
        )
        ...
        self.stdout.write(
            self.style.WARNING(
                "Рекомендуется создать бэкап перед очисткой: python manage.py backup_db"
            )
        )

        # Запрос подтверждения
        self.stdout.write(
            self.style.WARNING('Для подтверждения введите "yes" и нажмите Enter:')
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                "💡 Теперь можно выполнить повторный импорт с чистой базой"
            )
        )
