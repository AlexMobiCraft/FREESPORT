...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--with-categories',
            action='store_true',
            help='Создать категории товаров'
        )
        
        parser.add_argument(
            '--with-brands',
            action='store_true',
            help='Создать бренды товаров'
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.ERROR(f'❌ Ошибка загрузки каталога: {str(e)}')
            )
            raise
    ...
    def _create_brands(self) -> List[Brand]:
        ...
        brand_names = [
            'Nike Test', 'Adidas Test', 'Puma Test', 'Reebok Test',
            'Under Armour Test', 'New Balance Test', 'ASICS Test',
            'Wilson Test', 'Head Test', 'Babolat Test'
        ]
    ...
    def _create_categories(self) -> List[Category]:
        ...
        category_names = [
            'Обувь тестовая', 'Одежда тестовая', 'Аксессуары тестовые',
            'Инвентарь тестовый', 'Мячи тестовые', 'Ракетки тестовые',
            'Защита тестовая', 'Тренажеры тестовые'
        ]
    ...
    def _generate_product_data(self, index: int, brands: List[Brand], categories: List[Category]) -> Dict:
        ...
        materials = ['Cotton', 'Polyester', 'Nylon', 'Leather', 'Synthetic', 'Mesh', 'Canvas']
        colors = ['Black', 'White', 'Blue', 'Red', 'Green', 'Yellow', 'Gray', 'Navy', 'Pink']
        sizes = ['XS', 'S', 'M', 'L', 'XL', 'XXL']
        
        product_types = [
            'Кроссовки', 'Футболка', 'Шорты', 'Куртка', 'Мяч',
            'Ракетка', 'Перчатки', 'Очки', 'Сумка', 'Бутылка'
        ]
        ...
        return {
            ...
            'min_order_quantity': random.choice([1, 1, 1, 5, 10]),  # 60% - штучные товары
            'specifications': {
            ...
            'sync_status': random.choice(['synced', 'synced', 'pending']),  # 70% синхронизированных
