...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        logs_to_delete = CustomerSyncLog.objects.filter(
        ...
        if not force:
            ...
            if confirm.lower() != "yes":
                self.stdout.write(
                    self.style.WARNING("Отменено пользователем")
                )
                return
    ...
    def _delete_logs(self, queryset, batch_size, total_count):
        ...
        try:
            while True:
                ...
                self.stdout.write(
                    f"  Удалено: {deleted_count}/{total_count} "
                    f"({progress:.1f}%)",
                    ending="\r"
                )
            ...
            self.stdout.write(
                f"Удалено до ошибки: {deleted_count} логов"
            )
            raise
