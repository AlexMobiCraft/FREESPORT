...
class UserConsent(models.Model):
    ...
    # Семантика типов (решение Alex, стори 41.3, 2026-08-30).
    # `pdp_contract` читается шире своей исторической метки: это «согласие на
    # обработку ПДн на заявленном правовом основании», а не исключительно
    # договор поставки. Подписка на рассылку — законный частный случай:
    # обработка адреса является техническим условием отправки письма,
    # о котором человек попросил сам. Поэтому эндпоинт подписки
    # (`apps/common/views.py`, `subscribe`) осознанно пишет ДВЕ записи:
    # `pdp_contract` (ФЗ-152 ст. 9) и `marketing_email` (ФЗ-38 ст. 18) —
    # разные правовые основания, разные сроки отзыва.
    # Метку не переименовываем: смена текста choices тянет миграцию без
    # выигрыша в поведении. Отдельный тип `pdp_newsletter` рассмотрен
    # и отклонён как избыточный.
