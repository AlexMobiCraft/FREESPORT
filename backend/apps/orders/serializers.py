...
class OrderDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [
            'id', 'order_number', 'user', 'customer_display_name',
            'customer_name', 'customer_email', 'customer_phone',
            'status', 'total_amount', 'discount_amount', 'delivery_cost',
            'delivery_address', 'delivery_method', 'delivery_date',
            'payment_method', 'payment_status', 'payment_id',
            'notes', 'created_at', 'updated_at',
            'items', 'subtotal', 'total_items', 'calculated_total',
            'can_be_cancelled'
        ]
        read_only_fields = [
            'id', 'order_number', 'user', 'customer_display_name',
...
class OrderCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if user and hasattr(user, 'role'):
            if user.role.startswith('wholesale') and payment_method not in ['bank_transfer', 'payment_on_delivery']:
                raise serializers.ValidationError(
...
class OrderListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Order
        fields = [
            'id', 'order_number', 'customer_display_name',
            'status', 'total_amount', 'delivery_method',
