...
**`visible_brands()` - GET /products/visible-brands/**

- **Цель:** Получение ID брендов, у которых есть товары по текущим фильтрам каталога
- **Логика:** Копирует query-параметры, удаляет `brand`, применяет `ProductFilter`, исключает `brand_id IS NULL` и возвращает уникальные `brand_id`
- **Параметры:** category_id, min_price, max_price, in_stock, search; `brand` намеренно игнорируется
- **Ответ:** `{"brand_ids": [1, 2, ...]}`
- **Особенность:** Endpoint используется сайдбаром `/catalog` только в режиме `in_stock=true`; при ошибке frontend возвращается к полному списку брендов из первичного `has_stock=true`-гейта
...
**Назначение:** Получение публичного дерева категорий для витрины

#### Методы

**`get_queryset()`**

- **Цель:** Загрузка публичных корневых категорий каталога.
- **Логика:** В БД сохраняется полное дерево 1С с корневым якорем `СПОРТ`; эндпоинт отдаёт прямых детей активного `СПОРТ` как корни витрины. Если активных корневых якорей `СПОРТ` несколько, возвращается объединение их прямых детей и пишется warning для последующего восстановления данных.
- **Фильтрация:** Рекурсивно исключаются `uncategorized`, `onec-unresolved-category`, `Без категории` и категории-заглушки вида `Категория <uuid/legacy-id>`.
- **Пагинация:** Отключена (`pagination_class = None`), дерево возвращается целиком.
- **Возврат:** QuerySet публичных корневых категорий.

**`list()` - GET /categories-tree/**

- **Цель:** Получение публичного дерева категорий для навигации, главной страницы и `/catalog`.
- **Логика:** Иерархическая структура уже является витринной проекцией; фронтенд не должен применять эвристику "один root => показать children".
- **Ответ:** Дерево публичных категорий от витринных корневых узлов до листьев.
...
- **Логика:** Фильтрация is_active=True, сортировка по name. Для `list` поддерживается `has_stock=true`: бренд попадает в ответ, если есть активный товар с вариантом `stock_quantity > 0`. Для `featured` и `retrieve` этот гейт не применяется.
- **Возврат:** QuerySet[Brand]

**`list()` - GET /brands/**

- **Цель:** Получение списка активных брендов для фильтра каталога
- **Параметры:** `is_featured`, `has_stock`
- **Ответ:** Список брендов для фильтрации. `has_stock=false`, `0` или отсутствие параметра сохраняют прежнее поведение без гейта по наличию.
...
| GET    | `/api/v1/products/visible-brands/` | Видимые бренды по текущим фильтрам | Нет            |
| GET    | `/api/v1/categories/`        | Список категорий             | Нет            |
| GET    | `/api/v1/categories/{slug}/` | Детали категории             | Нет            |
| GET    | `/api/v1/categories-tree/`   | Публичное дерево категорий   | Нет            |
| GET    | `/api/v1/brands/`            | Список брендов, опционально `has_stock=true` | Нет            |
| GET    | `/api/v1/brands/{slug}/`     | Детали бренда                | Нет            |
| GET    | `/api/v1/brands/featured/`   | Избранные бренды главной страницы | Нет            |
