...
**UI Testing Framework: ✅ Playwright** [Source: https://playwright.dev/python/]

Playwright - современный E2E testing framework от Microsoft с отличной поддержкой Python.
...
# pytest.ini
[pytest]
addopts = --headed  # Показывать браузер при тестировании (опционально)
```

**Пример E2E теста:**
```python
# backend/tests/e2e/test_import_flow.py
from playwright.sync_api import Page, expect
import pytest

@pytest.fixture
def authenticated_page(page: Page):
    """Авторизованная страница Django Admin"""
...
def test_full_catalog_import_flow(authenticated_page: Page):
    # 1. Открыть страницу импорта
    authenticated_page.goto("http://localhost:8000/admin/integrations/import_1c/")
    
    # 2. Выбрать "Полный каталог"
    authenticated_page.check('input[name="import_type"][value="catalog"]')
    
    # 3. Нажать "Запустить импорт"
    authenticated_page.click('input[type="submit"]')
    
    # 4. Проверить редирект на страницу сессий
    expect(authenticated_page).to_have_url("/admin/integrations/session/")
    
    # 5. Проверить что сессия создана
    expect(authenticated_page.locator('table tbody tr').first).to_be_visible()
    
    # 6. Проверить статус Celery задачи
    expect(authenticated_page.locator('td:has-text("⏳")')).to_be_visible()
```

**Преимущества Playwright:**
- ✅ Автоматические ожидания (auto-wait)
- ✅ Скриншоты и видео при ошибках
- ✅ Параллельное выполнение тестов
- ✅ Тестирование JavaScript (автообновление)
- ✅ Современный API
...
- [ ] **7.1** Создать User Guide для администраторов
  ...
  **Файл:** `docs/admin-guides/import-from-1c-guide.md`
  
  **Детальная структура:** См. `docs/stories/epic-9/9.8-user-guide-structure.md`
  
  **Содержание User Guide (7 разделов):**
  
  1. **Введение**
     - Назначение системы импорта
     - Что импортируется (товары, цены, остатки, клиенты)
     - Требования (права администратора, файлы CommerceML 3.1)
  
  2. **Типы импорта** (детальное описание каждого)
     - 2.1 Полный каталог (15-30 мин, ~6200 товаров)
     - 2.2 Только остатки (2-5 мин, требует каталог)
     - 2.3 Только цены (2-5 мин, требует каталог)
     - 2.4 Клиенты (1-3 мин)
     - Для каждого: когда использовать, что импортируется, время, зависимости, файлы
  
  3. **Как запустить импорт** (пошаговая инструкция)
     - Шаг 1: Подготовка файлов (выгрузка из 1С, размещение)
     - Шаг 2: Запуск импорта (Django Admin → ИНТЕГРАЦИИ → Импорт из 1С)
     - Шаг 3: Подтверждение запуска (сообщение, редирект)
  
  4. **Отслеживание прогресса**
     - Страница сессий `/admin/integrations/session/`
     - Статусы: ⏳ PENDING → ▶️ STARTED → ✅ SUCCESS / ❌ FAILURE
     - Автообновление каждые 5 секунд
     - Детали сессии (report_details, error_message)
  
  5. **Интерпретация результатов**
     - Успешный импорт (проверка report_details, execution_time)
     - Импорт с ошибками (что делать, куда смотреть)
  
  6. **Troubleshooting** (типичные проблемы и решения)
     - "Сначала импортируйте каталог"
     - "Импорт уже запущен" (Redis lock)
     - "FileNotFoundError: rests.xml"
     - Импорт завис (>1 час)
  ...
  7. **FAQ**
     ...
     - Где посмотреть логи?
     - Что делать если импорт слишком долгий?
     - Можно ли откатить импорт?
  
  **Контакты поддержки:** email, telegram, документация

- [ ] **7.2** Создать Technical Documentation
  
  **Файл:** `docs/architecture/import-refactor-architecture.md`
  
  **Детальная Flow диаграмма:** См. `docs/stories/epic-9/9.8-flow-diagram.md`
  
  **Содержание Technical Documentation:**
  
  1. **Обзор изменений**
     - Разделение UI на две страницы (запуск vs мониторинг)
     - Сохранение всей логики импорта из Epic 3
     - Добавление прогресса импорта в реальном времени
  
  2. **Flow Диаграмма** (детальная, 11 шагов)
     - Пользователь → Форма → Валидация → Redis Lock → ImportSession
     - Celery задача → Management команды → Парсеры → Процессоры
     - Автообновление JavaScript → Обновленный статус
     - ASCII диаграмма с ветвлениями (ошибки, lock занят, успех)
  
  3. **Компоненты системы**
     - **Frontend:** ImportFrom1CAdmin, ImportSessionAdmin, JavaScript автообновление
     - **Backend:** ImportSession model, Celery tasks
     - **Management Commands:** import_catalog_from_1c, import_customers_from_1c
     - **Infrastructure:** Redis (lock + broker), Celery Worker, PostgreSQL
  
  4. **Последовательность действий** (summary)
     - 11 шагов от открытия страницы до отображения результата
  
  5. **Ключевые особенности**
     - Разделение ответственности
     - Асинхронность (не блокирует UI)
     - Надежность (Redis lock, валидация, retry)
     - Мониторинг (автообновление, детальные отчеты)
