...
export const logoutUnauthorizedHandler = http.post(
  "/api/v1/auth/logout/",
  () => {
    return HttpResponse.json(
...
export const logoutNetworkErrorHandler = http.post(
  "/api/v1/auth/logout/",
  () => {
...
describe("authStore.logout()", () => {
  ...
  it("calls authService.logoutFromServer with refresh token", async () => {
    ...
    expect(authService.logoutFromServer).toHaveBeenCalledWith(
      "test-refresh-token",
    );
  });
  ...
  it("clears state even when API fails (fail-safe)", async () => {
    (authService.logoutFromServer as vi.Mock).mockRejectedValue(
      new Error("API Error"),
    );
