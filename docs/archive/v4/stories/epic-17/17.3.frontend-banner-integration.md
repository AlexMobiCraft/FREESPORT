...
export interface Banner {
  ...
  title: string;
  subtitle: string;
  image_url: string;   // Относительный путь `/media/promos/...`
...
Баннерные изображения приходят как `image_url` (относительный путь `/media/promos/...`).
    ...
    vi.mocked(bannersService.getActive).mockResolvedValue([
      {
        ...
        image_url: '/media/promos/test.jpg',
