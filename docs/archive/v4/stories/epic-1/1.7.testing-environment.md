...
- [x] `pytest` и `npm test` выполняются успешно локально и в CI
- [x] Конфигурации `pytest.ini`, `jest.config.js`, `settings/test.py` синхронизированы с документацией
- [x] Тестовая БД создаётся автоматически и очищается после прогонов
- [x] Покрытие тестами >= 70% подтверждено отчётами (pytest-cov, Jest coverage)
- [x] Примеры тестов (backend и frontend) задокументированы в `docs/testing-docker.md`
- [x] CI pipeline включает тестовые шаги и блокирует merge при провале
...
- Примеры именования тестовых файлов: test\__.py (backend), _.test.tsx (frontend)
