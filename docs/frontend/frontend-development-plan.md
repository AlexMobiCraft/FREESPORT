...
- **Каталог товаров (`/catalog/[[...slug]]`)**: Создать страницу с поддержкой вложенных категорий, фильтрами, сортировкой и переключением вида. Фильтр «Бренд» должен первично загружаться через `brandsService.getAll({ has_stock: true })`, а в режиме `in_stock=true` динамически сужаться через `brandsService.getVisibleBrands(filters)` без сброса уже выбранного бренда. Использовать SSR/ISR для SEO.
