...
- [x] Страница `/news` отображает список опубликованных новостей
- [x] Страница `/news/{slug}` отображает детальную новость
- [x] Блок «Новости» на главной отображает 3-4 последние новости
- [x] SEO метаданные корректно настроены
- [x] Responsive дизайн (mobile, tablet, desktop)
- [x] Unit-тесты покрывают ≥70% кода
