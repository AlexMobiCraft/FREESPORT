...
15. [Архитектура интеграции с 1С](#15-архитектура-интеграции-с-1с)

**📋 Дополнительная документация:**
- [API Views Documentation](api-views-documentation.md) - Подробная техническая документация Django ViewSets и endpoints
- [Test Catalog API](test-catalog-api.md) - Структура и организация тестов API
- [Архитектура интеграции с 1С](architecture/20-1c-integration.md) - Подробная архитектура интеграции с 1С:Управление торговлей
- [Запрос к программисту 1С](architecture/request-to-1c-developer.md) - Техническое задание для интеграции с 1С
...
## 15. Архитектура интеграции с 1С

### Обзор интеграции

**FREESPORT Platform** интегрируется с **1С:Управление торговлей** для обеспечения полного цикла обмена данными:

- **Синхронизация товаров:** Загрузка каталога, цен, характеристик
- **Управление остатками:** Актуальная информация о наличии товаров  
- **Синхронизация покупателей:** Двусторонняя синхронизация клиентской базы
- **Обработка заказов:** Экспорт заказов из веб-платформы в 1С
- **Синхронизация статусов:** Обновление статусов заказов из 1С
...
graph TB
    subgraph "1С:Управление торговлей"
        A[Товары и номенклатура] --> B[Экспорт данных]
        C[Остатки товаров] --> B
...
**Management Commands:**
- `import_catalog_from_1c` - импорт товаров и категорий
- `import_offers_from_1c` - импорт цен и остатков  
- `import_customers_from_1c` - импорт клиентской базы
- `sync_customers_with_1c` - двусторонняя синхронизация клиентов
- `export_orders_to_1c` - экспорт заказов в 1С

**Сервисы:**
- `CustomerSyncService` - синхронизация клиентов
- `CustomerIdentityResolver` - идентификация клиентов между системами
- `CustomerConflictResolver` - разрешение конфликтов данных
- `DataParser` - парсинг файлов различных форматов
- `ImportDataValidator` - валидация импортируемых данных

**Модели:**
- Добавлены поля `onec_id`, `sync_status`, `last_sync_at` в модели User и Product
- `CustomerSyncLog` - аудит операций синхронизации
- `ImportLog` - логирование процессов импорта

### Синхронизация покупателей

**Двусторонняя синхронизация клиентов:**
1. **Импорт существующих клиентов из 1С** в платформу
2. **Экспорт новых регистраций** с платформы в 1С  
3. **Синхронизация изменений** данных клиентов
4. **Разрешение конфликтов** автоматически и через ручную модерацию

**Identity Resolution:**
- Идентификация по `email` + `onec_id`
- Нечеткий поиск по телефону + ФИО
- Маппинг ролей пользователей между системами

### Форматы данных

> **ПРИМЕЧАНИЕ:** Детальные форматы данных и схемы маппинга будут определены после получения ответов от программиста 1С на технические вопросы.

**Ожидаемые форматы:**
- CommerceML 2.0 (XML стандарт для 1С)
- JSON через HTTP API
- CSV файлы
- Собственные XML форматы

### Безопасность и производительность

**Безопасность:**
- Валидация и санитизация всех импортируемых данных
- Шифрование чувствительных данных клиентов
- Аудит всех операций синхронизации
- Контроль доступа к management commands

**Производительность:**
- Batch операции для массовой обработки данных
...
- Мониторинг очередей экспорта

### Техническая документация

Подробная техническая документация по интеграции с 1С:

- **[Архитектура интеграции с 1С](architecture/20-1c-integration.md)** - Полная техническая спецификация
- **[Запрос к программисту 1С](architecture/request-to-1c-developer.md)** - Технические вопросы для согласования форматов

---

## История изменений

### 2025-09-05 - Добавлена архитектура интеграции с 1С
- **Добавлен раздел 15: Архитектура интеграции с 1С** - полное описание интеграции с 1С:Управление торговлей
- **Синхронизация покупателей** - двусторонняя синхронизация клиентской базы с разрешением конфликтов
- **Management Commands** - команды для импорта товаров, клиентов, цен и остатков
- **Identity Resolution** - надежная идентификация клиентов между системами
- **Создан запрос к программисту 1С** - техническое задание для согласования форматов данных
- **Обновлена архитектурная документация** - добавлены ссылки на новые документы
