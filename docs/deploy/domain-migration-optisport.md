...
`ip4:31.31.197.15` в SPF **не является мусором от парковки**, хотя совпадает с бывшим
адресом парковочной страницы: это действующая A-запись `mail.optisport.ru`, и внутрь
`include:_spf.hosting.reg.ru` она не входит. Убирать её можно только после того, как по
заголовкам реально отправленного письма (`Received`, `Return-Path`, `Authentication-Results`)
подтверждено, что исходящий узел в include уже покрыт. Проще всего — командой
`test_email` из п. 6 и просмотром оригинала письма в почтовом клиенте.

Целевое значение `_dmarc` — добавляется адрес для агрегированных отчётов:

```
v=DMARC1; p=none; sp=none; adkim=r; aspf=r; rua=mailto:dmarc@optisport.ru; fo=1
```

Ящик `dmarc@optisport.ru` должен существовать: отчёты приходят ежедневно и в виде
XML-вложений. Адрес в **чужом** домене (например, на `gmail.com`) не годится — приём
отчётов за другой домен требует разрешающей записи вида
`optisport.ru._report._dmarc.<домен-получателя>`, которой у публичных почтовиков нет,
и отчёты будут молча отбрасываться.
