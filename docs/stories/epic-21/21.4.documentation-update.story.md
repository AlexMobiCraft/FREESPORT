...
- [x] **Task 1: Обновить source-tree.md** (AC: 1)
  - [x] Добавить модель `BlogPost` в секцию `backend/apps/common/` (рядом с `News`)
  - [x] Добавить `blogService.ts` в секцию `frontend/src/services/`
  - [x] Добавить страницы `/blog` и `/blog/[slug]` в секцию `frontend/src/app/`
  - [x] Сохранить консистентность с существующим форматированием

- [x] **Task 2: Обновить 03-api-specification.md** (AC: 2)
  - [x] Добавить секцию `# Blog API` с endpoints:
    - `GET /api/v1/blog/` — список статей с пагинацией
    - `GET /api/v1/blog/{slug}/` — детальная статья
  - [x] Добавить схемы `BlogPostList` и `BlogPostDetail` в components/schemas
  - [x] Использовать паттерн существующих News endpoints как референс

- [x] **Task 3: Обновить 02-data-models.md** (AC: 3)
  - [x] Добавить секцию `### BlogPost Model` после секции `News`
  - [x] Описать поля модели: title, slug, subtitle, excerpt, content, image, author, category, is_published, published_at, meta_title, meta_description, created_at, updated_at
  - [x] Добавить модель в ER-диаграмму (mermaid)

- [x] **Task 4: Обновить GEMINI.md** (AC: 4)
  - [x] В секции `## **Django App Structure**` добавить строку:
    - `- apps/common/: Общие утилиты, аудит, а также управление контентом: новости, блог и подписки на рассылку.`
  - [x] Обновить существующее описание apps/common/ чтобы включить Blog

- [x] **Task 5: Обновить CLAUDE.md** (AC: 5)
  - [x] В секции `### **Структура приложений Django**` обновить apps/common/:
...
- [x] **Task 6: Верификация изменений** (AC: 6)
  - [x] Проверить что все обновления соответствуют фактической реализации из Stories 21.1–21.3
  - [x] Проверить корректность ссылок и форматирования
  - [x] Запустить линтинг markdown если доступен
...
- All requested documentation files updated for Blog module.
- Blog API schemas and endpoints added to `03-api-specification.md`.
- BlogPost model documented in `02-data-models.md` with ER diagram update.
- `PROJECT_PROGRESS.md`, `GEMINI.md`, and `CLAUDE.md` updated.
