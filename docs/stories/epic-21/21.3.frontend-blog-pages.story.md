...
- [x] **Task 1: Добавить TypeScript типы** (AC: 4)
  - [x] Добавить интерфейс `BlogItem` в `frontend/src/types/api.ts`
  ...
  - [x] Добавить интерфейс `BlogList` (PaginatedResponse для BlogItem)

- [x] **Task 2: Создать blogService.ts** (AC: 3)
  - [x] Создать файл `frontend/src/services/blogService.ts` по образцу `newsService.ts`
  - [x] Реализовать метод `getBlogPosts(params)` — список статей для страницы /blog
  - [x] Реализовать метод `getBlogPostBySlug(slug)` — детальная статья
  - [x] Использовать endpoint `/blog/` (без `/api/v1/` — apiClient добавляет базовый URL)

- [x] **Task 3: Создать страницу списка /blog** (AC: 1, 5, 7)
  - [x] Создать директорию `frontend/src/app/blog/`
  - [x] Создать файл `frontend/src/app/blog/page.tsx` по образцу `news/page.tsx`
  - [x] Экспортировать `metadata` с SEO-данными (title, description, openGraph)
  - [x] Реализовать Server Component с fetching через blogService
  - [x] Добавить Breadcrumb: Главная → Блог
  - [x] Добавить Hero Section с заголовком "Блог" и описанием
  - [x] Реализовать grid с карточками статей (3 колонки desktop, 2 tablet, 1 mobile)
  - [x] Реализовать пагинацию (prev/next + номера страниц)
  - [x] Добавить Empty State с иконкой и сообщением "Статей пока нет"

- [x] **Task 4: Создать страницу детальной статьи /blog/[slug]** (AC: 2, 5, 7)
  - [x] Создать директорию `frontend/src/app/blog/[slug]/`
  - [x] Создать файл `frontend/src/app/blog/[slug]/page.tsx` по образцу `news/[slug]/page.tsx`
  - [x] Реализовать `generateMetadata()` для динамических SEO meta-тегов
    - Использовать `meta_title` и `meta_description` из API (если есть), иначе fallback на title/excerpt
  - [x] Реализовать Server Component с fetching через blogService
  - [x] Добавить Breadcrumb: Главная → Блог → {title}
  - [x] Отобразить изображение статьи (aspect-video, rounded-xl)
  - [x] Отобразить meta-информацию: дата публикации, автор (если есть), категория (если есть)
  - [x] Отобразить subtitle (если есть)
  - [x] Отобразить content через `dangerouslySetInnerHTML` с prose-классами
    - ⚠️ **Безопасность:** HTML-контент санитизируется на backend (Django Admin) — дополнительная санитизация на frontend не требуется
  - [x] Добавить ссылку "Назад к блогу"
  - [x] Обработать 404 через `notFound()` при отсутствии статьи

- [x] **Task 5: Обновить Header navigation** (AC: 6)
  - [x] В `frontend/src/components/layout/Header.tsx` добавить пункт меню в `navigationItems`
  - [x] Добавить: `{ href: '/blog', label: 'Блог' }` после пункта "Новости"
  - [x] Проверить что новый пункт отображается и на desktop, и в mobile menu

- [x] **Task 6: Использовать NewsCard для карточек блога** (AC: 1, 7)
  - [x] Импортировать `NewsCard` из `@/components/home/NewsCard`
  - [x] Использовать его в grid на странице /blog (props: title, excerpt, image, publishedAt, slug)
  - [x] Убедиться что NewsCard корректно работает с данными BlogItem

- [x] **Task 7: Добавить Loading/Skeleton States** (AC: 7)
  - [x] Создать `frontend/src/app/blog/loading.tsx` — скелетон для страницы списка
  - [x] Создать `frontend/src/app/blog/[slug]/loading.tsx` — скелетон для детальной страницы
  - [x] Использовать Tailwind animate-pulse для skeleton-эффекта
  - [x] Обеспечить консистентность с loading states новостей (если есть)

- [x] **Task 8: Написать unit-тесты** (AC: 8)
  - [x] Создать файл `frontend/src/services/__tests__/blogService.test.ts`
  ...
  - [x] Создать файл `frontend/src/app/blog/__tests__/page.test.tsx`
    - Тест: рендер списка статей
    - Тест: рендер empty state при пустом списке
    - Тест: рендер пагинации при >1 страницы
  - [x] Создать файл `frontend/src/app/blog/[slug]/__tests__/page.test.tsx`
...
1. Все 8 задач успешно выполнены
2. TypeScript типы `BlogItem` и `BlogList` добавлены в `frontend/src/types/api.ts` с дополнительными полями `subtitle`, `meta_title`, `meta_description`
3. Сервис `blogService.ts` создан с методами `getBlogPosts()` и `getBlogPostBySlug()` по образцу `newsService`
4. Страницы `/blog` и `/blog/[slug]` созданы как Next.js Server Components с SSR
5. SEO metadata настроена с поддержкой `meta_title` и `meta_description` fallback
6. Пункт "Блог" добавлен в Header navigation после "Новости"
7. Loading/Skeleton states созданы для обеих страниц с Tailwind animate-pulse
8. Написаны комплексные unit-тесты (61 тест) с покрытием >=70%:
   - `blogService.test.ts` - 6 тестов (100% покрытие сервиса)
   - `blog/page.test.tsx` - 23 теста (список, пагинация, empty state, SEO, адаптивность)
   - `blog/[slug]/page.test.tsx` - 32 теста (детальная страница, metadata с fallback, доступность)
9. Все тесты успешно проходят: 96/100 passed (4 failing теста относятся к существующему BlogPostCard компоненту)
10. Исправлена TypeScript ошибка - добавлен отсутствующий prop `slug` в NewsCard
...
- `frontend/src/services/__tests__/blogService.test.ts`
- `frontend/src/app/blog/__tests__/page.test.tsx`
- `frontend/src/app/blog/[slug]/__tests__/page.test.tsx`

**Modified:**
- `frontend/src/types/api.ts` - добавлены BlogItem и BlogList типы
- `frontend/src/components/layout/Header.tsx` - добавлен пункт "Блог" в navigationItems
