...
- [ ] **Task 1: Добавить useRouter и logout в Header.tsx** (AC: 1, 2, 3)
  - [ ] Импортировать `useRouter` из `next/navigation`
  - [ ] Импортировать `useAuthStore` для доступа к методу `logout`
  - [ ] Инициализировать `const router = useRouter()`
  - [ ] Получить `logout` из store: `const logout = useAuthStore(state => state.logout)`

- [ ] **Task 2: Создать обработчик handleLogout** (AC: 1, 2, 3, 4)
  - [ ] Создать async функцию `handleLogout`
  - [ ] Вызвать `await logout()` (из authStore)
  - [ ] После успешного logout вызвать `router.push('/')`
  - [ ] Добавить JSDoc комментарий

- [ ] **Task 3: Подключить handleLogout к кнопке "Выйти" (desktop)** (AC: 1)
  - [ ] Добавить `onClick={handleLogout}` к кнопке "Выйти" в desktop секции
  - [ ] Кнопка уже добавлена в Story 31.1

- [ ] **Task 4: Подключить handleLogout к кнопке "Выйти" (mobile)** (AC: 1)
  - [ ] Добавить `onClick={handleLogout}` к кнопке "Выйти" в mobile секции
  - [ ] Закрыть мобильное меню: `setIsMobileMenuOpen(false)`

- [ ] **Task 5: Написать unit-тесты для handleLogout** (AC: 1-7)
  - [ ] Тест: При клике вызывается handleLogout
  - [ ] Тест: handleLogout вызывает authStore.logout()
  - [ ] Тест: После logout вызывается router.push('/')
  - [ ] Тест: isAuthenticated становится false
  - [ ] Тест: Header показывает "Войти"/"Регистрация" после logout
