...
- [ ] **Task 1: Добавить метод `logout(refreshToken: string)` в authService.ts** (AC: 1, 2, 3, 4, 7)
  - [ ] Добавить async метод `logoutFromServer(refreshToken: string): Promise<void>`
  - [ ] Выполнять POST запрос на `/auth/logout/` с `{ refresh: refreshToken }`
  - [ ] При успехе (204) - resolved Promise
  - [ ] При ошибке (400/401/network) - rejected Promise с error message
  - [ ] Добавить JSDoc комментарий с описанием метода

- [ ] **Task 2: Обновить метод `logout()` в authStore.ts** (AC: 5, 6)
  - [ ] Сделать метод `logout()` асинхронным (`async logout(): Promise<void>`)
  - [ ] Получить refresh token перед очисткой: `localStorage.getItem('refreshToken')`
  - [ ] Вызвать `authService.logoutFromServer(refreshToken)` в try/catch
  - [ ] При ошибке API - логировать и продолжать локальную очистку (fail-safe)
  - [ ] Локальная очистка ВСЕГДА выполняется (существующий код)

- [ ] **Task 3: Обновить типизацию для logout API** (AC: 7)
  - [ ] Добавить тип `LogoutRequest` в `types/api.ts` (если требуется)
  - [ ] Убедиться, что все методы имеют корректную типизацию

- [ ] **Task 4: Написать unit-тесты для logout интеграции** (AC: 1-7)
  - [ ] Тест: `authService.logoutFromServer()` отправляет POST на `/auth/logout/`
  - [ ] Тест: Request body содержит `{ refresh: token }`
  - [ ] Тест: При 204 - resolved Promise
  - [ ] Тест: При 400 - rejected Promise
  - [ ] Тест: `authStore.logout()` вызывает `authService.logoutFromServer()`
  - [ ] Тест: При ошибке API локальная очистка все равно происходит
