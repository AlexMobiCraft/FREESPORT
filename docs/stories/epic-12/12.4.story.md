...
- [ ] Task 5: Реализация Badge компонентов (AC: 4)
  - [ ] 4.1 Использовать обновленный Badge из Story 12.2:
    - Варианты: sale, new, hit
    - Убедиться что цвета соответствуют design-system.json
  - [ ] 4.2 Позиционирование в grid layout:
    - absolute top-2 left-2
    - z-10 для наложения на изображение
  - [ ] 4.3 Позиционирование в list layout:
    - inline рядом с названием или ценой
    - ml-2 для отступа
  - [ ] 4.4 Логика отображения badge:
    - SALE: если есть old_price и скидка > 0
    - NEW: если product.is_new === true
    - HIT: если product.is_hit === true

- [ ] Task 5: Реализация кнопки "В избранное" (AC: 4)
  - [ ] 5.1 Создать компонент FavoriteButton или встроить в ProductCard:
  ...
  - [ ] 5.3 Позиционирование:
    ...
    - z-10
    - p-2 для увеличения области клика
    - rounded-full bg-white/80 backdrop-blur-sm
  - [ ] 5.4 Accessibility:
    - aria-label="Добавить в избранное" / "Удалить из избранного"
    - role="button"
    - Keyboard accessible (Enter/Space)
  - [ ] 5.5 Обработчик onClick:
    - Вызывать onToggleFavorite(product.id)
    - Prevent event bubbling

- [ ] Task 6: Реализация ролевого ценообразования (AC: 5)
  - [ ] 6.1 Создать утилиту для определения цены по роли:
    ```typescript
    ...
    ```
  - [ ] 6.2 Реализовать отображение цены:
    - Основная цена: text-title-m, font-semibold, text-text-primary
    - Старая цена (если есть скидка): text-body-s, text-text-muted, line-through, mr-2
  - [ ] 6.3 Реализовать отображение RRP/MSRP для B2B:
    - Условие: showRRP && product.recommended_retail_price
    - Текст: "РРЦ: {price}" text-body-s, text-text-secondary
    - Аналогично для MSRP
  - [ ] 6.4 Расчет и отображение скидки:
    - Процент: Math.round((1 - currentPrice / oldPrice) * 100)
    - Отображать в Badge variant="sale"

- [ ] Task 7: Реализация адаптивных стилей (AC: 6)
  - [ ] 7.1 Создать компонент ProductGrid для обертки карточек:
    - grid gap-4
...
- [ ] Task 9: Создание и обновление тестов (AC: 8)
  - [ ] 9.1 Создать файл тестов `frontend/src/components/business/ProductCard/__tests__/ProductCard.test.tsx`
  - [ ] 9.2 Тесты рендеринга:
    - Рендеринг grid layout
    - Рендеринг list layout
    - Отображение всех элементов (изображение, бренд, название, цена)
  - [ ] 9.3 Тесты ролевого ценообразования:
  ...
  - [ ] 9.5 Тесты интерактивности:
    - Клик на "В корзину" вызывает onAddToCart
    - Клик на избранное вызывает onToggleFavorite
    - Hover эффекты применяются (visual regression)
  - [ ] 9.6 Тесты accessibility:
    - aria-label присутствует на кнопках
    - alt текст на изображениях
    - Keyboard navigation работает
  - [ ] 9.7 Запустить тесты и проверить покрытие >= 80%

- [ ] Task 10: Документация и интеграция (AC: 9)
  - [ ] 10.1 Обновить JSDoc комментарии
  - [ ] 10.2 Создать примеры использования в комментариях
  - [ ] 10.3 Добавить экспорт в `frontend/src/components/business/index.ts`
  - [ ] 10.4 Визуальная проверка в dev режиме:
    - Создать тестовую страницу с примерами всех вариантов
    - Проверить grid и list layouts
    - Проверить все варианты badge
    - Проверить адаптивность
  - [ ] 10.5 Проверить что компонент не ломает существующие страницы
