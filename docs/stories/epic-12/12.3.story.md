...
- [ ] Task 1: Анализ текущего состояния компонентов (AC: 1, 2, 3, 4)
  - [ ] 1.1 Изучить текущую реализацию Breadcrumb в `frontend/src/components/ui/Breadcrumb/Breadcrumb.tsx`
  - [ ] 1.2 Изучить текущую реализацию Tabs в `frontend/src/components/ui/Tabs/Tabs.tsx`
  - [ ] 1.3 Изучить текущую реализацию Header в `frontend/src/components/layout/Header.tsx`
  - [ ] 1.4 Проверить отсутствие компонента Pagination (требуется создание)
  - [ ] 1.5 Сравнить с требованиями `docs/front-end-spec.md` и `docs/frontend/design-system.json`
  - [ ] 1.6 Составить список конкретных изменений для каждого компонента

- [ ] Task 2: Обновление компонента Breadcrumb (AC: 1)
  - [ ] 2.1 Обновить цвета текста:
    - Ссылки: text-neutral-700 (#5E5E5E)
    - Последний элемент: text-text-primary (#1B1B1B)
    - Hover: text-primary (#1F1F1F)
  - [ ] 2.2 Обновить разделитель ChevronRight:
    - Цвет: text-neutral-600 (#7D7D7D)
  - [ ] 2.3 Проверить spacing gap-2 (8px)
  - [ ] 2.4 Обновить transition на transition-colors duration-short
  - [ ] 2.5 Проверить edge cases (обрезка текста, collapse при >5 элементов)

- [ ] Task 3: Создание компонента Pagination (AC: 2)
  - [ ] 3.1 Создать новый файл `frontend/src/components/ui/Pagination/Pagination.tsx`
  - [ ] 3.2 Реализовать props interface:
    ```typescript
  ...
  - [ ] 3.4 Реализовать логику отображения страниц:
  ...
  - [ ] 3.5 Добавить accessibility:
    - aria-label для навигации
    - aria-current="page" для активной страницы
    - Disabled состояние для границ
  - [ ] 3.6 Добавить экспорт в `frontend/src/components/ui/index.ts`

- [ ] Task 4: Обновление компонента Tabs (AC: 3)
  - [ ] 4.1 Обновить цвета табов:
    - Активный: text-primary (#1B1B1B) - было text-primary (возможно синий)
    - Неактивный: text-text-secondary (#4D4D4D)
    - Hover: text-primary
  - [ ] 4.2 Обновить underline индикатор:
    - Цвет: bg-primary (#1F1F1F)
    - Высота: 3px
    - Border-radius: rounded-full
  - [ ] 4.3 Обновить spacing:
    - gap-6 (24px) между табами
  - [ ] 4.4 Обновить focus ring:
    - ring-2 ring-primary ring-offset-2
  - [ ] 4.5 Проверить keyboard navigation работает корректно
  - [ ] 4.6 Обновить transition на duration-short (120ms)

- [ ] Task 5: Обновление компонента Header (AC: 4)
  - [ ] 5.1 Обновить контейнер Header:
    - Высота: h-[60px]
    - Shadow: shadow-[0_6px_16px_rgba(31,42,68,0.05)]
    - Background: bg-white (#FFFFFF)
    - Убрать border-b (только shadow)
  - [ ] 5.2 Обновить логотип:
    - Типографика: text-title-m (20px), font-bold
    - Цвет: text-text-primary (#1B1B1B) - **НЕ синий**
    - B2B badge: bg-[#F9E1E1], text-[#A63232]
  - [ ] 5.3 Обновить навигацию:
    - Типографика: text-body-m
    - Цвет: text-text-primary
    - Hover: text-text-secondary
    - Active: без border-b, только цвет или другой индикатор
    - Spacing: gap-6 (24px)
  - [ ] 5.4 Обновить иконки действий:
    - Size: 24px (w-6 h-6)
    - Цвет: text-text-primary
    - Gap: gap-4 (16px)
    - Hover: text-text-secondary
  - [ ] 5.5 Добавить Cart Badge:
    - bg-[#F9E1E1], text-[#A63232]
    - radius: rounded-full
    - font-size: text-[10px], font-bold
    - Position: absolute top-0 right-0
  - [ ] 5.6 Обновить кнопки (использовать обновленный Button из Story 12.2):
    - Primary: bg-primary
    - Outline: border-primary, text-primary
  - [ ] 5.7 Обновить мобильное меню под новую палитру
  - [ ] 5.8 Использовать lucide-react иконки вместо inline SVG

- [ ] Task 6: Проверка accessibility (AC: 5)
  - [ ] 6.1 Проверить контраст для всех текстовых элементов
  - [ ] 6.2 Проверить focus ring на всех интерактивных элементах
  - [ ] 6.3 Проверить aria-* атрибуты:
  ...
  - [ ] 6.4 Проверить keyboard navigation в Tabs и Pagination

- [ ] Task 7: Обновление тестов (AC: 6)
  - [ ] 7.1 Обновить тесты Breadcrumb в `frontend/src/components/ui/Breadcrumb/__tests__/Breadcrumb.test.tsx`
    - Тестировать цвета и стили
    - Тестировать edge cases
  - [ ] 7.2 Создать тесты Pagination в `frontend/src/components/ui/Pagination/__tests__/Pagination.test.tsx`
  ...
  - [ ] 7.3 Обновить тесты Tabs в `frontend/src/components/ui/Tabs/__tests__/Tabs.test.tsx`
    - Тестировать новые стили
    - Тестировать keyboard navigation
  - [ ] 7.4 Создать/обновить тесты Header
    - Тестировать рендеринг для authenticated/unauthenticated
    - Тестировать B2B/B2C разделение
    - Тестировать мобильное меню
  - [ ] 7.5 Запустить тесты и убедиться в покрытии >= 80%

- [ ] Task 8: Документация и финальная проверка (AC: 7)
  - [ ] 8.1 Обновить JSDoc комментарии во всех компонентах
  - [ ] 8.2 Обновить ссылки на design-system.json
  - [ ] 8.3 Визуальная проверка в dev режиме
  - [ ] 8.4 Проверить что компоненты не ломают существующие страницы
  - [ ] 8.5 Проверить адаптивность (mobile/tablet/desktop)
