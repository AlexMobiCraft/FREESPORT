...
- [ ] **Task 1: Верификация текущего состояния** (AC: 1, 2)
  - [ ] 1.1 Проверить что `_import_base_images()` в variant_import.py использует `normalize_image_path()` ✅ (L505)
  - [ ] 1.2 Проверить что `_import_variant_images()` в variant_import.py использует `normalize_image_path()` ✅ (L769)
  - [ ] 1.3 Документировать текущее состояние

- [ ] **Task 2: Анализ legacy метода** (AC: 3)
  - [ ] 2.1 Изучить `_import_base_images_simple()` в processor.py (L617-700)
  - [ ] 2.2 Определить все места вызова этого метода
  - [ ] 2.3 Проверить что legacy метод НЕ использует `normalize_image_path()`

- [ ] **Task 3: Добавить deprecation warning в legacy метод** (AC: 3)
  - [ ] 3.1 Добавить `warnings.warn()` с `DeprecationWarning` в начало `_import_base_images_simple()`
  - [ ] 3.2 Добавить docstring note о deprecated статусе
  - [ ] 3.3 Указать альтернативу: `VariantImportProcessor._import_base_images()`

- [ ] **Task 4: Unit-тесты для normalize_image_path()** (AC: 4)
  - [ ] 4.1 Тест: путь с `import_files/` prefix → возвращает путь без prefix
  - [ ] 4.2 Тест: путь без `import_files/` prefix → возвращает путь без изменений
  - [ ] 4.3 Тест: edge cases (пустая строка, только файл, глубокий путь)

- [ ] **Task 5: Integration тесты импорта изображений** (AC: 4)
  - [ ] 5.1 Тест: импорт изображения с путём `import_files/xx/file.jpg`
  - [ ] 5.2 Тест: импорт изображения с путём `xx/file.jpg`
  - [ ] 5.3 Тест: оба формата путей → одинаковый результат в media storage
...
- [ ] Подтверждено что `_import_base_images()` использует `normalize_image_path()` ✅
- [ ] Подтверждено что `_import_variant_images()` использует `normalize_image_path()` ✅  
- [ ] Legacy метод `_import_base_images_simple()` помечен как deprecated
- [ ] Unit-тесты для `normalize_image_path()` написаны и проходят
- [ ] Integration тесты для обоих форматов путей написаны и проходят
- [ ] Покрытие тестами >= 90%
