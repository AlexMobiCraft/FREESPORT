...
- [x] **Task 1: Анализ текущего состояния `_execute_import_type()`** (AC: 1, 2)
  - [x] 1.1 Изучить файл `backend/apps/integrations/tasks.py`
  - [x] 1.2 Документировать текущие вызовы legacy команды
  - [x] 1.3 Проверить параметры передаваемые в команды

- [x] **Task 2: Обновить вызов для типа "catalog"** (AC: 1, 2)
  - [x] 2.1 Заменить `import_catalog_from_1c` на `import_products_from_1c` (L115-119)
  - [x] 2.2 Убедиться что `--file-type=all` поддерживается новой командой
  - [x] 2.3 Сохранить передачу `--celery-task-id` параметра
  - [x] 2.4 Обновить log сообщение (L114)

- [x] **Task 3: Обновить вызов для типа "stocks"** (AC: 1, 2)
  - [x] 3.1 Заменить `import_catalog_from_1c` на `import_products_from_1c` (L132-136)
  - [x] 3.2 Изменить `--file-type=rests` на соответствующий параметр новой команды
  - [x] 3.3 Обновить log сообщение (L131)

- [x] **Task 4: Обновить вызов для типа "prices"** (AC: 1, 2)
  - [x] 4.1 Заменить `import_catalog_from_1c` на `import_products_from_1c` (L143-147)
  - [x] 4.2 Изменить `--file-type=prices` на соответствующий параметр новой команды
  - [x] 4.3 Обновить log сообщение (L140-141)

- [x] **Task 5: Проверить совместимость параметров** (AC: 2)
  - [x] 5.1 Изучить `import_products_from_1c.py` для проверки поддерживаемых параметров
  - [x] 5.2 Убедиться что `--celery-task-id` работает в новой команде
  - [x] 5.3 Проверить поддержку `--file-type` или альтернативных параметров

- [x] **Task 6: Интеграционный тест через Admin UI** (AC: 3, 4)
  - [x] 6.1 Создать/обновить тест `test_admin_import_catalog_type`
  - [x] 6.2 Создать/обновить тест `test_admin_import_stocks_type`
  - [x] 6.3 Создать/обновить тест `test_admin_import_prices_type`
  - [x] 6.4 Добавить тест `test_celery_task_id_passed_correctly`
...
- [x] `_execute_import_type("catalog")` вызывает `import_products_from_1c`
- [x] `_execute_import_type("stocks")` вызывает `import_products_from_1c`
- [x] `_execute_import_type("prices")` вызывает `import_products_from_1c`
- [x] `--celery-task-id` передаётся корректно во всех вызовах
- [x] Интеграционные тесты написаны и проходят
- [ ] Admin UI позволяет запускать все типы импорта (требует ручной проверки)
- [ ] Production smoke test пройден (требует ручной проверки)
...
| 2025-12-09 | 1.1 | Implementation complete | James (Dev) |
...
- Заменены все вызовы `import_catalog_from_1c` на `import_products_from_1c` для типов catalog, stocks, prices в `_execute_import_type()`
- Команда `import_products_from_1c` уже поддерживает параметры `--file-type` (goods/offers/prices/rests/all) и `--celery-task-id`
- Интеграционные тесты обновлены для проверки новой команды с `--celery-task-id`
- Все тесты проходят (Exit code: 0)

### File List

- `backend/apps/integrations/tasks.py` — обновлён для использования `import_products_from_1c`
- `backend/tests/integration/test_async_import_tasks.py` — обновлены тесты для новой команды
