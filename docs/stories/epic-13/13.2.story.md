...
> **БЛОКИРУЮЩАЯ ЗАВИСИМОСТЬ:** Story может стартовать только после перевода Story 13.1 в состояние **Done** и применения всех миграций (создание `Brand1CMapping`, добавление `Brand.normalized_name`, удаление `Brand.onec_id`).
...
> **Зависимость:** Эта история исполняется только после успешного внедрения Story 13.1 (Brand1CMapping + `Brand.normalized_name`), чтобы гарантировать наличие master-брендов и корректного маппинга. На момент создания истории Story 13.1 находится в статусе Draft и ещё не реализована.
...
- [ ] **Task 1: Обновить модель Product** (AC: 1, 2)
  - [ ] В `backend/apps/products/models.py` добавить поле `onec_brand_id = models.CharField("ID бренда в 1С", max_length=100, null=True, blank=True, db_index=True)` рядом с существующими 1С-метаданными, соблюдая импорт `from __future__ import annotations` и типизацию. [Source: architecture/02-data-models.md#модели-каталога-товаров]
  - [ ] В `backend/apps/products/services/processor.py` добавить логику присваивания `product.onec_brand_id = brand_id_from_xml` при создании/обновлении товара. Не создавать отдельную helper-функцию — достаточно прямого присваивания с проверкой на `None`. [Source: architecture/20-1c-integration.md#4-схемы-маппинга-данных]
  - [ ] Проверить публичные serializers (`ProductListSerializer`, `ProductDetailSerializer` в `apps/products/serializers.py`), убедившись, что `onec_brand_id` не включено в `Meta.fields`. Для внутренних 1C API (если потребуется) создать отдельный serializer. Добавить smoke-тест в `backend/tests/unit/test_serializers/test_product_serializer.py`, подтверждающий отсутствие поля в публичных ответах. [Source: architecture/03-api-specification.md#product-catalog-endpoints]

- [ ] **Task 2: Создать и применить миграцию** (AC: 1, 4)
  - [ ] Выполнить `docker compose run --rm backend python manage.py makemigrations products -n add_onec_brand_id_to_product` для генерации миграции с nullable CharField(max_length=100), индексом и verbose name. [Source: architecture/09-database-schema.md]
  - [ ] **Миграция данных НЕ требуется:** Согласно Epic 13, данные будут очищены и загружены заново из 1С после внедрения дедупликации брендов. Поле `onec_brand_id` для существующих товаров останется `NULL` до следующего импорта. RunPython-миграция не создаётся. [Source: epics/epic-13/epic-13.brand-deduplication.md#миграция-существующих-данных]
  - [ ] Запустить `docker compose run --rm backend python manage.py migrate products` и задокументировать версию миграции в Change Log этой истории (секция внизу документа).

- [ ] **Task 3: Обновить импорт из 1С** (AC: 2)
  - [ ] В `backend/apps/products/services/parser.py` (если файл не существует — создать) добавить извлечение `onec_brand_id` из CommerceML XML (`<Производитель><Ид>` или соответствующий элемент). [Source: architecture/20-1c-integration.md#6-воркфлоу-импорта-каталога]
  - [ ] В `backend/apps/products/services/processor.py` добавить присваивание `product.onec_brand_id = parsed_brand_id` при создании/обновлении товара (внутри метода, обрабатывающего товары из CommerceML). Если `parsed_brand_id` отсутствует в XML — оставить поле `None`, не затирая существующее значение.
  - [ ] Обработать кейс отсутствия ID: не затирать существующее значение, логировать предупреждение уровня `logger.warning(f"Product {product.onec_id}: onec_brand_id not provided in CommerceML")` в файл импорта.
  - [ ] Обновить импортные логи: в `processor.py` добавить статистику `onec_brand_ids_set` (счётчик успешно заполненных полей) и вывести в финальный отчёт команды. Документировать изменения в `scripts/inport_from_1C/README_SERVER_SYNC.md`, добавив описание нового поля и его назначения.

- [ ] **Task 4: Django Admin и внутренние инструменты** (AC: 3)
  - [ ] В `backend/apps/products/admin.py` включить `onec_brand_id` в `readonly_fields`, `search_fields`, возможный `list_display` и отдельный `fieldsets` блок "1С данные". [Source: architecture/source-tree.md#backend]
  - [ ] Добавить фильтр/поиск по `onec_brand_id` только для чтения; убедиться, что через админку нельзя отредактировать поле вручную.

- [ ] **Task 5: Testing & QA** (AC: 1-4)
  - [ ] **AC1 (Модель):** В `backend/tests/unit/test_models/test_product_model.py` добавить тест создания Product с `onec_brand_id`, проверку nullable, db_index и max_length=100.
  - [ ] **AC2 (Импорт):** В `backend/tests/unit/test_services/test_import_product_sync.py` добавить тесты парсинга CommerceML, убеждающиеся, что `onec_brand_id` берётся из XML и не перезаписывается пустым значением (idempotency).
  - [ ] **AC3 (Admin):** Добавить Django Admin smoke-тест в `backend/tests/unit/test_admin/test_products_admin.py` с использованием `admin_client`, удостоверившись, что поле отображается как read-only, доступно в search_fields и list_display.
  - [ ] **AC4 (Миграция):** Тест миграции НЕ требуется, так как RunPython-миграция не создаётся. Достаточно проверить, что миграция применяется без ошибок через стандартный тест Django migrations.
  - [ ] **API Guardrail:** В `backend/tests/unit/test_serializers/test_product_serializer.py` добавить проверку, что `ProductListSerializer` и `ProductDetailSerializer` не возвращают `onec_brand_id` в публичных ответах.
  - [ ] Выполнить выборочную проверку логов импортных сессий после внедрения, подтверждая наличие статистики `onec_brand_ids_set` в отчёте команды.

## Dev Notes

### Previous Story Insights

- **Жёсткая зависимость:** Начинать Story 13.2 можно только после применения миграций и кода Story 13.1 (созданы `Brand1CMapping`, добавлено `Brand.normalized_name`, удалён `Brand.onec_id`). На момент создания Story 13.2 Story 13.1 находится в статусе Draft и ещё не реализована. [Source: epics/epic-13/epic-13.brand-deduplication.md#story-131]
...
- `docs/architecture/source-tree.md#backend` — подтверждает расположение `apps/products` и связанных сервисов.
...
|---|---|---|
| `models.py` | Modified | Добавлено поле `onec_brand_id` |
| `migrations/00xx_*.py` | Created | Миграция для нового поля |
| `admin.py` | Modified | Отображение в админке |
| ... | ... | ... |

## QA Results

QA Agent заполняет результаты проверки после завершения истории.
