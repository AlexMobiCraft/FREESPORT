...
- [ ] Обработка состояний (AC: 7)
  - [ ] Проверка `product.is_available` для каждого товара
  - [ ] Отображение бейджа "Нет в наличии" если `is_available === false`
  - [ ] Disabled state для кнопки "В корзину" если товар недоступен
...
**Product Model (для отображения в избранном):** [Source: docs/architecture/02-data-models.md]
...
interface Product {
  ...
  is_available: boolean;         // Computed: is_active && available_quantity > 0
  is_active: boolean;

  // Ценообразование
  retail_price: string;          // Decimal
  current_user_price?: string;   // Цена для текущего пользователя

  // Изображения
  base_images?: string[];        // Массив URL изображений
  variants?: ProductVariant[];   // Варианты товара с изображениями
}
```
...
2. Проверить `product.is_available`
3. Если доступен:
   - Вызвать `cartStore.addItem(productId, 1)`
   - Toast success "Товар добавлен в корзину"
...
Для каждого товара в избранном нужно проверять `is_available`:
- Загрузить полную информацию о товаре через `GET /api/v1/products/{id}/`
- Или включить поле `is_available` в ответ `/api/v1/favorites/`
- Отображать badge "Нет в наличии" если `is_available === false`
