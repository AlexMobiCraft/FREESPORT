...
3. **Все существующие тесты запускаются** под Vitest:
   - 9 store тестов (authStore, cartStore)
   - 16 UI component тестов
   - 3 environment тестов
   - 34 service тестов (auth, products, cart, categories, orders)
...
- [ ] **Task 8: Обновление документации** (AC: 7)
  ...
  - [ ] Создать migration guide (опционально) для других разработчиков

- [ ] **Task 9: Финальная проверка и валидация** (AC: 3, 4, 9)
  - [ ] Запустить все тесты локально: `npm test`
  - [ ] Проверить coverage report: `npm run test:coverage`
  - [ ] Убедиться что все 62 теста проходят (9 store + 16 UI + 3 env + 34 service)
...
**Current Test Status**:
- ✅ 9 store тестов проходят (authStore, cartStore)
- ✅ 16 UI component тестов проходят
- ✅ 3 environment тестов проходят
- ❌ 34 service тестов написаны, но заблокированы MSW проблемой
