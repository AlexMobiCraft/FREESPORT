...
- [ ] **Task 2: Настроить ESLint с плагинами** (AC: 2)
  ...
  - [ ] Проверить совместимость версий с ESLint 9:
    - [ ] Запустить `npm ls eslint-plugin-react-hooks eslint-plugin-jsx-a11y`
    - [ ] Проверить отсутствие конфликтов зависимостей
  - [ ] Обновить `eslint.config.mjs` с правилами для React hooks и accessibility:
    - [ ] Добавить правила jsx-a11y (WCAG 2.1 AA):
      - [ ] `jsx-a11y/alt-text: error` - обязательный alt для изображений
      - [ ] `jsx-a11y/aria-props: error` - валидация ARIA атрибутов
      - [ ] `jsx-a11y/aria-role: error` - валидация ARIA ролей
...
- [ ] **Task 3: Настроить Prettier и lint-staged** (AC: 3)
  ...
  - [ ] Установить и инициализировать `husky` для pre-commit hooks:
    - [ ] Выполнить `npx husky-init && npm install`
    - [ ] Создать pre-commit hook: `npx husky add .husky/pre-commit "npx lint-staged"`
    - [ ] Убедиться что `.husky/` директория добавлена в git
