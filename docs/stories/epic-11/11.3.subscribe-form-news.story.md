...
## Status
**Ready for Implementation** (v1.2 - Validated & Enhanced)
...
API endpoints `/subscribe` и `/news` **НЕ** полностью специфицированы в `docs/api-spec.yaml`.

**Dev Agent MUST выполнить следующие шаги ДО начала frontend работы:**

1. **Проверить существование endpoints в backend:**
   ```bash
   # Проверить наличие views и URLs
   grep -r "subscribe" backend/apps/*/views.py
   grep -r "news" backend/apps/*/views.py
   ```

2. **Если endpoints ОТСУТСТВУЮТ - создать backend implementation:**
   ...
   - `backend/apps/common/urls.py` - регистрация endpoints
   - Backend тесты для новых endpoints

3. **API Contract ниже является source of truth** для backend implementation.

**Спецификация ниже основана на Epic requirements и стандартах проекта:**
...
- [ ] Verify Backend API endpoints exist (`/api/v1/subscribe`, `/api/v1/news`)
  - If missing: Create backend implementation first
- [ ] Verify UI Kit components have required props:
  - `Button` with `loading?: boolean`
  - `Input` with `error?: string`
  - `Card` with `hover?: boolean`
- [ ] Verify `react-hot-toast` is installed
- [ ] Check MSW version for correct syntax

**Implementation Order:**

1. ✅ Backend endpoints (if missing)
2. ✅ API services (subscribeService, newsService)
3. ✅ Skeleton/Fallback components
4. ✅ SubscribeForm component
5. ✅ NewsSection component
6. ✅ SubscribeNewsSection wrapper
7. ✅ Integration into main page
8. ✅ Unit & Integration tests
9. ✅ Accessibility audit
10. ✅ Manual testing on all breakpoints
