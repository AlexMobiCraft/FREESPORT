...
- ✅ Проверка дубликатов в `validate_email()`
- ✅ Реактивация неактивных подписок (не создаем дубликаты)
- ✅ Захват IP и User-Agent из request context
...
**Ключевые решения:**
- ✅ RESTful naming: `/subscribe/` (POST), `/news/` (GET)
...
- [ ] **2.4: Обновить URLs**
  - [ ] Добавить роуты в `backend/apps/common/urls.py`
...
REST_FRAMEWORK = {
    ...
    'DEFAULT_THROTTLE_RATES': {
        'anon': '10/hour',  # Лимит для subscribe endpoint
    }
}
```
...
**Next Action:** Передать Dev Agent для Phase 2: Backend Implementation
