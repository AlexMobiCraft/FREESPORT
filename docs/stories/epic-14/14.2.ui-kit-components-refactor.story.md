...
- [ ] 1.1 Открыть `frontend/src/components/ui/Button/Button.tsx`
- [ ] 1.2 Заменить `bg-[#1F1F1F]` → `bg-primary`
- [ ] 1.3 Заменить `hover:bg-[#3A3A3A]` → `hover:bg-primary-hover`
- [ ] 1.4 Добавить `active:bg-primary-active`
- [ ] 1.5 Обновить secondary вариант на токены
- [ ] 1.6 Добавить subtle вариант (если отсутствует)
- [ ] 1.7 Запустить тест: `npm test -- Button.test.tsx`

### Task 2: Рефакторинг Badge Component (AC: 2)
- [ ] 2.1 Открыть `frontend/src/components/ui/Badge/Badge.tsx`
- [ ] 2.2 Обновить `new` вариант: bg `#E7F3FF`, text `text-primary`
- [ ] 2.3 Обновить `hit` вариант: bg `#E0F5E8`, text `text-accent-success`
- [ ] 2.4 Обновить `promo` вариант: bg `#FFF0F5`, text `text-accent-promo`
- [ ] 2.5 Обновить `sale` вариант: bg `#FFE1E8`, text `text-accent-danger`
- [ ] 2.6 Добавить `discount` вариант: bg `#F0E7FF`, text `#7C3AED`
- [ ] 2.7 Запустить тест: `npm test -- Badge.test.tsx`

### Task 3: Рефакторинг Checkbox & Toggle (AC: 3, 4)
- [ ] 3.1 Открыть `frontend/src/components/ui/Checkbox/Checkbox.tsx`
- [ ] 3.2 Заменить `checked:bg-[#1F1F1F]` → `checked:bg-primary`
- [ ] 3.3 Заменить `checked:border-[#1F1F1F]` → `checked:border-primary`
- [ ] 3.4 Добавить `focus:ring-primary/12`
- [ ] 3.5 Открыть `frontend/src/components/ui/Toggle/Toggle.tsx`
- [ ] 3.6 Заменить `bg-[#1F1F1F]` → `bg-primary` для checked состояния
- [ ] 3.7 Запустить тесты: `npm test -- Checkbox.test.tsx Toggle.test.tsx`

### Task 4: Рефакторинг Tabs & Input (AC: 5, 6)
- [ ] 4.1 Открыть `frontend/src/components/ui/Tabs/Tabs.tsx`
- [ ] 4.2 Заменить `text-[#1F1F1F]` → `text-primary` для active состояния
- [ ] 4.3 Открыть `frontend/src/components/ui/Input/Input.tsx`
- [ ] 4.4 Заменить `focus:outline-[#1F1F1F]/60` → `focus:outline-primary/60`
- [ ] 4.5 Заменить `focus:ring-[#1F1F1F]/12` → `focus:ring-primary/12`
- [ ] 4.6 Запустить тесты: `npm test -- Tabs.test.tsx Input.test.tsx`

### Task 5: Рефакторинг Toast & Link (AC: 7, 8)
- [ ] 5.1 Открыть `frontend/src/components/ui/Toast/Toast.tsx`
- [ ] 5.2 Заменить все хардкод border цвета на семантические токены
- [ ] 5.3 Проверить наличие `frontend/src/components/ui/Link/Link.tsx`
- [ ] 5.4 Если существует: заменить цвета на `primary` токены
- [ ] 5.5 Запустить тест: `npm test -- Toast.test.tsx`

### Task 6: Рефакторинг Header (AC: 9)
- [ ] 6.1 Открыть `frontend/src/components/layout/Header.tsx`
- [ ] 6.2 Найти cart badge элемент
- [ ] 6.3 Заменить bg `#F9E1E1` → `#FFE1E8`
- [ ] 6.4 Заменить text `#A63232` → `text-accent-danger`

### Task 7: Unit Testing (AC: 10)
- [ ] 7.1 Запустить все тесты: `npm test`
- [ ] 7.2 Проверить coverage: `npm run test:coverage`
- [ ] 7.3 Обновить snapshots: `npm run test:update-snapshots`
- [ ] 7.4 Провести визуальный review обновлённых snapshots
- [ ] 7.5 Убедиться что coverage >= 80%

### Task 8: Code Review & Git Commit
- [ ] 8.1 Провести self-review изменений
- [ ] 8.2 Запустить линтер: `npm run lint`
