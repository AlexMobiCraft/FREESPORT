...
- [ ] Написать unit тесты для CartItemCard (AC: 1)
  ...
  - [ ] Тест error state при неудачном API запросе
...
- [ ] Написать unit тесты для CartSummary (AC: 2)
  ...
  - [ ] Тест без промокода (promoCode === null)
  - [ ] Тест disabled button (условный рендеринг Link/button)
  - [ ] Тест sticky positioning classes

- [ ] Написать unit тесты для QuantitySelector (AC: 1)
  - [ ] Создать `frontend/src/components/cart/__tests__/QuantitySelector.test.tsx`
  - [ ] Тест рендеринга с начальным значением
  - [ ] Тест инкремента/декремента
  - [ ] Тест disabled states при min/max
  - [ ] Тест keyboard input
...
- [ ] Написать accessibility тесты (AC: 8)
  - [ ] Проверить установку: `npm list @axe-core/react jest-axe`
  - [ ] Установить если отсутствует: `npm install -D @axe-core/react jest-axe`
