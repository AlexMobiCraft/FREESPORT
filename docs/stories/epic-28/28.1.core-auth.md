...
1. **Login Flow**: Пользователь вводит Email и Пароль. При успехе (200 OK) происходит перенаправление на URL из query параметра `?redirect=` или на Главную страницу. `authStore` обновляется статусом (isAuthenticated: true).
...
- [x] **Task 3.1**: Создать Login Page
  ...
  - [x] Fallback редирект на главную `/` если нет redirect param
