...
- [ ] **Task 2: Обновить JWT Login endpoint** (AC: 3)
  ...
  - [ ] 2.2 Добавить проверку `verification_status == 'pending'` перед выдачей токена
  - [ ] 2.3 Возвращать 403 с кодом `account_pending_verification` для pending users
  - [ ] 2.4 Добавить структурированное логирование попыток входа pending users (см. Dev Notes)
