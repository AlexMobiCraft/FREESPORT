...
- [ ] **Task 3: Обновить .env файлы** (AC: 2, 6)
  - [ ] 3.1 Добавить email переменные в `.env.example`
  - [ ] 3.2 Добавить комментарии с примерами для Gmail и Yandex
  - [ ] 3.3 Обновить `.env.prod.example`
...
| .env.example | `backend/.env.example` или root `.env.example` | MODIFY |
| .env.prod.example | `.env.prod.example` | MODIFY |
