...
1. Созданы management-команды `load_test_catalog` и `import_catalog_from_1c` с параметрами управления.
...
- [ ] **Создать/дополнить команду `import_catalog_from_1c` для импорта реальных данных из XML (AC: 1)**
  ...
  - [ ] Примечание: может реализовываться параллельно с задачами 1-5
...
- [ ] **Реализовать загрузку категорий с иерархией (AC: 2) - для реальных данных**
  ...
  - [ ] Создать/обновить модель `Category` (Выполнено в рамках Task 3.1.1-A).
...
python manage.py import_catalog_from_1c --file="backend/tests/legacy/Обмен с сайтом/goods/goods.xml"

# Загрузка торговых предложений (остатки, цены)
python manage.py import_catalog_from_1c --file="backend/tests/legacy/Обмен с сайтом/offers/offers.xml" --chunk-size=500

# Тестовый запуск импорта без записи в БД
python manage.py import_catalog_from_1c --file="backend/tests/legacy/Обмен с сайтом/goods/goods.xml" --dry-run
