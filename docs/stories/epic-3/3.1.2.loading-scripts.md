...
- [ ] **Создать команду `restore_db` (AC: 7)**
  - [ ] Создать каркас команды с базовыми параметрами
  - [ ] Реализовать параметр `--backup-file` (required) для указания файла бэкапа
  - [ ] Реализовать флаг `--confirm` для пропуска подтверждения
  - [ ] Реализовать восстановление базы из SQL файла через psql
  - [ ] Добавить валидацию существования файла бэкапа
  - [ ] Добавить проверку прав superuser
  - [ ] Реализовать подтверждение действия (y/n prompt)
  - [ ] Unit тесты (3 теста)

- [ ] **Создать команду `rotate_backups` (AC: 8)**
  - [ ] Создать каркас команды с базовыми параметрами
  - [ ] Реализовать параметр `--keep` (default=3) для указания количества хранимых копий
  - [ ] Реализовать поиск backup файлов в BACKUP_DIR
  - [ ] Реализовать сортировку по дате создания
  - [ ] Реализовать удаление старых файлов
  - [ ] Добавить dry-run режим для предпросмотра
