...
- [ ] **Реализовать логику регистрации на портале (AC: 2)**
  ...
  - [ ] Создать запись в SyncConflict: conflict_type='portal_registration_blocked' (для регистрации)
...
- [ ] **Создать записи в SyncConflict (AC: 5)**
  ...
  - [ ] Для каждого конфликта создавать запись с:
    - [ ] conflict_type: 'portal_registration_blocked' (для регистрации) или 'customer_data' (для импорта)
    ...
    - [ ] resolution_details: детали примененных изменений + conflict_source
