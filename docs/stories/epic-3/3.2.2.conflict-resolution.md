...
Эта история является критически важным компонентом процесса синхронизации клиентов между 1С и платформой:

1. **Импорт клиентов (Story 3.2.1)** - загрузка существующих клиентов из 1С
2. **Разрешение конфликтов (эта история)** - обработка дубликатов и несоответствий данных
3. **Синхронизация изменений (будущие истории)** - двусторонняя синхронизация обновлений

**Процесс разрешения конфликтов:**

- Выполняется автоматически при импорте клиентов из 1С
- Использует CustomerConflictResolver для обнаружения и разрешения конфликтов
...
- **Нечеткий поиск:** Python difflib.SequenceMatcher, python-Levenshtein (расстояние Левенштейна)
- **Обработка данных:** Django ORM с оптимизацией запросов (select_related, prefetch_related)
- **Нормализация данных:** Регулярные выражения (re) для телефонов, transliterate для транслитерации имен
- **Уведомления:** Django email backend, requests для интеграции с Slack/Teams API
- **Административный интерфейс:** Django Admin с кастомными actions и admin.ListFilter
- **Алгоритмы схожести:** fuzzywuzzy/FuzzyWuzzy для сравнения строк с порогами
- **Очередь задач:** Celery с Redis для асинхронной обработки конфликтов

**Переменные окружения:**

- `CONFLICT_RESOLUTION_THRESHOLD`: Порог схожести для нечеткого поиска (по умолчанию: 0.85)
- `CONFLICT_AUTO_RESOLVE`: Включить автоматическое разрешение конфликтов (True/False)
- `CONFLICT_NOTIFICATION_EMAIL`: Email для уведомлений о конфликтах
- `SLACK_WEBHOOK_URL`: URL для отправки уведомлений в Slack
- `TEAMS_WEBHOOK_URL`: URL для отправки уведомлений в Teams
...
- Модель ImportSession используется для отслеживания сессий импорта
...
- CustomerConflictResolver интегрируется с CustomerDataProcessor
- Конфликты логируются в CustomerSyncLog и SyncConflict
- Разрешенные конфликты обновляются в рамках ImportSession

### Edge Cases и обработка ошибок

**1. Низкая уверенность нечеткого поиска:**

- **Сценарий:** Схожесть данных ниже порога (85%)
- **Решение:** Создание записи в SyncConflict с типом 'manual_review'
- **Логирование:** Запись в CustomerSyncLog с деталями схожести

**2. Множественные потенциальные совпадения:**

- **Сценарий:** Найдено несколько кандидатов с высокой схожестью
- **Решение:** Создание записей для всех кандидатов, эскалация на модератора
- **Логирование:** Запись в CustomerSyncLog с полным списком кандидатов

**3. Конфликт ролей B2B/B2C:**

- **Сценарий:** Клиент имеет противоречивые роли в разных системах
- **Решение:** Приоритет данным из 1С, создание записи для ручной проверки
- **Логирование:** Запись в CustomerSyncLog с типом 'role_conflict'

**4. Ошибки нормализации данных:**

- **Сценарий:** Невозможно нормализовать телефон или имя
- **Решение:** Пропуск поля с логированием предупреждения
- **Логирование:** Запись в CustomerSyncLog с типом 'normalization_error'

**5. Сбой при слиянии записей:**

- **Сценарий:** Ошибка базы данных при попытке слияния
- **Решение:** Откат транзакции, создание записи для ручного разрешения
- **Логирование:** Запись в CustomerSyncLog с типом 'merge_error'

**6. Циклические зависимости:**

- **Сценарий:** Обнаружены циклические ссылки при слиянии
- **Решение:** Прерывание процесса, эскалация на администратора
- **Логирование:** Запись в CustomerSyncLog с типом 'circular_dependency'
...
- **Unit-тесты:** Для изолированного тестирования CustomerConflictResolver и алгоритмов нечеткого поиска
- **Интеграционные тесты:** Для проверки полной системы разрешения конфликтов с реальными данными
- **Тесты edge cases:** Для проверки обработки ошибок и особых сценариев
- **Performance тесты:** Для проверки производительности при обработке больших объемов конфликтов

**Ключевые тестовые сценарии:**

1. **Успешное разрешение email конфликта:**
   - Создание двух клиентов с одинаковым email
   - Проверка стратегии merge_by_email
   - Проверка корректности слияния данных

2. **Нечеткий поиск по ФИО:**
   - Создание клиентов с похожими именами (Иван Петров vs John Petrov)
   - Проверка алгоритма схожести с транслитерацией
   - Проверка ранжирования по confidence score

3. **Обработка множественных совпадений:**
   - Создание нескольких кандидатов с высокой схожестью
   - Проверка эскалации на модератора
   - Проверка создания записей в SyncConflict

4. **Разрешение конфликта ролей:**
   - Создание клиента с противоречивыми ролями B2B/B2C
   - Проверка стратегии manual_review
   - Проверка приоритета данных из 1С

5. **Обработка ошибок слияния:**
   - Имитация ошибки базы данных при слиянии
   - Проверка отката транзакции
   - Проверка создания записи для ручного разрешения

6. **Performance тестирование:**
   - Создание 1000+ конфликтов различных типов
   - Проверка времени обработки (<2 минут)
   - Проверка использования памяти

**Тестовые данные:**

- Создать тестовые наборы данных с различными типами конфликтов
- Включить пограничные случаи (низкая схожесть, множественные совпадения)
- Создать данные для тестирования транслитерации имен

**Критерии успеха тестирования:**

- Покрытие кода ≥90% для критических модулей
- Все edge cases обработаны корректно
- Performance тесты подтверждают обработку ≥1000 конфликтов за <2 минут
- False positive rate <5%

**Особые соображения по тестированию:**

- Использование mock данных для имитации различных сценариев конфликтов
- Тестирование интеграции с Django Admin интерфейсом
- Проверка работы уведомлений (email, Slack/Teams)
- Тестирование обратимости операций слияния
