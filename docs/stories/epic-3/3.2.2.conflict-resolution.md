...
2. При регистрации на портале: если клиент найден по ИНН/email, присваивается статус 'confirmed_client'.
...
- [ ] **Реализовать логику регистрации на портале (AC: 2)**
  ...
  - [ ] Если найден клиент из 1С (created_in_1c=True): присвоить статус 'confirmed_client'
...
- Присваивается статус 'confirmed_client', добавляется пароль для входа
...
- `FROM_EMAIL`: Email отправителя для уведомлений (из Django settings)
...
class CustomerConflictResolver:
    ...
    def resolve_conflict(self, existing_customer, onec_data, conflict_source):
        ...
        with transaction.atomic():
            ...
            # 6. Отправляем email уведомление
    ...
    def _handle_portal_registration(self, existing_customer, onec_data):
        ...
        # Только добавляем password и статус
        existing_customer.is_confirmed_client = True
        existing_customer.save()
        
        return {
            'action': 'confirmed_client',
            'message': 'Client from 1C confirmed, no data modified'
        }
...
- **Решение:** Логирование ошибки, конфликт все равно разрешается
- **Логирование:** ERROR в CustomerSyncLog, но status='success' для конфликта
...
- Проверить: статус 'confirmed_client' присвоен, данные НЕ изменены
