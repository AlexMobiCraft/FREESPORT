...
7. APM мониторинг включен (Django Debug Toolbar для dev)
...
- [ ] **Task 1: Создать функции расчета метрик** (AC: 2, 5)
  ...
  - [ ] Функция get_problem_metrics():
    ...
    - sync_conflicts (если модель SyncConflict существует)
...
- Django Debug Toolbar (для dev performance monitoring)

**Dependency на Stories 9.1-9.3:**

- Screenshots для admin-guide.md требуют завершенных Stories 9.1-9.3
...
def get_problem_metrics():
    ...
    return {
        ...
        # Добавить sync_conflicts если модель существует
    }
```
