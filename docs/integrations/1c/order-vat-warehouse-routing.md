...
| `Intex ОСНОВНОЙ` | `ИП Семерюк Д.В.`    | переменный (10% / 22%) | `37985d34-e784-11ef-9a90-fa163ea88911` |
| `2 ТЛВ склад`    | `ИП Терещенко Л.В.`  | фикс. 5%        | `b41f7acc-58e5-11f0-90b3-fa163ea88911`  |

`WAREHOUSE_NAME_BY_ID` переводит GUID из `rests.xml` в имя склада. `WAREHOUSE_RULES` задает организацию и — только для складов с единой ставкой — `vat_rate`. `ORGANIZATION_BY_VAT` остается fallback для случаев, когда склад не удалось определить однозначно.
...
Для складов без `vat_rate` в `WAREHOUSE_RULES` (`1 СДВ склад`, `Intex ОСНОВНОЙ`) первый шаг даёт `None` и цепочка продолжается к `ProductVariant.vat_rate` — ставке из `goods.xml`.
...
Это позволяет корректно экспортировать `Intex ОСНОВНОЙ` и `1 СДВ склад` даже при переменной ставке НДС.
...
| -------- | ------------------ |
| Склад **всегда** одна ставка НДС (как `2 ТЛВ склад` → 5%) | добавить `"vat_rate": X` |
| Склад имеет товары с **разными** ставками (как `1 СДВ склад`) | только `"organization"`, без `vat_rate` |

```python
"WAREHOUSE_RULES": {
    "1 СДВ склад":    {"organization": "ИП Семерюк Д.В."},                    # переменный НДС
    "Intex ОСНОВНОЙ": {"organization": "ИП Семерюк Д.В."},                    # переменный НДС
...
- **Решение:**
  ...
  3. `WAREHOUSE_RULES`: убран `vat_rate` у `1 СДВ склад` и `Intex ОСНОВНОЙ` — у них переменный НДС.
