...
**Причина проблемы**: При импорте из 1С поле "Детский размер" (булевый флаг с значением "Да"/"Нет") ошибочно записывалось в `size_value` вместо реального размера из поля "Размер\_...".
