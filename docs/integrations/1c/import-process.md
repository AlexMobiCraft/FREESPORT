...
Общий `import_files` эта задача **не подрезает**: связи «обмен изображениями ↔
будущий XML» протокол не задаёт, и картинка старше суток остаётся законным
источником для `goods.xml`, который приедет завтра. Рост общего каталога — открытый
пункт tech-debt, а не задача уборки по возрасту.
