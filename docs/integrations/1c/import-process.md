...
него нет, а чужие ему не принадлежат. Изображения остаются в
`goods/import_files/` и достаются задаче своего `goods`-сегмента; cleanup команды
...
### 6. Неоднозначное имя останавливает импорт
