...
папка удаляется после обмена (`remove_session_dirs`). Осиротевшее при падении
воркера подбирает `cleanup_stale_exchange_dirs` — периодическая задача с порогом
**24 часа**, она же подрезает старые файлы общего `import_files` (после изоляции
его не чистит ни одна сессия). Задача обязана быть объявлена в
