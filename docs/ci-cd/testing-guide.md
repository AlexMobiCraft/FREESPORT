...
5. Смёржите PR (`gh pr merge <N> --merge`)
6. Синкните релиз: `git push origin origin/develop:refs/heads/main`

**Ожидаемый результат**:

- ✅ Все 6 required-чеков отработали на PR и позволили мерж
- ✅ После мёрджа в `develop` не запустилось ни одного прогона
- ✅ Push в `main` прошёл fast-forward, без merge-коммита
- ✅ Push в `main` запустил только `deploy.yml` и `sync-to-public.yml`
