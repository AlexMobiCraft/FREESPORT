...
Деплой FREESPORT Platform осуществляется через GitHub Actions workflow `deploy.yml` — это единственное место, где собираются прод-образы и выполняется деплой.
Используется **одно production-окружение**. Запускается двумя способами:

- **push в `main`** — штатный релиз (синк `develop` → `main` fast-forward пушем);
- **ручной запуск** — повторный деплой или откат по входу `image_tag`.

Сборка тега `:production` разрешена только с ветки `main`: иначе следующий деплой
поднял бы на проде код другой ветки.

## Архитектура деплоя

```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│ Push в main     │────▶│ Build Images    │────▶│ Deploy Server   │
│ или ручной      │     │ GHCR + approval │     │ SSH + approval  │
│ запуск (откат)  │     │ (при откате нет)│     │                 │
└─────────────────┘     └─────────────────┘     └─────────────────┘
                                                        │
                                                        ▼
                                               ┌─────────────────┐
