...
> ⚠️ **Пока не применены.** `main` и `develop` отдают «Branch not protected»; ниже — намерение, а не текущее состояние. Оставшиеся блокеры — в `deferred-work.md`.

### Ветка `main`

- 🔒 **Защищена от прямого пуша**
- ✅ **Требуется Pull Request** для изменений
- 👥 **Одобрения не требуются** (`required_approving_review_count: 0`) — мейнтейнер один, свой PR апрувить нельзя
