...
### Этап 2.5: Привязка существующего 1С-клиента к регистрации

Если клиент уже выгружен из 1С (`created_in_1c=True`, пароль на портале ещё не задан) и регистрируется на сайте с теми же реквизитами, система привязывает регистрацию к найденной записи вместо создания дубля или жёсткого отказа.
...
В обоих случаях ответ `UserRegistrationView.post()` не содержит JWT и не раскрывает ФИО/роль найденной записи. `UserConsent` при этом **создаётся** (стори 41.2, 2026-09-05): форму заполнил живой человек, галочку поставил он, его ПДн уже обрабатываются — доказательство согласия обязано лечь в журнал независимо от того, чем регистрация закончилась. С 2026-09-09 такие записи помечаются источником `1c_link` (стори 41.9); версия текста у них та же, что и при обычной регистрации, — человек видел ту же форму.
