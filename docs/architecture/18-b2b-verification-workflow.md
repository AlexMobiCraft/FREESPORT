...
> **Розничная саморегистрация отключена.** `SELF_SERVICE_ROLES` не содержит
> `retail`, а поле `role` обязательно (`Meta.extra_kwargs`), иначе модельный
> `default="retail"` создавал бы розничный аккаунт в обход запрета. Ветки
> `role == "retail"` ниже сохранены в коде как защита при прямом вызове
> сериализатора, но через `/api/v1/auth/register/` недостижимы: такая заявка
> отклоняется с 400 `{"role": ["Недопустимая роль для регистрации."]}`.
> Уже существующие розничные аккаунты продолжают входить и работать.
...
1. **Frontend форма регистрации** отправляет данные:
   ...
   > [!IMPORTANT]
   > Story 41.9: `pdp_consent` и `pdp_consent_text_version` **обязательны** — без них
   > запрос получает `400` (у версии — машинный код `consent_text_outdated`).
   > `marketing_consent_text_version` обязателен только при `marketing_consent: true`.
   > Версии в примере — действующие на дату правки; они **устаревают** при каждой
   > правке формулировки чекбокса, поэтому копировать их из документа нельзя.
   > Канонический источник — реестр `backend/apps/common/consent_texts.json`
   > (действующая ревизия каждой поверхности) и собранная из него константа фронта
   > `frontend/src/constants/consentTexts.ts`, которую и отправляют формы.
   > Назначение поля — назвать формулировку, которую клиент показал человеку. Сервер
   > проверяет только, что версия действующая, и факт показа подтвердить не может,
   > поэтому сторонний API-клиент обязан присылать версию текста, который он
   > действительно показал (границы гарантии — `11-security-performance.md`, §5.1).

2. **UserRegistrationSerializer валидация:**
   - Проверка уникальности email
   - Проверка `pdp_consent` и сверка присланных версий формулировок с реестром
   - Валидация обязательных B2B полей
   - Создание пользователя с `is_verified = False`
   - Запись одной или двух `UserConsent` (`pdp_contract`, при согласии — ещё
     `marketing_email`) с `source = "registration"` и версией из реестра
...
### Этап 2.5: Привязка существующего 1С-клиента к регистрации — ОТКЛЮЧЕНА (латентный сценарий)

> [!WARNING]
> **Ветка отключена с `ffee94d5` (2026-07-26) и по HTTP недостижима.** Метод
> `UserRegistrationSerializer._link_matched_1c_customer` в коде остался, но его
> никто не вызывает: регистрация с совпавшими реквизитами создаёт обычную заявку.
> Причина отключения — ИНН публичен, а email заполнен лишь у 149 из 4606 записей
> 1С, то есть знание одного ИНН позволяло занять контрагента чужой компании.
> Раздел ниже описывает, **как ветка работала и как она заработает снова**, если
> её вернут с настоящим доказательством права на компанию. Ничего из описанного
> сегодня не происходит.

Замысел: если клиент уже выгружен из 1С (`created_in_1c=True`, пароль на портале ещё не задан) и регистрируется на сайте с теми же реквизитами, система привязывает регистрацию к найденной записи вместо создания дубля или жёсткого отказа.

**Идентификация:** `UserRegistrationSerializer.validate()` вызывает `CustomerIdentityResolver().identify_customer({"email": ..., "tax_id": ...})` (приоритет: `tax_id` → `email`). Матч учитывается только если найденная запись удовлетворяет всем условиям: `created_in_1c=True`, `verification_status != "verified"`, `role != "retail"` (retail временно вне скоупа — см. ниже). Любой другой матч (портальный дубликат, уже верифицированная 1С-запись, retail-матч) отклоняется как обычный дубликат: 400 под `email`/`tax_id`.

**Ветвление по email:**

- **Email формы совпадает с email в 1С** — `UserRegistrationSerializer.create()` в одной атомарной операции устанавливает пароль и `verification_status="pending"`, ставит `send_admin_verification_email.delay()` (тот же путь, что и для обычной B2B-заявки, включая финальное одобрение через `approve_b2b_users`). Пароль и переход в `pending` выполняются одним вызовом — иначе возникает окно, в которое рабочий пароль уже есть, а `UserLoginView` блокирует вход только по `verification_status == "pending"` (`is_active` не проверяется).
- **Email формы отличается** — пароль пока не сохраняется. `send_portal_link_confirmation_email.delay()` отправляет ссылку-подтверждение на **новый** email из формы (доказывает лишь его живость, никогда не уходит на старый email из 1С). После перехода по ссылке `PortalLinkConfirmView` (`POST /api/v1/auth/portal-link/confirm/`) декодирует токен (`django.core.signing`, единственная точка финализации — отдельного validate-token эндпоинта нет), заменяет email, устанавливает пароль и переводит запись в `pending` тем же атомарным шагом.

В обоих случаях ответ `UserRegistrationView.post()` не содержит JWT и не раскрывает ФИО/роль найденной записи. Поведение согласия сохранено на случай безопасного возврата ветки: `UserConsent` **создавался бы** (стори 41.2, 2026-09-05) — форму заполнил живой человек, галочку поставил он, его ПДн уже обрабатываются, поэтому доказательство согласия обязано лечь в журнал независимо от того, чем регистрация закончилась. Источник таких записей — `1c_link` (стори 41.9); версия текста у них та же, что и при обычной регистрации, — человек видел ту же форму. Код записи в `UserRegistrationView.post` живой и выбирает источник по уже вычисленному `pending_1c_link`; сегодня флаг всегда ложен, потому что выставить его может только отключённый `_link_matched_1c_customer`. Ветка покрыта тестами через патч сериализатора (`backend/tests/integration/test_auth_registration_consent.py`, помощник `_pending_create`), а не по HTTP.

**"1C wins":** ФИО/роль/название компании формы никогда не переопределяют данные найденной записи. Email — единственное исключение, и то лишь после явного подтверждения владения новым адресом.

**Retail временно вне скоупа:** доступ к порталу через привязку 1С-записи предоставляется только B2B. Совпадение email с retail-записью из 1С обрабатывается как обычный дубликат (400 под `email`), без ссылки и без привязки.
