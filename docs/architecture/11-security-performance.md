...
- Версия вычисляется как `<метка>-<первые 8 hex sha256 текста>` из реестра `apps/common/consent_texts.json`, поэтому правка текста меняет версию сама: пропустить бамп механически невозможно. Историческая версия разрешается обратно в свой текст через `resolve_consent_text()` — история ревизий дополняется, а не переписывается.
