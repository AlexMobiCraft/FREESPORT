...
- Фиксирует каждое согласие пользователя на обработку ПДн (`pdp_contract`) и рекламные рассылки (`marketing_email`) отдельной строкой; журнал ведётся как append-only.
...
- **Границы append-only.** Правку и удаление закрывают admin (`has_add_permission` / `has_change_permission` / `has_delete_permission` возвращают `False`) и отсутствие API, который писал бы в журнал что-либо кроме вставки. Механической неизменяемости на уровне БД нет: `UserConsent.objects.update()/delete()`, `TRUNCATE` и любой прямой SQL журнал меняют — ни триггера, ни `REVOKE UPDATE/DELETE` для роли приложения не заведено. Гарантия здесь организационная (нет пути в коде), а не техническая.
...
- Валидация: `pdp_consent` обязателен в payload и должен быть истинным.
  **Известная асимметрия (не исправлена):** строгую проверку на JSON boolean `true` делает только подписка (`SubscribeSerializer.validate` сверяет `initial_data` через `is not True`). Регистрация использует DRF `BooleanField` и потому принимает truthy-значения `"true"`, `"on"`, `"1"`, `1` (`backend/apps/users/serializers.py`). Замечание зафиксировано ревью стори 41.9 как pre-existing и вынесено в `deferred-work.md`; до его закрытия утверждать строгую проверку для регистрации нельзя.
...
- **Повторная подписка уже активного email тоже пишет обе записи** (Story 41.9, шестой круг ревью, 2026-09-10). Каждое явное подтверждение — отдельный факт согласия: после правки формулировки именно оно доказывает согласие на новую редакцию, и терять его нельзя. Ответ неотличим от новой подписки (нейтральный `200`, см. раздел об enumeration ниже), строка `Newsletter` активного подписчика не меняется. Та же запись делается, если параллельный запрос успел создать подписку первым: гонка на уникальном email разрешается повторным чтением строки, а не ответом «уже подписан».
...
**Аудитируемость формулировки — Story 41.9 (2026-09-09):**

- Дата, IP и User-Agent не отвечают на главный вопрос ФЗ-152 ст. 9 — **на что именно** человек соглашался. Формулировка чекбокса живёт во фронтенде и меняется: после правки текста (стори 41.3) прежние записи молча стали относиться к формулировке, которой человек не видел.
- `UserConsent` получил два поля: `source` (`newsletter` | `registration` | `1c_link` | `unknown`) и `consent_text_version`. `unknown` зарезервирован за строками, существовавшими до миграции `0019`; код, пишущий новое согласие, обязан передать фактические значения — иначе вставка падает на CHECK-ограничении. Источник вдобавок ограничен перечислением на уровне БД (`userconsent_source_valid`, миграция `0020`): Django `choices` проверяются формами и `full_clean()`, а прямой `objects.create()` их не касается, и значение вне списка легло бы в юридически значимый журнал молча.
  **Асимметрия двух ограничений.** У `source` база проверяет принадлежность списку значений, у `consent_text_version` — только непустоту (`userconsent_text_version_required`). Строку, не соответствующую ни одной ревизии реестра, БД примет: набор допустимых версий меняется правкой JSON, и зашивать его в CHECK значило бы требовать миграцию на каждую правку формулировки. Что версия принадлежит реестру, обеспечивают точки записи (значение берётся из `current_consent_text_version()`, а не из запроса) и сверка присланной клиентом версии — то есть код, а не схема.
- Версия вычисляется как `<метка>-<первые 32 hex sha256 текста>` из реестра `apps/common/consent_texts.json`, поэтому правка текста меняет версию сама: пропустить бамп механически невозможно. Длина хеша — 128 бит (шестой круг ревью, 2026-09-10): при прежних 8 hex (32 бита) коллизия подбиралась перебором — при одной метке «Текст согласия 1115» и «Текст согласия 1675» давали одну версию `f4ef3d1f`, и две разные формулировки стали бы неразличимы в журнале. Версия с меткой-датой занимает 43 символа из `max_length=64`. Историческая версия разрешается обратно в свой текст через `resolve_consent_text()` — история ревизий дополняется, а не переписывается.
- `source` различает три источника при **двух** точках записи в коде: `registration` и `1c_link` — одна и та же пара `create` в `UserRegistrationView.post`, различаемая флагом `pending_1c_link`.
- Соответствие текста форм и реестра стережёт Vitest-тест `frontend/src/__tests__/consent-texts-registry.test.tsx`: он рендерит все четыре формы с чекбоксом согласия и сверяет доступное имя чекбокса с текущей ревизией реестра. Тест обязан падать (не пропускаться) при отсутствии файла реестра — бэкенд-тесты сделать этого не могут, в тестовый контейнер смонтирован только `backend/`.
- **Клиент присылает показанную версию, сервер её сверяет (доработка по ревью, 2026-09-09).** `POST /api/v1/subscribe/` требует `consent_text_version`, `POST /api/v1/auth/register/` — `pdp_consent_text_version` и (при `marketing_consent: true`) `marketing_consent_text_version`. Несовпадение с действующей ревизией или отсутствие поля — `400` с кодом `consent_text_outdated` и требованием обновить страницу. Версии сверяются field-level валидаторами сериализаторов, а не в object-level `validate()`, поэтому код приходит и тогда, когда в запросе есть другие ошибки (email, роль, галочка, несовпадение паролей) — DRF при любой field-level ошибке `validate()` не вызывает (пятый круг ревью, 2026-09-10). Значения приходят из константы `frontend/src/constants/consentTexts.ts`, собранной в тот же бандл, что и сам текст. Запрошенная у сервера версия не дала бы ничего — старая вкладка получила бы актуальное значение и записала согласие на формулировку, которой не видела. В журнал кладётся значение из реестра, а не присланное клиентом.
  **Что эта проверка даёт и чего не даёт.** Она подтверждает, что запрос пришёл из бандла, в котором лежит действующая ревизия, — то есть отсекает вкладку, открытую до правки формулировки. Доказательством факта показа текста конкретному человеку строка версии не является: произвольный API-клиент (curl, скрипт интеграции, сторонний фронтенд) присылает её, ничего не отрисовав, и сервер такой запрос примет. Связку «версия ↔ отрисованный текст» держит только официальный фронтенд, и сторожит её Vitest-тест реестра, а не сервер.
- **История ревизий страхуется сверкой.** Раздел `known_versions` реестра перечисляет версии всех когда-либо действовавших формулировок; загрузчик требует точного совпадения этого списка с набором, вычисленным из ревизий. Изменить текст старой ревизии или удалить её «как устаревшую», не тронув список, нельзя — реестр перестанет загружаться, назвав пропавшие версии. Это процедурный страж от односторонней правки, а не механическая неизменяемость: список лежит в том же редактируемом JSON, и согласованная замена ревизии вместе с её строкой пройдёт. Дубли ключей в JSON отбраковываются (`object_pairs_hook`), а версия длиннее `max_length=64` не проходит загрузку.
- `source` и `consent_text_version` показаны в админке в списке и фильтрах; оба read-only, запреты add/change/delete сохранены (границы append-only — выше).
- **Не покрыто:** `policy_version` остаётся константой `"1.0"`. Текст политики ПДн живёт в БД (модель `Page`, slug `privacy-policy`) и ревизий не имеет, поэтому какая редакция политики действовала в момент согласия — из журнала не восстановить (см. `deferred-work.md`).
