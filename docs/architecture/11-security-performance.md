...
- **История ревизий защищена от правки.** Раздел `known_versions` реестра перечисляет версии всех когда-либо действовавших формулировок; загрузчик требует точного совпадения этого списка с набором, вычисленным из ревизий. Изменить текст старой ревизии или удалить её «как устаревшую» нельзя — реестр перестанет загружаться, назвав пропавшие версии. Дубли ключей в JSON отбраковываются (`object_pairs_hook`), а версия длиннее `max_length=64` не проходит загрузку.
