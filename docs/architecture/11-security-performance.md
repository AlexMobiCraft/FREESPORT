...
- `UserConsent` получил два поля: `source` (`newsletter` | `registration` | `1c_link` | `unknown`) и `consent_text_version`. `unknown` зарезервирован за строками, существовавшими до миграции `0019`; код, пишущий новое согласие, обязан передать фактические значения — иначе вставка падает на CHECK-ограничении.
