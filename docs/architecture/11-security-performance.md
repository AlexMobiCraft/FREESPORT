...
- `source` и `consent_text_version` показаны в админке в списке и фильтрах; оба read-only, журнал остаётся append-only.
