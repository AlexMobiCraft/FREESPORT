...
└── tests/
    ...
    │   ├── test_1c_integration.py  # Интеграция с 1С
...
#### 10.2.3. E2E-тесты (внешняя директория `e2e/`)

- **Назначение**: Тестирование полных пользовательских сценариев через браузер
- **Технологии**: `Playwright`
- **Пример**: "Пользователь логинится, добавляет товар в корзину, оформляет заказ"

#### 10.2.4. Устаревшие тесты (`tests/legacy/`)

**⚠️ ВАЖНО**: 
- Содержат тесты для устаревшего функционала
