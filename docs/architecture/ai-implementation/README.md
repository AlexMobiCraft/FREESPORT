...
- 🚨 **НЕ проставляйте `unit`/`integration`/`performance` вручную** — их назначает каталог теста через хук в `backend/conftest.py` (см. `backend/docs/testing-standards.md`). Явный маркер ставится только там, где каталог классифицирует тест неверно.
