...
**Инвариант дерева категорий 1С:** `Category` хранит полную исходную иерархию из `groups.xml`, включая корневой якорь `СПОРТ` (`parent=None`). Публичная навигация витрины не строится напрямую от всех `parent=None`: эндпоинт `/api/v1/categories-tree/` отдаёт прямых детей `СПОРТ` как корневые узлы и скрывает служебные категории (`uncategorized`, `onec-unresolved-category`, `Без категории`) и ветки-заглушки `Категория <id>`.
...
> [!IMPORTANT]
> Актуализация от `2026-07-11`: поле `User.customer_code` нигде не заполнялось автоматически (ни при регистрации, ни при импорте контрагентов из 1С), из-за чего checkout был заблокирован почти для всех пользователей. Теперь `OrderNumberingService.next_master_number()` при отсутствии кода лениво присваивает следующий свободный 5-значный код через новый singleton-счётчик `CustomerCodeSequence` (`apps/orders/models.py`, `db_table = "customer_code_sequences"`, единственная строка `pk=1`, поле `last_value`). Уже проставленный вручную код никогда не перезаписывается.
