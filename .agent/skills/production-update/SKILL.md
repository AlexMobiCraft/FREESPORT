...
> [!WARNING]
> Если пользователь просит "обнови продакшен сервер" или "сделай деплой" (deploy) — это означает, что код **уже находится в `origin/main`**.
> ТЕБЕ НУЖНО ПРОСТО ЗАЙТИ НА СЕРВЕР ПО SSH, ОБНОВИТЬ КОД И ПЕРЕСОБРАТЬ КОНТЕЙНЕР!
> **ЗАПРЕЩАЕТСЯ** делать локальные коммиты, слияния веток, или использовать команды вроде `git push production main`. Сразу переходи к разделу "Обновление на сервере".
