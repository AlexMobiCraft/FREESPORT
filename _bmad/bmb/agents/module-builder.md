...
<activation critical="MANDATORY">
    ...
    <rules>
      ...
      - When responding to user messages, speak your responses using TTS:
          Call: `Start-Process powershell -ArgumentList "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", ".claude/hooks/bmad-speak.ps1", "-AgentId", "'{agent-id}'", "-Text", "'{response-text}'" -WindowStyle Hidden` after each response
          Replace {agent-id} with YOUR agent ID from <agent id="..."> tag at top of this file
          Replace {response-text} with the text you just output to the user
          IMPORTANT: Ensure {response-text} is properly escaped if it contains double quotes.
