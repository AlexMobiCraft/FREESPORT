...
class DocsValidator:
    ...
    def validate_cross_references(self) -> bool:
        ...
        if all_valid:
            self.print_info(
                f"Все кросс-ссылки валидны ({len(md_files)} файлов проверено)"
            )
    ...
    def find_obsolete_terms(self) -> bool:
        ...
        if found_issues:
            ...
            for file_path, issues in found_issues.items():
                print(
                    f"{Colors.BOLD}{file_path}{Colors.RESET} ({len(issues)} упоминаний):"
                )
            ...
            self.print_warning(
                f"Найдено {sum(len(v) for v in found_issues.values())} устаревших терминов"
            )
            return False
        else:
    ...
    def check_api_coverage(self) -> bool:
        ...
        coverage = (
            len(documented_endpoints) / len(api_endpoints) * 100
            if api_endpoints
            else 100
        )
        print(
    ...
    def _find_api_endpoints(self) -> Set[str]:
        ...
        for py_file in self.backend_dir.rglob("*.py"):
            ...
            is_views_module = (
                any(part == "views" for part in parts[:-1])
                or py_file.name == "views.py"
            )
            ...
            if is_views_module:
                ...
                for match in re.finditer(class_pattern, content):
                    ...
                    if any(
                        keyword in base_classes for keyword in ("ViewSet", "APIView")
                    ):
    ...
    def check_dates_freshness(self) -> bool:
        ...
        date_patterns = [
            ...
            (
                r"\*\*Последнее обновление:\s*(\d{2}\.\d{2}\.\d{4})",
        ...
        if outdated_files:
            ...
            for item in sorted(
                outdated_files, key=lambda x: x["days_old"], reverse=True
            )[:10]:
                print(f"  {item['file']}")
                print(
                    f"    {item['label']}: {item['date']} ({item['days_old']} дней назад)"
                )
        ...
        for category, sections in required_sections.items():
            ...
            for md_file in category_dir.rglob("*.md"):
                ...
                try:
                    ...
                    if missing_sections:
                        issues_found.append(
                            {"file": str(relative_path), "missing": missing_sections}
                        )
    ...
    def print_summary(self):
        ...
        print(
            f"  {Colors.YELLOW}[!] Предупреждений: {len(self.warnings)}{Colors.RESET}"
        )
...
def main():
    """Главная функция."""
    parser = argparse.ArgumentParser(description="Валидация документации FREESPORT")
    parser.add_argument(
        "command",
        nargs="?",
        default="validate",
