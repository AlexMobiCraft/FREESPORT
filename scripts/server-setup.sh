...
setup_project() {
    ...
    sudo -u freesport chmod +x scripts/*.sh
...
#!/bin/bash
cd /opt/freesport
sudo -u freesport ./scripts/deploy.sh update
EOF
...
#!/bin/bash
cd /opt/freesport
sudo -u freesport ./scripts/deploy.sh backup
EOF
...
/var/log/freesport/*.log {
    ...
    postrotate
        cd /opt/freesport && sudo -u freesport docker compose -f docker-compose.prod.yml restart nginx
    endscript
}
EOF
...
initial_deploy() {
    ...
    cd $PROJECT_DIR
    sudo -u freesport ./scripts/deploy.sh init
...
show_next_steps() {
    ...
    echo "- Статус сервисов: cd $PROJECT_DIR && ./scripts/deploy.sh status"
    echo "- Просмотр логов: cd $PROJECT_DIR && docker compose -f docker-compose.prod.yml logs -f"
    echo "- Обновление: cd $PROJECT_DIR && ./scripts/deploy.sh update"
    echo ""
    echo "Важные файлы:"
