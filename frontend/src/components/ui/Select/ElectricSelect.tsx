...
export function ElectricSelect({
  ...
  return (
    <div ref={containerRef} className={cn('relative', className)}>
      ...
      >
        <span
          className={selectedOption ? 'text-[var(--foreground)]' : 'text-[var(--color-text-muted)]'}
        >
        ...
        <ChevronDown
          className={cn(
            ...
            isOpen ? 'rotate-180 text-[var(--color-primary)]' : 'text-[var(--color-text-secondary)]'
          )}
        />
      </button>
        ...
        >
          {options.map(option => (
            ...
            >
              ...
              {option.value === value && <Check className="w-4 h-4 text-[var(--color-primary)]" />}
            </button>
          ))}
        </div>
      )}
