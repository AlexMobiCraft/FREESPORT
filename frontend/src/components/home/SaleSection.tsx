 ...
 *
 * Блок "Распродажа" на главной странице.
 * Отображает товары с флагом is_sale=true в горизонтальной ленте.
 *
 * Features:
...
export const SaleSection: React.FC = () => {
  ...
  const fetchSale = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await productsService.getSale();
      setProducts(data);
    } catch (err) {
      console.error(err);
  ...
  return (
    ...
    >
      {/* Заголовок секции */}
      <h2 id="sale-heading" className="text-3xl font-bold mb-8 text-text-primary">
        Распродажа
      </h2>
      ...
      {isLoading && (
        <div
          role="status"
          aria-label="Загрузка товаров распродажи"
