    ...
    const onSubmit = async (data: SubscribeFormData) => {
        try {
            ...
            if (error instanceof Error && error.message === 'already_subscribed') {
                toast.error('ЭТОТ EMAIL УЖЕ ПОДПИСАН', { style: { borderRadius: '0', background: '#000', color: '#fff', border: '1px solid red' } });
            } else {
                toast.error('ОШИБКА ПОДПИСКИ', { style: { borderRadius: '0', background: '#000', color: '#fff', border: '1px solid red' } });
            }
        }
    };

    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-w-xl mx-auto md:max-w-none md:mx-0">
            <div>
